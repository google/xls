...
// The reference code is based on the go Payne-Hanek implementation,
...
using PackedFloat64 = PackedTupleView<PackedBitsView<1>, PackedBitsView<11>, PackedBitsView<52>>;
...
double unflatten(uint64_t in) {
  return absl::bit_cast<double>(in);
}

// Cast from double to uint64_t
uint64_t flatten(double in) {
  return absl::bit_cast<uint64_t>(in);
}

// Return the number of leading zeros in num;
uint64_t clz(uint64_t num) {
  uint64_t leading_zeros = 0;
  for(uint64_t mask = static_cast<uint64_t>(1) << 63;
      !(mask & num);
      mask = mask >> 1) {
    leading_zeros++;
  }
...
ResultT ComputeExpected(double x) {
  ...
  int exp = int((ix>>FP64_EXP_SHIFT)&FP64_EXP_MASK) - FP64_EXP_BIAS - FP64_EXP_SHIFT;
  ...
  absl::uint128 z2_prod = static_cast<absl::uint128>(z2) * static_cast<absl::uint128>(ix);
  uint64_t z2hi = absl::Uint128High64(z2_prod);
  absl::uint128 z1_prod = static_cast<absl::uint128>(z1) * static_cast<absl::uint128>(ix);
...
ResultT ComputeActual(FpTrigReduce * jit_wrapper, double x) {
  ...
  jit_wrapper->Run(packed_x, packed_result);
  result.j = result.j & 7;
  return result;
}
...
void LogMismatch(uint64_t index, double input, ResultT expected,
  ...
  XLS_LOG(ERROR) << absl::StrFormat(
      ...
      "  Actual: %d, %#08x\n"
      "  Expected: %d, %#08x\n",
      index, absl::bit_cast<uint64_t>(input), input,
      actual.j, absl::bit_cast<uint64_t>(actual.fraction),
      expected.j, absl::bit_cast<uint64_t>(expected.fraction));
