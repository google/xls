...
// Combines two FifoConfigs into one, assuming that they are concatenated and
// that a value passes through `a` before it passes through `b`.
std::optional<FifoConfig> combineFifoConfigs(std::optional<FifoConfig> a,
                                             std::optional<FifoConfig> b) {
  if (!a && !b) {
    return std::nullopt;
  }
  if (!a) {
  ...
  bool bypass = a->getBypass() || b->getBypass();
  // The depth of the combined FIFO is the sum of the depths of the two FIFOs,
  // as a notional value passes through one FIFO and then the other.
  int64_t depth = a->getFifoDepth() + b->getFifoDepth();
  // registerPushOutputs affects the "push" ports, which are input ports.
  bool registerPushOutputs = a->getRegisterPushOutputs();
  // registerPopOutputs affects the "pop" ports, which are output ports.
  bool registerPopOutputs = b->getRegisterPopOutputs();
  return FifoConfig::get(a->getContext(), depth, bypass, registerPushOutputs,
                         registerPopOutputs);
}
...
class SendOfBlockingReceiveOp : public OpRewritePattern<SSendOp> {
 public:
  ...
  LogicalResult matchAndRewrite(SSendOp send,
    ...
    auto sendChanOp = sendChan.getDefiningOp<SchanOp>();
    auto recvChanOp = recvChan.getDefiningOp<SchanOp>();

    if (sendChanOp && recvChanOp) {
      // Both channels are interior channels, so we can pick either one to be
      // the final channel. Moreover, we can combine the FifoConfigs of the two
      // channels.
    ...
    } else if (sendChanOp && !sendChanOp.getFifoConfig().has_value()) {
    ...
    } else if (recvChanOp && !recvChanOp.getFifoConfig().has_value()) {
      ...
      //
      // Or, one is an interior channel that has a FifoConfig and the other is
      // a result/argument that cannot have a FifoConfig.
      return failure();
    }
