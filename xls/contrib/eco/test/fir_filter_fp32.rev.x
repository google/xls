...
pub fn fir_filter_float32<NUM_TAPS:u32, NUM_SAMPLES:u32,
  ...
  let coefficent_sum = for(tap_idx, acc): (u32, F32) in u32:0..NUM_TAPS {
    float32::add(acc, coefficients[tap_idx])
  }(float32::zero(u1:0));
  let normalized_coefficients = for(tap_idx, acc): (u32, F32[NUM_TAPS]) in u32:0..NUM_TAPS {
    let new_coeff = float32::mul(coefficients[tap_idx], float32::fast_rsqrt(coefficent_sum));
  ...
  for(out_idx, fir_output): (u32, F32[NUM_OUTPUTS]) in u32:0..NUM_OUTPUTS {
    ...
    let point_output = for(tap_idx, acc): (u32, F32) in u32:0..NUM_TAPS {
      ...
      let product = float32::mul(normalized_coefficients[tap_idx],
                                           samples[sample_idx]);
