...
Required flags:
  --input_ir_path       Path to the input IR file to patch.
  --input_patch_path    Path to the IrPatchProto generated by ged_main.
  --output_ir_path      Destination for the patched IR.

Optional flags:
  --input_schedule_path Path to a textproto PackageScheduleProto used to
...
static absl::StatusOr<xls::PipelineSchedule> PipelineScheduleFromProto(
  ...
  return xls::PipelineSchedule::Create(
      function, std::move(cycle_map),
      xls::PipelineSchedule::Options{.min_clock_period_ps =
                                         min_clock_period_ps});
}
static absl::Status RealMain(
  ...
  if (ir_path.empty()) {
    return absl::InvalidArgumentError("Missing --input_ir_path flag.");
  }
  if (patch_path.empty()) {
    return absl::InvalidArgumentError("Missing --input_patch_path flag.");
  }
...
int main(int argc, char** argv) {
  std::vector<std::string_view> positional_args = xls::InitXls(kUsage, argc, argv);
  return xls::ExitStatus(RealMain(absl::GetFlag(FLAGS_input_ir_path),
                                  absl::GetFlag(FLAGS_input_patch_path),
