...
class GeneratedTester : public XlsccTestBase {
 ...
 private:
  ...
  absl::StatusOr<xls::Value> RunIntTest(
    ...
    std::string ac_include = std::string("-isystem") + ac_include_dir;
    std::string ac_datatypes_include = absl::StrCat(
        "-I",
        (std::filesystem::path(ac_include_dir) / "ac_datatypes" / "include")
            .string());
    ...
    argv.push_back(ac_datatypes_include);
