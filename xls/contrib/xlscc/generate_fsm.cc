...
absl::Status NewFSMGenerator::LayoutValuesToSaveForNewFSMStates(
  ...
  for (NewFSMState& state : layout.states) {
    ...
    for (const auto& [key, count] : remaining_input_count_by_value) {
      ...
      state.values_to_save.push_back(key.value);
    }

    // Sort for determinism
    std::sort(state.values_to_save.begin(), state.values_to_save.end(),
              [](const ContinuationValue* a, const ContinuationValue* b) {
                return a->output_node->id() < b->output_node->id();
              });
  }
