...
absl::Status FindContinuationNamesInThisContext(
    ...
    std::string& continuation_name, const TrackedBValue* bval,
      ...
      for (auto rev_it = context_stack_.rbegin();
        ...
        for (const TrackedBValue* bval : bvalues) {
          ...
          XLS_RETURN_IF_ERROR(FindContinuationNamesInThisContext(
              context, idx_from_top, name_found, bval, decl_by_bval, loc));
        }
      }
...
absl::Status Translator::AddContinuationsToNewSlice(
  ...
  for (const ContinuationInput& continuation_in : new_slice.continuations_in) {
    ...
    if (continuation_in.continuation_out->literal.has_value() &&
        !(after_op.op == OpType::kLoop &&
          after_op.loop_op_type == LoopOpType::kBegin)) {
...
absl::Status Translator::AddFeedbacksForSlice(GeneratedFunctionSlice& slice,
  ...
  if (!(slice.after_op->op == OpType::kLoop &&
        slice.after_op->loop_op_type == LoopOpType::kEndJump)) {
    return absl::OkStatus();
  }
...
absl::Status RemoveDuplicateInputs(GeneratedFunction& func, bool& changed,
  ...
  struct InputKey {
    ...
    bool operator<(const InputKey& other) const {
      ...
      return continuation_out < other.continuation_out;
    }
  };

  for (GeneratedFunctionSlice& slice : func.slices) {
    std::map<InputKey, std::vector<std::list<ContinuationInput>::iterator>>
        inputs_by_key;
...
absl::Status SubstiteLiterals(GeneratedFunction& func, bool& changed,
                              const xls::SourceInfo& loc) {
  for (GeneratedFunctionSlice& slice : func.slices) {
...
absl::Status Translator::OptimizeContinuations(GeneratedFunction& func,
  ...
  do {
    ...
    XLS_RETURN_IF_ERROR(SubstiteLiterals(func, changed, loc));
  } while (changed);
