...
absl::Status RemovePassThroughs(GeneratedFunction& func, bool& changed,
  ...
  for (GeneratedFunctionSlice& slice : func.slices) {
    ...
    for (ContinuationValue& continuation_out : slice.continuations_out) {
      ...
      if (output_source.size() != 1) {
        continue;
      }
      const xls::NodeSource& single_output_source =
          output_source.elements().front();
      if (!single_output_source.node()->Is<xls::Param>()) {
        continue;
      }

      xls::Param* pass_in_param = single_output_source.node()->As<xls::Param>();

      xls::LeafTypeTreeView<xls::NodeSource> input_source =
          visitor.GetValue(pass_in_param);
