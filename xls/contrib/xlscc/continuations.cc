  ...
  }
  {
    ...
    for (GeneratedFunctionSlice& slice : func.slices) {
      for (const ContinuationInput& continuation_in : slice.continuations_in) {
        if (continuation_in.choose_in_states.empty()) {
          continue;
        }
        absl::btree_set<StateId>& state_ids_for_node =
            state_ids_by_param[continuation_in.input_node];
        for (const StateId& state_id : continuation_in.choose_in_states) {
          auto [_, inserted] = state_ids_for_node.insert(state_id);
          if (!inserted) {
...
absl::Status RemovePassThroughs(GeneratedFunction& func, bool& changed,
  ...
  for (GeneratedFunctionSlice& slice : func.slices) {
    for (const ContinuationValue& continuation_out : slice.continuations_out) {
      ...
      auto pass_through_to_inputs_found =
          continuation_inputs_by_output_node.find(continuation_out.output_node);
      if (pass_through_to_inputs_found ==
          continuation_inputs_by_output_node.end()) {
        continue;
      }
      // Copy this so that we can mutate the maps
      std::vector<ContinuationInput*> pass_through_to_inputs =
          pass_through_to_inputs_found->second;
      ...
      if (absl::c_any_of(pass_through_to_inputs,
                         [&](ContinuationInput* pass_through_to_input) {
                           GeneratedFunctionSlice* downstream_slice =
                               slice_by_continuation_input.at(
                                   pass_through_to_input);

                           return downstream_slice == &slice;
                         })) {
        continue;
      }
      ...
      for (auto& [pass_through_to_input_ptr, upstream_inputs] :
        ...
        CHECK_EQ(std::erase(continuation_inputs_by_input_node.at(
                                pass_through_to_input_ptr->input_node),
                            pass_through_to_input_ptr),
                 1);
        ...
        auto it_erase =
            absl::c_find_if(downstream_slice->continuations_in,
                            [&](const ContinuationInput& in) {
                              return &in == pass_through_to_input_ptr;
                            });
        CHECK(it_erase != downstream_slice->continuations_in.end());
        auto insert_it = downstream_slice->continuations_in.erase(it_erase);
...
absl::Status RemoveDuplicateInputs(GeneratedFunction& func, bool& changed,
  ...
  for (GeneratedFunctionSlice& slice : func.slices) {
    ...
    for (auto& [key, input_set] : inputs_by_key) {
      CHECK(!input_set.input_iterators.empty());
...
absl::Status RemovePassthroughFeedbacks(GeneratedFunction& func, bool& changed,
  ...
  for (GeneratedFunctionSlice& slice : func.slices) {
    ...
    for (const xls::Param* param : params_per_slice.at(&slice)) {
      ...
      // Insert states from other inputs on the param into the upstream
      // (non-feedback) input.
      for (const ContinuationInput* input : param_inputs.all_inputs) {
        if (input == param_inputs.upstream_input) {
          continue;
        ...
        for (const StateId& state_id : input->choose_in_states) {
          param_inputs.upstream_input->choose_in_states.insert(state_id);
        }
