...
class TranslatorTest(absltest.TestCase):
  ...
  def test_combo_mux(self):
    ...
    args = dict(a=v1(0), in_z=v32(10), in_vz=v1(1), out1_vz=v1(1), out2_vz=v1(0))
    result = ir_interpreter.run_function_kwargs(f, args)
    self.assertEqual(str(result), "(0, 0, 0, 1, 10)")

    args = dict(a=v1(0), in_z=v32(10), in_vz=v1(1), out1_vz=v1(1), out2_vz=v1(1))
    result = ir_interpreter.run_function_kwargs(f, args)
    self.assertEqual(str(result), "(1, 0, 0, 1, 10)")

    args = dict(a=v1(1), in_z=v32(22), in_vz=v1(1), out1_vz=v1(1), out2_vz=v1(0))
    result = ir_interpreter.run_function_kwargs(f, args)
    self.assertEqual(str(result), "(1, 1, 22, 0, 0)")

    args = dict(a=v1(1), in_z=v32(22), in_vz=v1(0), out1_vz=v1(1), out2_vz=v1(0))
    result = ir_interpreter.run_function_kwargs(f, args)
    self.assertEqual(str(result), "(1, 0, 22, 0, 0)")

  def parse_and_get_function(self, source, hls_types_by_name=None, channels_in=None, channels_out=None):
    hls_types_by_name = hls_types_by_name if hls_types_by_name else {}
    translator = xlscc_translator.Translator("mypackage", hls_types_by_name, channels_in, channels_out)
