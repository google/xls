...
class ChannelType(Type):
  ...
  def get_xls_type(self, p):
    ...
    if self.is_input:
      return p.get_tuple_type([p.get_bits_type(1), self.channel_type.get_xls_type(p)])
    else:
      return p.get_bits_type(1)
    ...
    for named_field in self.struct.fields:
      ...
      if field.HasField("as_int"):
        ...
        elem_type = IntType(field.as_array.int_element_type.width, 
                            field.as_array.int_element_type.signed,
                            False)
...
class Function(object):
    ...
    if param_list is not None:
      for name, child in param_list.children():
        ...
        if isinstance(t, ChannelType):
          ...
          # TODO: Support passing to helper functions
          assert (name in translator.channels_in) or (name in translator.channels_out)
  ...
  def get_ref_param_names(self):
    ...
    def ref_name(name):
      ...
      return param_type.is_ref and (not param_type.is_const) and (not isinstance(param_type, ChannelType))
...
class Translator(object):
  ...
  def __init__(self, package_name, hls_types_by_name=None, channels_in=None, channels_out=None):
    self.functions_ = {}
    self.global_decls_ = {}
  ...
  def parse_type(self, ast_in):
    ...
    if isinstance(ast, c_ast.TypeDecl):
      ident = ast.type
      assert isinstance(ident, c_ast.IdentifierType) or isinstance(ident, c_ast.TemplateInst)
      ...
      for p_name, ptype in func.params.items():
        if not isinstance(ptype, ChannelType):
          ...
          self.cvars[p_name + "_vz"] = CVar(
              self.fb.add_param(p_name + "_vz", p.get_bits_type(1), func.loc), bit_type)
          self.channel_params.append((p_name + "_vz", 1))
          self.cvars[p_name + "_lz"] = CVar(self.gen_default_init(bit_type, func.loc), bit_type)
          self.lvalues[p_name + "_lz"] = True
          if ptype.is_input:
            self.cvars[p_name] = CVar(
                self.fb.add_param(p_name + "_z", ptype.channel_type.get_xls_type(p), func.loc), ptype)
            self.channel_params.append((p_name + "_z", ptype.channel_type.bit_width))
          else:
            self.cvars[p_name] = CVar(self.gen_default_init(ptype.channel_type, func.loc), ptype.channel_type)
      ...
      for pname, ptype in func.params.items():
        if isinstance(ptype, ChannelType):
          ...
          if not ptype.is_input:
            ...
            self.channel_returns.append((pname+"_z", ptype.channel_type.bit_width))
