...
FREE_OPS = ('kArray kArrayConcat kBitSlice kConcat kIdentity '
...
def _bitwidth_sweep(stride_degree: int = 0):
  """
  Yields a standard range of bitwidth. Increasing stride_degree
  increases the stride of the range (maximum stride_degree = 3).
  """
...
def _set_addressable_element_count_factor(expr: delay_model_pb2.DelayExpression):
      expr.factor.source = delay_model_pb2.DelayFactor.ADDRESSABLE_ELEMENT_COUNT
...
def _synthesize_op_and_make_bare_data_point(
  ...
  """Characterize an operation via synthesis server, setting the area
  and op of the data point but not any other information about the node / operands.
  ...
  Returns:
    datapoint produced via the synthesis server with the area and op (but no other fields)
    set.
  """
...
def _build_data_point(
  op: str, kop: str, node_dimensions: List[int], operand_dimensions: List[List[int]],
  stub: synthesis_service_pb2_grpc.SynthesisServiceStub,
  attributes: Sequence[Tuple[str, str]] = (),
  literal_operand: Optional[int] = None) -> delay_model_pb2.DataPoint:
  """Characterize an operation via synthesis server, setting the area
  ...
  result = _synthesize_op_and_make_bare_data_point(op, kop, op_type, operand_types, stub, attributes, literal_operand)
  return result
...
def _build_data_point_bit_types(
  ...
  Returns:
    Complete datapoint for the op, including bitwidths of tehe node and operands.
  """
...
def _run_nary_op(
  ...
  for bit_count in _bitwidth_sweep(0):
    results.append(_build_data_point_bit_types(op, kop,  bit_count, [bit_count]*num_inputs, stub))
    logging.info('# nary_op: ' + op + ', ' + str(bit_count) + ' bits, ' 
...
def _run_single_bit_result_op_and_add (
  ...
  for input_bits in _bitwidth_sweep(0):
    logging.info('# reduction_op: ' + op + ', ' + str(input_bits) + ' bits')
    model.data_points.append(_build_data_point_bit_types(op, kop, 1, [input_bits] * num_inputs, stub))
  ...
  for num_cases in _operand_count_sweep():
    for bit_count in _bitwidth_sweep(0):
      ...
      if math.pow(2, select_bits) == num_cases:
        model.data_points.append(_build_data_point_bit_types(op, kop, bit_count, [select_bits] + ([bit_count] * num_cases),
          stub))
      else:
        model.data_points.append(_build_data_point_bit_types(op, kop, bit_count, [select_bits] + ([bit_count] * (num_cases+1)),
          stub))
        logging.info('# select_op: ' + op + ', ' + str(bit_count) + ' bits, ' + str(num_cases) + ' cases' \
...
def _run_one_hot_select_op_and_add (
  ...
  for num_cases in _operand_count_sweep():
    for bit_count in _bitwidth_sweep(0):
      ...
      model.data_points.append(_build_data_point_bit_types(op, kop, bit_count, [select_bits] + ([bit_count] * num_cases),
        stub))
      logging.info('# one_hot_select_op: ' + op + ', ' + str(bit_count) + ' bits, ' + str(num_cases) + ' cases' \
          + ' --> ' + str(model.data_points[-1].delay))
...
def _run_encode_op_and_add (
  ...
  for input_bits in _bitwidth_sweep(0):
    ...
    model.data_points.append(_build_data_point_bit_types(op, kop, node_bits, [input_bits], stub))
    logging.info('# encode_op: ' + op + ', ' + str(input_bits) + ' input bits ' \
        + ' --> ' + str(model.data_points[-1].delay))
...
def _run_decode_op_and_add (
  ...
  for node_bits in _bitwidth_sweep(0):
    ...
    logging.info('# decode_op: ' + op + ', ' + str(node_bits) + ' bits ' \
        + ' --> ' + str(model.data_points[-1].delay))
  ...
  for input_bits in _bitwidth_sweep(2):
    for start_bits in range(3, bits.min_bit_count_unsigned(input_bits-1)+1):
      for node_bits in range(1, input_bits, BITWIDTH_STRIDE_DEGREES[2]):
        model.data_points.append(_build_data_point_bit_types(op, kop, node_bits, [input_bits, start_bits], stub,
          attributes=[('width', str(node_bits))]))
        logging.info('# idx ' + str(idx) + ': dynamic_bit_slice_op: ' + op + ', ' + str(start_bits) + ' start bits, ' + str(input_bits) 
...
def _run_one_hot_op_and_add(
  ...
  for bit_count in _bitwidth_sweep(0):
    ...
    model.data_points.append(_build_data_point_bit_types(op, kop,  bit_count+1, [bit_count], stub,
      attributes=[('lsb_prio', 'true')]))
    logging.info('# one_hot: ' + op + ', ' + str(bit_count) + ' input bits' \
        + ' --> ' + str(model.data_points[-1].delay))
  ...
  for mplier_count in _bitwidth_sweep(2):
    for mcand_count in _bitwidth_sweep(2):
      for node_count in _bitwidth_sweep(2):
        model.data_points.append(_build_data_point_bit_types(op, kop, node_count, [mplier_count, mcand_count], stub))
        logging.info('# mul: ' + op + ', ' + str(mplier_count) + ' * ' + str(mcand_count) + 
            ', result bits ' + str(node_count) + ' --> ' +str(model.data_points[-1].delay))

  # Validate model
  delay_model.DelayModel(model)

def _yield_array_dimension_sizes_helper(num_dimensions: int, dimension_sizes: List[int]):
  """
    Yields a sweep of array dimension size lists for 'num_dimension' number of dimensions'.
    'dimension_sizes' is used to recursively build up the size list.
  """
  if num_dimensions <= 0:
    ...
    for dim_size in _array_elements_sweep():
      ...
      yield from _yield_array_dimension_sizes_helper(num_dimensions - 1, dimension_sizes)

def _yield_array_dimension_sizes(num_dimensions: int):
  """Yields a sweep of array dimension size lists for 'num_dimension' number of dimensions'."""
  dimension_sizes = [-1]*num_dimensions
  yield from _yield_array_dimension_sizes_helper(num_dimensions, dimension_sizes)


def _get_array_num_elements(array_dims: List[int], index_depth: int = None):
  """Returns the number of elements in a (nested) array with dimensions 'array_dims'
  if the array is indexed 'index_depth' times. If 'index_depth' is not specified,
  the maximum number of possible indices is assumed."""
...
def _run_array_index_op_and_add(
  ...
  """
  Area is a function of #elements*weight + elements*bitwidth*weight.  This seems to
  hold across a range of element counts, bitwidth, and number of dimensions i.e. The
  weight isn't an artifact of where we sampled data - It is actually ~constant rather
  than being something like the ratio of #elements to #bitwidths or similar.
  """
  ...
  for num_dims in range(1,3):
    for array_dimension_sizes in _yield_array_dimension_sizes(num_dims):
      ...
      for element_bit_count in _bitwidth_sweep(3):
        array_and_element_dimensions = [element_bit_count] + array_dimension_sizes
        ...
        # Record data point
        result = _build_data_point(op, kop, [element_bit_count], operand_dimensions, stub)
        result.operation.bit_count = element_bit_count
        operand = result.operation.operands.add()
        operand.bit_count = math.prod(array_and_element_dimensions)
        model.data_points.append(result)

        logging.info(str(kop) + ': ' + ','.join(str(item) for item in operand_dimensions) + \
            ' --> ' + str(result.delay))
  ...
  for num_dims in range(1,3):
    for array_dimension_sizes in _yield_array_dimension_sizes(num_dims):
      ...
      for element_bit_count in _bitwidth_sweep(3):
        ...
        # Record data point
        result = _build_data_point(op, kop, array_and_element_dimensions, operand_dimensions, stub)
        array_operand = result.operation.operands.add()
        array_operand.bit_count = math.prod(array_and_element_dimensions)
