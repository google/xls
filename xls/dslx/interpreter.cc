...
absl::Status Interpreter::LogConcolicTests() {
  // Run the entry function with the generated inputs from the concolic engine
  // to compute the return value used later for the test generation.
  //
  // Also, the new set of inputs might execute parts of the program that weren't
  // explored before; hence, we keep the concolic engine running in the
  // background.
  if (test_generator_->GetInputValues().empty()) {
    return absl::OkStatus();
  }
  std::cerr << "Generated test cases:\n";
  int64_t test_num = 0;
  int64_t test_size = test_generator_->GetInputValues().size();
  while (test_num < test_size) {
    XLS_ASSIGN_OR_RETURN(
        InterpValue result,
        RunFunction(entry_module_->name(),
                    test_generator_->GetInputValues().at(test_num)));
    XLS_RETURN_IF_ERROR(test_generator_->GenerateTest(result, test_num));
    // If new inputs were generated by the concolic engine, test size is
    // expanded.
    test_size = test_generator_->GetInputValues().size();
    test_num++;
  }
  return absl::OkStatus();
}
