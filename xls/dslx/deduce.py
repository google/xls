...
def _concretize_struct_annotation(
    type_annotation: ast.TypeAnnotation,
    struct: ast.Struct,
    base_type: ConcreteType) -> ConcreteType:
  """Concretizes a parametric struct annotation using the provided bindings.
...
def _get_imported_module_via_node_to_type(
  ...
  return node_to_type.get_imported(mod_ref)
...
def _concretize_TypeAnnotation(
  ...
  if self.is_typeref():
    ...
    maybe_struct = ast_helpers.evaluate_to_struct_or_enum_or_annotation(
        ...
        ctx.node_to_type)
    if (isinstance(maybe_struct, ast.Struct)
        and maybe_struct.is_parametric() and self.parametrics):
...
def _validate_struct_members_subset(
    members: ast.StructInstanceMembers,
...
def _deduce_StructInstance(
  ...
  seen_names, arg_types, member_types = _validate_struct_members_subset(
                                            ...
                                            struct_type, self.struct_text, ctx)
...
def _deduce_SplatStructInstance(
  ...
  (seen_names, seen_arg_types,
   seen_member_types) = _validate_struct_members_subset(
      self.members, struct_type, self.struct_text, ctx)
