...
def _typecheck_struct_members_subset(
    members: ast.StructInstanceMembers, struct_type: ConcreteType, struct_text: str,
...
def _deduce_StructInstance(self: ast.StructInstance,
  ...
  seen_names = _typecheck_struct_members_subset(self.unordered_members, struct_type, self.struct_text, node_to_type)
...
def _deduce_SplatStructInstance(
    self: ast.SplatStructInstance,
    node_to_type: NodeToType) -> ConcreteType:  # pytype: disable=wrong-arg-types
  """Deduces the type of the struct instantiation expression and its members."""
  struct_type = deduce(self.struct, node_to_type)
  splatted_type = deduce(self.splatted, node_to_type)
  if splatted_type != struct_type:
    raise XlsTypeError(self.splatted.span, struct_type, splatted_type, 'Splatted expression must have the same type as the struct being instantiated.')
  _typecheck_struct_members_subset(self.members, struct_type, self.struct_text, node_to_type)
  return struct_type
