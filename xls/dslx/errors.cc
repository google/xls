...
static constexpr std::string_view label_payload_name = "label";
...
absl::Status FailureErrorStatusForAssertion(const Span& span,
  ...
  error.SetPayload(label_payload_name, absl::Cord(label));
  return error;
}

std::optional<std::string> GetAssertionLabelFromError(
    const absl::Status& status) {
  auto label_opt = status.GetPayload(label_payload_name);
  if (label_opt.has_value()) {
    return std::string(label_opt.value());
  } else {
    return std::nullopt;
  }
}
