...
pub fn difference_or_zero<N: u32>(x: uN[N], y: uN[N]) -> uN[N] {
    if x < y { uN[N]:0 } else { x - y }
}

#[test]
fn difference_or_zero_test() {
    assert_eq(difference_or_zero(u8:0, u8:0), u8:0);
    assert_eq(difference_or_zero(u8:0, u8:1), u8:0);
    assert_eq(difference_or_zero(u8:0, u8:255), u8:0);

    assert_eq(difference_or_zero(u8:128, u8:0), u8:128);
    assert_eq(difference_or_zero(u8:128, u8:1), u8:127);
    assert_eq(difference_or_zero(u8:128, u8:128), u8:0);
    assert_eq(difference_or_zero(u8:128, u8:129), u8:0);
    assert_eq(difference_or_zero(u8:128, u8:255), u8:0);

    // Edge cases: max and min values
    assert_eq(difference_or_zero(u8:255, u8:255), u8:0);
    assert_eq(difference_or_zero(u8:255, u8:254), u8:1);
    assert_eq(difference_or_zero(u8:255, u8:0), u8:255);
    assert_eq(difference_or_zero(u8:1, u8:255), u8:0);
    assert_eq(difference_or_zero(u8:1, u8:2), u8:0);
    assert_eq(difference_or_zero(u8:1, u8:1), u8:0);
    assert_eq(difference_or_zero(u8:2, u8:1), u8:1);
    assert_eq(difference_or_zero(u8:2, u8:2), u8:0);
    assert_eq(difference_or_zero(u8:2, u8:3), u8:0);

    // 1-bit values
    assert_eq(difference_or_zero(u1:0, u1:0), u1:0);
    assert_eq(difference_or_zero(u1:1, u1:0), u1:1);
    assert_eq(difference_or_zero(u1:1, u1:1), u1:0);
    assert_eq(difference_or_zero(u1:0, u1:1), u1:0);

    // 4-bit values
    assert_eq(difference_or_zero(u4:0, u4:15), u4:0);
    assert_eq(difference_or_zero(u4:15, u4:0), u4:15);
    assert_eq(difference_or_zero(u4:8, u4:8), u4:0);
    assert_eq(difference_or_zero(u4:7, u4:8), u4:0);
    assert_eq(difference_or_zero(u4:8, u4:7), u4:1);

    // Large difference
    assert_eq(difference_or_zero(u16:0, u16:65535), u16:0);
    assert_eq(difference_or_zero(u16:65535, u16:0), u16:65535);
    assert_eq(difference_or_zero(u16:65535, u16:65535), u16:0);
    assert_eq(difference_or_zero(u16:1, u16:65535), u16:0);
    assert_eq(difference_or_zero(u16:65535, u16:1), u16:65534);
}

// Returns the value of x-1 with saturation at 0.
pub fn bounded_minus_1<N: u32>(x: uN[N]) -> uN[N] { difference_or_zero(x, uN[N]:1) }
