...
Each test function bundles the five rounding-mode checks for one
...
class RoundingMode(Enum):
    ...
    def __str__(self):
        # For codegen purposes (DSLX output)
        return self.name
...
def fraction_from_bits(bits: int, nfrac: int, *, signed: bool, width: int) -> Fraction:
    """Return the exact real value represented by *bits* interpreted as a
    fixed-point number with *nfrac* fractional bits.
    If *signed* is False, bits are treated as unsigned.
    Otherwise two's-complement is assumed.
    """
...
def round_fraction(value: Fraction, mode: RoundingMode) -> int:
    ...
    Returns the rounded integer (unbounded - overflow handled separately).
    """
...
def needs_overflow_signed(val: int, int_bits: int) -> bool:
    ...
    includes the sign bit for two's-complement values.  A value is
    representable iff it lies within the conventional two's-complement range
    ...
    *no* non-zero integer can be represented once the value has been rounded
    (because the subsequent left-shift by `nfrac` would shift the sign bit out
    of the word). Hence every value except 0 overflows.
    """
    if int_bits == 0:
        return val != 0
...
def generate_test_function(width: int, bits: int, nfrac: int) -> str:
    ...
    func_lines.append(f"    // unsigned, Real value is {fmt_real(real_unsigned)}")
    for mode in ROUNDING_MODES:
        ...
        func_lines.append(f"    // {mode_name}({fmt_real(real_unsigned)}) = {rounded}{ov_comment}")
        func_lines.append(f"    assert_eq((u1:{carry}, u{width}:{val_lit}), "
                          f"round::round(round::RoundingMode::{mode_name}, num_bits_rounded, "
                          f"round::Sign::NonNegative, unrounded));")
    func_lines.append("")

    # --- sign & magnitude, negative --------------------------------------
    real_sm = -real_unsigned
    func_lines.append(f"    // sign & magnitude, Real value is {fmt_real(real_sm)}")
    for mode in ROUNDING_MODES:
        mode_name = mode.name
        ...
        func_lines.append(f"    // {mode_name}({fmt_real(real_sm)}) = {rounded_display}{ov_comment}")
        func_lines.append(f"    assert_eq((u1:{carry}, u{width}:{val_lit}), "
                          f"round::round(round::RoundingMode::{mode_name}, num_bits_rounded, "
                          f"round::Sign::Negative, unrounded));")
    func_lines.append("")

    # --- signed two's-complement ----------------------------------------
    real_signed = fraction_from_bits(bits, nfrac, signed=True, width=width)
    func_lines.append(f"    // signed, Real value is {fmt_real(real_signed)}")
    for mode in ROUNDING_MODES:
        mode_name = mode.name
        ...
        val_lit = bits_literal(val_bits, nfrac, width)
        ov_comment = " -> overflow" if overflow else ""
        func_lines.append(f"    // {mode_name}({fmt_real(real_signed)}) = {rounded}{ov_comment}")
        func_lines.append(f"    assert_eq((u1:{carry}, s{width}:{val_lit}), "
                          f"round::round(round::RoundingMode::{mode_name}, num_bits_rounded, "
                          f"round::Sign::NonNegative, unrounded as s{width}));")
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Generate DSLX round() tests.")
    parser.add_argument(
        "--round-tests",
        type=Path,
        default=Path(__file__).with_name("round_tests.x"),
        help="Path to the round_tests.x file to update (default: sibling round_tests.x)",
    )
    parser.add_argument(
        "--dslx-fmt",
        type=Path,
        help="Optional path to dslx_fmt; if provided, formatted output replaces round_tests.x",
    )
...
def main(argv: list[str] | None = None) -> None:
    ...
    if marker_index == -1:
        raise RuntimeError("Generated section header not found in round_tests.x")
    ...
    if args.dslx_fmt is not None:
        ...
        if result.stdout:
            ...
            raise RuntimeError("dslx_fmt produced no output; refusing to overwrite round_tests.x")
