...
TEST(PrintPositionalErrorTest, MultiSpanErrorTest) {
  const std::string filename = "multi_span_error_file.x";
  const std::string content = R"(line 1
line 2
line 3
line 4
line 5
...
line 7)";
  ...
  FileTable file_table;
  Fileno fileno = file_table.GetOrCreate(filename);
  const Span error_span1(Pos(fileno, 0, 0), Pos(fileno, 0, 4));
  const Span error_span2(Pos(fileno, 6, 0), Pos(fileno, 6, 4));

  std::stringstream ss;
  XLS_ASSERT_OK(PrintPositionalError(
      {error_span1, error_span2}, "my error message", ss,
      /*color=*/PositionalErrorColor::kNoColor, file_table, vfs,
      /*error_context_line_count=*/1));
  EXPECT_EQ(ss.str(),
            "multi_span_error_file.x:1:1-1:5\n"
            "0001: line 1\n"
            "~~~~~~^--^ my error message\n"
            "multi_span_error_file.x:7:1-7:5\n"
            "0007: line 7\n"
            "~~~~~~^--^\n");
}
