...
absl::Status ConstexprEvaluator::HandleNumber(const Number* expr) {
  ...
  XLS_RET_CHECK(ced.non_constexpr.empty());
  ...
  if (expr->type_annotation() != nullptr) {
    ...
    if (bt->size().IsParametric()) {
      XLS_ASSIGN_OR_RETURN(temp_type,
                           InstantiateParametricNumberType(ced.env, bt));
...
absl::Status ConstexprEvaluator::InterpretExpr(const Expr* expr) {
  ...
  XLS_ASSIGN_OR_RETURN(std::unique_ptr<BytecodeFunction> bf,
                       BytecodeEmitter::EmitExpression(
                           import_data_, type_info_, expr, ced.env, bindings_));
...
absl::StatusOr<ConstexprEnvData> MakeConstexprEnv(
  ...
  // parametric results, e.g. in parametric instantiation, without needing to put
  // everything we discover immediately into the type info.
