...
absl::StatusOr<ParseTypecheckResult> ParseTypecheck(std::string text) {
  ...
  XLS_ASSIGN_OR_RETURN(
      ...
      ParseAndTypecheck(text, /*path=*/"test.x", /*module_name=*/"test",
                        import_data.get()));
  return ParseTypecheckResult{.import_data = std::move(import_data),
                              .tm = std::move(tm)};
}
...
fn select_mask(xs: u6[2], selector: u6) -> bool[2] {
  ...
  XLS_ASSERT_OK_AND_ASSIGN(
      ...
      ReplaceInvocationsInModule(pt.tm, select_mask, rule, *pt.import_data,
                                 "test.rw"));
  Function* select_mask_new = new_tm.module->GetFunction("select_mask").value();
...
fn id2<N: u32>(x: uN[N]) -> uN[N] { x }
  ...
  auto status_or = ReplaceInvocationsInModule(pt.tm, caller, rule,
                                              *pt.import_data, "test.rw");
...
fn caller(x: u32) -> u32 { f<base::E::B>(x) }
  ...
  XLS_ASSERT_OK_AND_ASSIGN(
      ...
      ParseAndTypecheck(kTest, /*path=*/test_path,
                        /*module_name=*/"test", import_data.get()));
  Module* m = tm.module;
...
TEST(ReplaceInvocationsTest,
     ParametricEnumTypeAliasCrossModuleExplicitReplacement) {
  const std::string kBase = R"(// base.x
pub enum E : u2 {
  A = 0,
  B = 1,
