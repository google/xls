...
ContextType = Union['NodeToType', 'Bindings']
GetImportedCallback = Callable[[ast.ModRef, ContextType],
                               Tuple[ast.Module, ContextType]]

def evaluate_to_struct_or_enum_or_annotation(
      node: Union[ast.TypeDef, ast.ModRef, ast.Struct],
      get_imported_module: GetImportedCallback,
    ...
    Args:
      ...
      get_imported_module: Callback that returns the referenced module given
        a ModRef using the provided evaluation_context.
      evaluation_context: Evaluation information being kept by the caller
        (either NodeToType in type deduction or Bindings in interpreter
        evaluation).
    """
    ...
    td = evaluate_to_struct_or_enum_or_annotation(
        td, get_imported_module, evaluation_context)
