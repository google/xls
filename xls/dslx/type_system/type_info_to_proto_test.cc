...
class TypeInfoToProtoWithBothTypecheckVersionsTest
 ...
 public:
  absl::StatusOr<std::string> TypecheckModuleAndDumpTypeInfo(
      std::string_view program, TypeInfoProto* proto_out = nullptr,
    ...
    XLS_ASSIGN_OR_RETURN(TypecheckedModule tm,
                         ParseAndTypecheck(program, "fake.x", "fake",
  ...
  void ExpectText(std::string_view actual, bool version_specific = false) {
    ...
    ExpectEqualToGoldenFile(golden_file_path, actual);
  }
};

TEST_P(TypeInfoToProtoWithBothTypecheckVersionsTest, IdentityFunction) {
  std::string program = R"(fn id(x: u32) -> u32 { x })";
  XLS_ASSERT_OK_AND_ASSIGN(std::string type_info_text,
                           TypecheckModuleAndDumpTypeInfo(program));
  ExpectText(type_info_text);
}
...
fn id(x: u32) -> u32 { pid<u32:32>(x) }
  ...
  XLS_ASSERT_OK_AND_ASSIGN(std::string type_info_text,
                           TypecheckModuleAndDumpTypeInfo(program));
  ExpectText(type_info_text, true);
}
...
fn f() -> S { S { x: u32:42 } }
  ...
  TypeInfoProto tip;
  XLS_ASSERT_OK_AND_ASSIGN(std::string type_info_text,
                           TypecheckModuleAndDumpTypeInfo(program, &tip));
  ExpectText(type_info_text, true);
  EXPECT_THAT(
...
type MyFoo = my_imported_module::Foo;
  ...
  XLS_ASSERT_OK_AND_ASSIGN(
      std::string type_info_text,
      TypecheckModuleAndDumpTypeInfo(program, nullptr, &import_data));
  ExpectText(type_info_text, true);
}
