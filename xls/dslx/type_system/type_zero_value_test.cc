...
TEST(TypeZeroValueTest, UsesExplicitZeroDiscriminantInsteadOfDenseStorageTag) {
  ...
  SumType sum_type = make_type(SumType::ZeroSelectionState::kUnspecified);
  ...
  SumType absent = make_type(SumType::ZeroSelectionState::kNoZeroVariant);
  tm.type_info->NoteConstExpr(idle_discriminant, InterpValue::MakeUBits(2, 0));
...
TEST(TypeZeroValueTest, RejectsUnspecifiedExplicitGenericZeroSelection) {
  ImportData import_data = CreateImportDataForTest();
  XLS_ASSERT_OK_AND_ASSIGN(
      TypecheckedModule tm,
      ParseAndTypecheck(R"(
...
enum E<N: u32>: u32 { A(u8) = N }
  ...
  SumType unspecified(*def, std::move(variants));
  ...
  EXPECT_THAT(
      MakeZeroValue(unspecified, import_data, tm.module->span()),
