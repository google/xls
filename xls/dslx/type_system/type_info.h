...
class TypeInfo {
 public:
  ...
  void NoteUnrolledLoop(const ForLoopBase* loop, const ParametricEnv& env,
                        Expr* unrolled_expr);
  std::optional<Expr*> GetUnrolledLoop(const ForLoopBase* loop,
                                       const ParametricEnv& env) const;

  // Returns all concrete unrolled loop expressions for a given loop across all
  // environments.
  std::vector<Expr*> GetAllUnrolledLoops(const ForLoopBase* loop) const;
 ...
 private:
  ...
  absl::flat_hash_map<const ForLoopBase*,
