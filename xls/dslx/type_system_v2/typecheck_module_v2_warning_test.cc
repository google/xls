...
TEST(TypecheckV2WarningTest,
     SemanticSumConstructorPreservesDisabledSemanticAnalysis) {
...
fn main() -> E {
  ...
  EXPECT_NE(dynamic_cast<Invocation*>(
...
TEST(TypecheckV2WarningTest,
     SemanticSumConstructorPreservesDistinctAndEarlierWarnings) {
  constexpr std::string_view kImported = R"(#![feature(type_inference_v2)]
...
fn main() -> E {
  ...
  auto main = module_info->module().GetFunction("main");
  ASSERT_TRUE(main.has_value());
  ASSERT_NE(dynamic_cast<Invocation*>(
                ToAstNode((*main)->body()->statements().back()->wrapped())),
            nullptr);
...
TEST(TypecheckV2WarningTest,
     SemanticSumWarningsDistinguishConstructorsFromCalls) {
  XLS_ASSERT_OK_AND_ASSIGN(TypecheckResult result, TypecheckV2(R"(
enum E { V(u32) }
fn payload() -> u32 { trace!(u32:1) }
fn make() -> E { E::V(u32:1) }
fn main() {
  E::V(u32:0);
  E::V(payload());
  ...
  const auto& warnings = result.tm.warnings.warnings();
  ASSERT_EQ(warnings.size(), 1);
  EXPECT_EQ(warnings[0].kind, WarningKind::kUselessExpressionStatement);
  EXPECT_THAT(warnings[0].message, HasSubstr("E::V(u32:0)"));
}
