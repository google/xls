...
class EvaluatorImpl : public Evaluator {
 public:
  ...
  absl::StatusOr<bool> EvaluateBoolOrExpr(
    ...
    std::optional<InterpValue> value = FastEvaluate(parametric_context, expr);
    ...
    if (!value.has_value()) {
      ...
      if (expr->kind() == AstNodeKind::kNumber) {
        literal_cache_.emplace(expr->ToString(), *value);
      }
    }
 ...
 private:
  ...
  std::optional<InterpValue> FastEvaluate(
    ...
    if (expr->kind() == AstNodeKind::kNumber) {
      ...
      const auto it = literal_cache_.find(expr->ToString());
  ...
  absl::flat_hash_map<std::string, InterpValue> literal_cache_;
};
