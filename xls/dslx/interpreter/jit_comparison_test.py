...
class JitComparisonTest(absltest.TestCase):
  ...
  def test_array_equivalence_fail(self):
    ...
    ir_elements_extra = (
        ir_elements + [ir_value.Value(ir_bits.UBits(value=5, bit_count=4))])
    ...
    ir_elements_wrong = (
        ir_elements[:2] +
        [ir_value.Value(ir_bits.UBits(value=5, bit_count=4))] + ir_elements[3:])
  ...
  def test_tuple_equivalence_fail(self):
    ...
    ir_members_wrong = (
        ir_members[:2] + [ir_value.Value(ir_bits.UBits(value=5, bit_count=4))] +
        ir_members[3:])
