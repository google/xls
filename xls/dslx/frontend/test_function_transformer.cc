...
class AstNodeVisitorWithDefaultChildHandler : public AstNodeVisitorWithDefault {
 public:
...
class SpawnFinder : public AstNodeVisitorWithDefaultChildHandler {
 public:
...
absl::StatusOr<Function*> CreateNextFunction(
  ...
  // variables with accesses to the proc fields (e.g. `self.field`).
...
class NameDefFinder : public AstNodeVisitorWithDefaultChildHandler {
 public:
...
class NameRefFinder : public AstNodeVisitorWithDefaultChildHandler {
 public:
...
absl::StatusOr<absl::flat_hash_set<const NameDef*>> find_promoted(
  ...
  // and not local variables. This will populate the hash set
  NameDefFinder def_finder;
  for (const auto& stmt : const_statements) {
    XLS_RETURN_IF_ERROR(stmt->Accept(&def_finder));
  }
  const auto& name_defs = def_finder.name_defs();
  NameRefFinder ref_finder;
  for (const auto& stmt : non_const_statements) {
    XLS_RETURN_IF_ERROR(stmt->Accept(&ref_finder));
  }
  absl::flat_hash_set<const NameDef*> promoted;
  for (const auto& name_ref : ref_finder.name_refs()) {
    if (!std::holds_alternative<const NameDef*>(name_ref->name_def())) {
      continue;
    }
    const NameDef* name_def = std::get<const NameDef*>(name_ref->name_def());
    if (name_defs.find(name_def) != name_defs.end()) {
      // Can't just compare the names, because of shadowing.
...
TestFunctionTransformer::PromoteVariablesToFields(
  ...
  // Identify promoted variables and add them to ProcDef
  absl::flat_hash_map<std::string, int> name_occurrences;
  for (const NameDef* def : defs_to_promote) {
    name_occurrences[def->identifier()]++;
  }
  ...
  absl::flat_hash_map<std::string, int> name_indices;
  ...
  // become flat proc fields.
  for (const NameDef* def : sorted_defs_to_promote) {
    std::string base_name = def->identifier();
    std::string unique_name = base_name;
    if (name_occurrences[base_name] > 1) {
      unique_name =
          absl::StrCat("__", base_name, "_", name_indices[base_name]++);
    }
