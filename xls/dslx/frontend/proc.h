...
class TestProc : public AstNode {
 public:
  ...
  TestProc(Module* owner, Proc* proc,
      ...
      : AstNode(owner), proc_(proc), expected_fail_(expected_fail) {}
  ~TestProc() override;
  ...
  std::optional<std::string> expected_fail() const { return expected_fail_; }

 private:
  Proc* proc_;
  std::optional<std::string> expected_fail_;
};
