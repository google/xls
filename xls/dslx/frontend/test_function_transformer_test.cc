...
fn main() {
  ...
  std::string new_body = impl_code.substr(new_pos, next_pos - new_pos);
  EXPECT_TRUE(absl::StrContains(new_body, "let (tx, rx) = chan<u32>(\"c\");"));
  EXPECT_TRUE(absl::StrContains(new_body, "let __x_0 = u32:1;"));
  EXPECT_TRUE(absl::StrContains(new_body, "let __x_1 = u32:2;"));

  // Runtime in 'next'
  std::string next_body = impl_code.substr(next_pos);
  // tx and shadowed x must be promoted.
  EXPECT_TRUE(
      absl::StrContains(next_body, "send(token(), self.tx, read(self.__x_0))"));
  EXPECT_TRUE(
...
fn main() {
  ...
  EXPECT_TRUE(absl::StrContains(new_body,
                                "let (tx_out, rx_out) = chan<u32>(\"out\");"));
