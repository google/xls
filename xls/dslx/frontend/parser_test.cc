...
TEST_F(ParserTest, IOObjectsNoFeatureFlagChannelStrictness) {
  constexpr std::string_view kProgram = R"(
proc Main {
    i: u32,
}
...
TEST_F(ParserTest, IOObjectsNoFeatureFlagFlowControl) {
  constexpr std::string_view kProgram = R"(
proc Main {
    i: u32,
}
...
#![feature(io_objects)]
  ...
  EXPECT_THAT(module->attributes(),
              testing::ElementsAre(ModuleAttribute::kIOObjects));
}
