 ...
 public:
  ...
  absl::Status DefaultHandler(const AstNode* node) override {
    ...
    if (node->kind() == AstNodeKind::kTypeAnnotation) {
      ...
      if (type_annotation->annotation_kind() ==
          TypeAnnotationKind::kTypeVariable) {
        const auto* tvta =
            absl::down_cast<const TypeVariableTypeAnnotation*>(node);
  ...
  absl::Status HandleLambda(const Lambda* node) override {
    ...
    CloneReplacer insert_self =
        [self_nd, seen, name_ref_replacements](
            ...
            const absl::flat_hash_map<const AstNode*, AstNode*>& __)
    ...
    XLS_ASSIGN_OR_RETURN(
        ...
        CloneAst(original_fn->return_type(), std::move(swap_name_refs)));
