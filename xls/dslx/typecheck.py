...
def check_top_node_in_module(
    f: Union[ast.Function, ast.Test, ast.Struct, ast.TypeDef],
  ...
  stack = [_TypecheckStackRecord(f.name.identifier, type(f))]  # type: List[_TypecheckStackRecord]
...
def check_module(module: Module,
  ...
  for member in ctx.module.top:
    if isinstance(member, ast.Import):
      ...
      assert isinstance(member,
                        (ast.Function, ast.Test, ast.Struct, ast.QuickCheck, ast.TypeDef)), member
  ctx.fn_stack.pop()
  ...
  typedef_map = {t.name.identifier: t for t in ctx.module.top if isinstance(t, ast.TypeDef)}
