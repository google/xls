...
// branch kind, and a `line_<N>_col_<M>` location.
...
// `emit_cover=false` disables auto-cover synthesis too, for if/else and match.
TEST_F(FrontendCoverpointInsertionTest, EmitCoverFalseDisablesAutoCover) {
  constexpr std::string_view program = R"(
fn f(x: u32) -> u32 {
...
y + z
  ...
  // Auto-cover is enabled; only `emit_cover=false` should suppress the covers.
  ConvertOptions options;
  options.emit_cover = false;
  options.emit_auto_cover.Insert(AutoCoverKind::kBranch);
  options.force_implicit_token_calling_convention = true;
  XLS_ASSERT_OK_AND_ASSIGN(xls::Function * f,
                           ConvertToFunction(program, "f", options));
...
y
  ...
  EXPECT_THAT(CoverLabels(f),
              UnorderedElementsAre("__itok__test_module__f_then_line_3_col_9",
                                   "__itok__test_module__f_else_line_3_col_9"));
  ...
  EXPECT_THAT(fired_le,
              UnorderedElementsAre("__itok__test_module__f_else_line_3_col_9"));
...
// `if ... else if ... else` emits covers only for leaf branches (then, else_if,
// else), labeling the else-if body `else_if`; the outer else gets no cover so
...
if x > u32:10 { u32:1 } else if x > u32:4 { u32:2 } else { u32:3 }
  ...
  // Outer if `then` sits at line 3 col 1; the else-if arm is the then-arm of
  // the inner conditional (line 3 col 30) and is labeled `else_if`; the final
  // else is labeled `else`.
  EXPECT_THAT(
      CoverLabels(f),
      UnorderedElementsAre("__itok__test_module__f_then_line_3_col_1",
                           "__itok__test_module__f_else_if_line_3_col_30",
                           "__itok__test_module__f_else_line_3_col_30"));

  // x > 10: outer then only.
  XLS_ASSERT_OK_AND_ASSIGN(
      auto fired_gt10,
  ...
  // 4 < x <= 10: else_if arm only (the outer else is suppressed, so it does
  // not double-count the else-if arm).
  XLS_ASSERT_OK_AND_ASSIGN(
      auto fired_else_if,
      FiredCovers(f, ImplicitArgs(f, true, {Value(UBits(6, 32))})));
  EXPECT_THAT(fired_else_if, UnorderedElementsAre(
                                 "__itok__test_module__f_else_if_line_3_col_30"));
  ...
  XLS_ASSERT_OK_AND_ASSIGN(
      ...
      FiredCovers(f, ImplicitArgs(f, true, {Value(UBits(4, 32))})));
  EXPECT_THAT(fired_both_else, UnorderedElementsAre(
                                   "__itok__test_module__f_else_line_3_col_30"));
}
...
match x { u32:1 => { u32:1 }, u32:2 => { u32:2 }, _ => { u32:3 } }
  ...
  EXPECT_THAT(
      CoverLabels(f),
      UnorderedElementsAre("__itok__test_module__f_match_arm0_line_3_col_1",
                           "__itok__test_module__f_match_arm1_line_3_col_1",
                           "__itok__test_module__f_match_arm2_line_3_col_1"));
  ...
  EXPECT_THAT(fired_1, UnorderedElementsAre(
                           "__itok__test_module__f_match_arm0_line_3_col_1"));
...
u32:1
  ...
  EXPECT_THAT(CoverLabels(f),
              UnorderedElementsAre("__itok__test_module__f_then_line_3_col_13",
                                   "__itok__test_module__f_else_line_3_col_13",
                                   "__itok__test_module__f_then_line_4_col_1",
                                   "__itok__test_module__f_else_line_4_col_1"));
...
fn b(y: u32) -> u32 { if y > u32:9 { u32:2 } else { u32:3 } }
  ...
  EXPECT_THAT(
      CoverLabels(fa),
      UnorderedElementsAre("__itok__test_module__a_then_line_2_col_23",
                           "__itok__test_module__a_else_line_2_col_23"));
  EXPECT_THAT(
      CoverLabels(fb),
      UnorderedElementsAre("__itok__test_module__b_then_line_3_col_23",
                           "__itok__test_module__b_else_line_3_col_23"));
}
...
if x != u32:0 { x } else { u32:0 }
  ...
  for (int i = 0; i < labels.size(); ++i) {
    EXPECT_THAT(labels[i], HasSubstr(absl::StrFormat("_line_%d_col_%d",
                                                     locations[i].first + 1,
...
if x != u32:0 { x } else { u32:0 }
  ...
  EXPECT_THAT(CoverLabels(f),
              UnorderedElementsAre("__itok__test_module__f_then_line_3_col_1",
                                   "__itok__test_module__f_else_line_3_col_1"));
  for (Node* node : f->nodes()) {
    if (node->Is<Cover>()) {
...
let a = match x { u32:1 => { u32:1 }, _ => { u32:0 } }; let b = match y { u32:2 => { u32:2 }, _ => { u32:0 } }; a + b
  ...
  EXPECT_THAT(labels, UnorderedElementsAre(
                          "__itok__test_module__f_match_arm0_line_3_col_9",
                          "__itok__test_module__f_match_arm1_line_3_col_9",
                          "__itok__test_module__f_match_arm0_line_3_col_65",
                          "__itok__test_module__f_match_arm1_line_3_col_65"));
  ...
  EXPECT_THAT(
      fired_both_first,
      UnorderedElementsAre("__itok__test_module__f_match_arm0_line_3_col_9",
                           "__itok__test_module__f_match_arm0_line_3_col_65"));
}
