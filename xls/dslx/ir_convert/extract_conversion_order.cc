...
absl::StatusOr<std::vector<Proc*>> GetTopLevelProcs(Module* module,
  ...
  auto collect_spawns_from_proc = [&](Proc* proc) -> absl::Status {
    if (proc->used_in_tests() && !include_tests) {
      return absl::OkStatus();
    }
  ...
  for (Proc* proc : module->GetProcs()) {
    XLS_RETURN_IF_ERROR(collect_spawns_from_proc(proc));
  }
  if (include_tests) {
    for (TestProc* test_proc : module->GetTestProcs()) {
      XLS_RET_CHECK(test_proc != nullptr);
      XLS_RETURN_IF_ERROR(collect_spawns_from_proc(test_proc->proc()));
    }
  }
  ...
  auto add_if_top_level_proc = [&](Proc* proc) {
...
absl::StatusOr<std::vector<ConversionRecord>> GetOrder(Module* module,
  ...
  auto handle_function = [&](Function* f) -> absl::Status {
    ...
    if (f->used_in_tests() && !include_tests) {
      return absl::OkStatus();
    }
