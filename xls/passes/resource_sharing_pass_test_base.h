...
#ifndef XLS_PASSES_RESOURCE_SHARING_PASS_TEST_BASE_H_
...
inline uint64_t NumberOfAdders(Function* f) {
  return NumberOfNodes(f, {Op::kAdd, Op::kSub});
}

inline uint64_t NumberOfSelects(Function* f) {
  return NumberOfNodes(f, {Op::kPrioritySel, Op::kOneHotSel, Op::kSel});
}

inline uint64_t NumberOfMultiplications(Function* f) {
  return NumberOfNodes(f, {Op::kSMul, Op::kUMul});
}

inline uint64_t NumberOfShifts(Function* f) {
  return NumberOfNodes(f, {Op::kShll, Op::kShra, Op::kShrl});
}

inline void InterpretAndCheck(Function* f, const std::vector<int32_t>& inputs,
...
TYPED_TEST_P(ResourceSharingPassTestBase, MergeSingleUnsignedMultiplication) {
...
TYPED_TEST_P(ResourceSharingPassTestBase, MergeSingleUnsignedMultiplication2) {
  // Create the function builder
  auto p = this->CreatePackage();
  FunctionBuilder fb(this->TestName(), p.get());
...
TYPED_TEST_P(ResourceSharingPassTestBase, MergeSingleSignedMultiplication) {
  // Create the function builder
  auto p = this->CreatePackage();
  FunctionBuilder fb(this->TestName(), p.get());
...
TYPED_TEST_P(ResourceSharingPassTestBase,
             MergeMultipleUnsignedMultiplications) {
  // Create the function builder
  auto package = this->CreatePackage();
  FunctionBuilder fb(this->TestName(), package.get());
...
TYPED_TEST_P(ResourceSharingPassTestBase, MergeMultipleSignedMultiplications) {
  // Create the function builder
  auto package = this->CreatePackage();
  FunctionBuilder fb(this->TestName(), package.get());
...
TYPED_TEST_P(ResourceSharingPassTestBase, MergeMutuallyExclusiveAnds) {
  // Define the IR code
  auto p = this->CreatePackage();
  FunctionBuilder fb(this->TestName(), p.get());
...
TYPED_TEST_P(ResourceSharingPassTestBase, NoFoldAndsDifferentOperandCount) {
  auto p = this->CreatePackage();
  FunctionBuilder fb(this->TestName(), p.get());
  Type* u32_type = p->GetBitsType(32);
  Type* u1_type = p->GetBitsType(1);
  ...
  XLS_ASSERT_OK_AND_ASSIGN(Function * f, fb.BuildWithReturnValue(select));
  EXPECT_THAT(this->Run(f), IsOkAndHolds(false));
}

TYPED_TEST_P(ResourceSharingPassTestBase, MergeAndsDifferentWidth) {
  auto p = this->CreatePackage();
  FunctionBuilder fb(this->TestName(), p.get());
...
TYPED_TEST_P(ResourceSharingPassTestBase, NotPossibleFolding0) {
...
TYPED_TEST_P(ResourceSharingPassTestBase, NotPossibleFolding1) {
  // Create the function builder
  auto package = this->CreatePackage();
  FunctionBuilder fb(this->TestName(), package.get());
...
TYPED_TEST_P(ResourceSharingPassTestBase, NotPossibleFolding2) {
  // Create the function builder
  auto package = this->CreatePackage();
  FunctionBuilder fb(this->TestName(), package.get());
...
TYPED_TEST_P(ResourceSharingPassTestBase, NotPossibleFolding3) {
  // Create the function builder
  auto package = this->CreatePackage();
  FunctionBuilder fb(this->TestName(), package.get());
...
TYPED_TEST_P(ResourceSharingPassTestBase, NotPossibleFolding4) {
  // Create the function builder
  auto package = this->CreatePackage();
  FunctionBuilder fb(this->TestName(), package.get());
...
TYPED_TEST_P(ResourceSharingPassTestBase, MergeAdds) {
  // Create the function builder
  auto p = this->CreatePackage();
  FunctionBuilder fb(this->TestName(), p.get());
...
TYPED_TEST_P(ResourceSharingPassTestBase, MergeAddsWithDifferentBitwidths) {
  // Create the function builder
  auto p = this->CreatePackage();
  FunctionBuilder fb(this->TestName(), p.get());
...
TYPED_TEST_P(ResourceSharingPassTestBase, MergeSubs) {
  // Create the function builder
  auto p = this->CreatePackage();
  FunctionBuilder fb(this->TestName(), p.get());
...
TYPED_TEST_P(ResourceSharingPassTestBase, MergeSubs2) {
  // Create the function builder
  auto p = this->CreatePackage();
  FunctionBuilder fb(this->TestName(), p.get());
...
TYPED_TEST_P(ResourceSharingPassTestBase, MergeSubsWithDifferentBitwidths) {
  // Create the function builder
  auto p = this->CreatePackage();
  FunctionBuilder fb(this->TestName(), p.get());
...
TYPED_TEST_P(ResourceSharingPassTestBase, MergeAddsAndSubs) {
  // Create the function builder
  auto p = this->CreatePackage();
  FunctionBuilder fb(this->TestName(), p.get());
...
TYPED_TEST_P(ResourceSharingPassTestBase, MergeShift) {
  // Create the function builder
  auto p = this->CreatePackage();
  FunctionBuilder fb(this->TestName(), p.get());
...
TYPED_TEST_P(ResourceSharingPassTestBase,
             MergeMultipliesAndAddsUsedByTwoSelects) {
  // Create the function builder
  auto p = this->CreatePackage();
  FunctionBuilder fb(this->TestName(), p.get());
...
TYPED_TEST_P(ResourceSharingPassTestBase, PreventCyclesInFoldingChains) {
...
TYPED_TEST_P(ResourceSharingPassTestBase, PreventCyclesInFoldingChainsNary) {
...
REGISTER_TYPED_TEST_SUITE_P(
    ...
    MergeMultipleUnsignedMultiplications, MergeMultipleSignedMultiplications,
    MergeMutuallyExclusiveAnds, NoFoldAndsDifferentOperandCount,
    MergeAndsDifferentWidth, NotPossibleFolding0, NotPossibleFolding1,
    NotPossibleFolding2, NotPossibleFolding3, NotPossibleFolding4,
    SkipMultiplicationsThatAreNotPostDominatedBySelectAndNotErased,
    MergeMultiplicationsThatAreNotPostDominatedBySelectButAreErased,
    MergeSingleUnsignedMultiplicationDifferentBitwidths,
...
#endif  // XLS_PASSES_RESOURCE_SHARING_PASS_TEST_BASE_H_
