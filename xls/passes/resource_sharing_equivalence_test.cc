...
TEST_F(ResourceSharingEquivalenceTest, AddSubEquivalenceMapping) {
  // 1. add32 -> sub32: same bitwidth, different ops
  {
    auto p = CreatePackage();
  ...
  // 2. add_neg -> sub32: operand 1 is already kNeg
  {
    auto p = CreatePackage();
  ...
  // 3. add16 -> sub32: different ops AND different bit widths
  {
    auto p = CreatePackage();
...
TEST_F(ResourceSharingEquivalenceTest, ShiftEquivalenceMapping) {
  // 1. Shll -> Shrl: Shift Left mapped to Shift Right Logical
  {
    auto p = CreatePackage();
  ...
  // 2. Shrl -> Shll: Shift Right Logical mapped to Shift Left
  {
    auto p = CreatePackage();
