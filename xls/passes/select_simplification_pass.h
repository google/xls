...
//     At higher optimization levels (`SplitsEnabled`), a binary `select` whose
//     two arms provably differ by a fully-known XOR term (e.g.
//     `select(p, [x, xor(x, c)])`) is strength-reduced into an XOR with an
//     AND-masked constant: `xor(x, and(sign_ext(p, W), c))`, with `p`
//     complemented when the XOR arm is the on-false case. The arms need not
//     literally form `xor(base, c)`: per-piece xor-with-constant differences
//     introduced by earlier passes (e.g. `concat_simp`) are also recognized.
//     This is the select pattern generated by the per-step of an unrolled
//     CRC/shift loop.
//
