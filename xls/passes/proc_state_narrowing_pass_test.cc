...
TEST_F(ProcStateNarrowingPassTest, BasicLoop) {
  ...
  auto tok = pb.Send(chan, pb.GetTokenParam(), state);
  ...
  XLS_ASSERT_OK_AND_ASSIGN(Proc * proc, pb.Build(tok));
...
TEST_F(ProcStateNarrowingPassTest, MultiPath) {
  ...
  ProcBuilder pb(TestName(), "tok", p.get());
  auto state = pb.StateElement("the_state", UBits(1, 32));
  auto send_tok = pb.Send(test_chan, pb.GetTokenParam(), state);
  ...
  auto tok = pb.TupleIndex(reset_val, 0);
