...
class ProcStateNarrowingPassTest : public IrTestBase {
 public:
  ...
  absl::StatusOr<bool> RunProcStateCleanup(Proc* p) {
    ...
    ProcStateEliminationPass psep;
    PassResults r;
    OptimizationContext ctx;
    return psep.Run(p->package(), {}, &r, ctx);
  }
};
...
void IrFuzzProcStateNarrowing(FuzzPackageWithArgs fuzz_package_with_args) {
  ProcStateNarrowingPass pass;
  OptimizationPassChangesOutputs(std::move(fuzz_package_with_args), pass);
}
FUZZ_TEST(IrFuzzTest, IrFuzzProcStateNarrowing)
