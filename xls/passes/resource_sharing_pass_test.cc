...
TEST_F(ResourceSharingPassTest, InfluencedBySourceAndOp) {
  ...
  for (const auto& bool_op : {and_node, nand_node}) {
    EXPECT_TRUE(InfluencedBySource(bool_op, i_node, reachability, *bdd, {}));
    EXPECT_FALSE(InfluencedBySource(
        bool_op, i_node, reachability, *bdd,
        {std::make_pair(TreeBitLocation{j_node, 0}, false)}));
    EXPECT_TRUE(InfluencedBySource(bool_op, j_node, reachability, *bdd, {}));
    EXPECT_FALSE(InfluencedBySource(
        bool_op, j_node, reachability, *bdd,
...
TEST_F(ResourceSharingPassTest, InfluencedBySourceOrOp) {
  ...
  for (const auto& bool_op : {or_node, nor_node}) {
    EXPECT_TRUE(InfluencedBySource(bool_op, i_node, reachability, *bdd, {}));
    EXPECT_FALSE(
        InfluencedBySource(bool_op, i_node, reachability, *bdd,
                           {std::make_pair(TreeBitLocation{j_node, 0}, true)}));
    EXPECT_TRUE(InfluencedBySource(bool_op, j_node, reachability, *bdd, {}));
    EXPECT_FALSE(
        InfluencedBySource(bool_op, j_node, reachability, *bdd,
