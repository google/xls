...
class TimingAnalysis {
 ...
 private:
  absl::flat_hash_map<NaryFoldingAction *, uint64_t> delay_increase;
  absl::flat_hash_map<NaryFoldingAction *, double> delay_spread;
};

// This function implements the strict weak ordering necessary for absl::sort
template <class T>
bool StrictWeakOrdering(T left, T right, int64_t left_id, int64_t right_id) {
  // Compare the two values
  if (left < right) {
    return true;
  }

  // Enforce the asymmetry property
  if (right < left) {
    return false;
  }

  // Rely on their ID
  // This function assumes the ID comparison is strict weak (this is the case
  // for IR IDs.
...
std::optional<std::unique_ptr<BinaryFoldingAction>> CanMapInto(
  ...
  if (node_to_map->op() != folding_destination->op()) {
    ...
    const std::vector<Op> kAddOrSub = {Op::kAdd, Op::kSub};
    const std::vector<Op> kRightShiftOrDynamicBitslice = {Op::kShrl,
                                                          Op::kDynamicBitSlice};
...
void SortFoldingActionsInDescendingOrderOfTheirAreaSavings(
  ...
  auto area_comparator = [&ta](std::unique_ptr<NaryFoldingAction> &f0,
    ...
    const double kDelayDeltaWeight = 1.01;
    ...
    double f1_delay_total =
        ...
        f1_delay_spread;
    return StrictWeakOrdering(f0_delay_total, f1_delay_total, f0_id, f1_id);
  };
...
TimingAnalysis::TimingAnalysis(
  ...
  for (const std::unique_ptr<NaryFoldingAction> &folding : folding_actions) {
    ...
    delay_spread[folding.get()] = delta_spread;
