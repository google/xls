...
int64_t GetDelayIncrease(
  ...
  if (next_folding_action) {
    min_delay = max_delay = *node_delay.GetInfo(next_folding_action->GetTo());
    min_delay = std::min(min_delay,
                         *node_delay.GetInfo(next_folding_action->GetFrom()));
    max_delay = std::max(max_delay,
                         *node_delay.GetInfo(next_folding_action->GetFrom()));
  } else if (!current_folding_actions.empty()) {
    min_delay = max_delay =
        *node_delay.GetInfo(current_folding_actions[0]->GetTo());
  }
  for (auto folding : current_folding_actions) {
...
double GetDelaySpread(BinaryFoldingAction* folding_action,
                      const CriticalPathDelayAnalysis& node_delay) {
  int64_t to_delay = *node_delay.GetInfo(folding_action->GetTo());
  int64_t from_delay = *node_delay.GetInfo(folding_action->GetFrom());
  return std::pow(static_cast<double>(to_delay - from_delay), 2);
}
...
absl::StatusOr<absl::flat_hash_set<std::pair<Node*, Node*>>>
ComputeMutualExclusionAnalysis(FunctionBase* f, OptimizationContext& context,
                               ResourceSharingPass::Analyses& analyses) {
  absl::flat_hash_set<std::pair<Node*, Node*>> mutual_exclusivity;

  for (Node* one_node : f->nodes()) {
    if (!CanTarget(one_node) || !ShouldTarget(one_node)) {
      continue;
    }
    for (Node* other_node : f->nodes()) {
      if (one_node->id() >= other_node->id()) {
        continue;
      }
      if (!CanTarget(other_node) || !ShouldTarget(other_node)) {
        continue;
      }
      if (analyses.single_select_visibility.IsMutuallyExclusive(one_node,
                                                                other_node) ||
          analyses.visibility.IsMutuallyExclusive(one_node, other_node)) {
        mutual_exclusivity.insert({one_node, other_node});
        mutual_exclusivity.insert({other_node, one_node});
      }
    }
  }
  ...
  if (VLOG_IS_ON(5)) {
    ...
    for (const auto& pair : mutual_exclusivity) {
      if (pair.first->id() < pair.second->id()) {
        node_to_mutual_exclusive_nodes[pair.first].insert(pair.second);
      }
    }
...
ComputeFoldableActions(
  ...
  for (auto& [from_node, to_node] : mutual_exclusivity) {
    if (to_node->id() < from_node->id()) {
      continue;
    }
    if (!CanTarget(from_node) || !ShouldTarget(from_node)) {
      continue;
    }
    if (!CanTarget(to_node) || !ShouldTarget(to_node)) {
      continue;
    }
    bool can_from_to = CanMapOpInto(from_node, to_node);
    bool can_to_from = CanMapOpInto(to_node, from_node);
    if (!can_from_to && !can_to_from) {
      continue;
    }
    ...
    if (analyses.single_select_visibility.IsMutuallyExclusive(from_node,
                                                              to_node)) {
      XLS_ASSIGN_OR_RETURN(
          from_edges,
          analyses.single_select_visibility.GetEdgesForVisibilityExpr(
              from_node));
      XLS_ASSIGN_OR_RETURN(
          to_edges,
          analyses.single_select_visibility.GetEdgesForVisibilityExpr(to_node));
    } else {
      XLS_ASSIGN_OR_RETURN(
          from_edges,
          analyses.visibility.GetEdgesForMutuallyExclusiveVisibilityExpr(
              from_node, {to_node}, ResourceSharingPass::kMaxEdgesToHandle));
      XLS_ASSIGN_OR_RETURN(
          to_edges,
          analyses.visibility.GetEdgesForMutuallyExclusiveVisibilityExpr(
              from_node, {to_node}, ResourceSharingPass::kMaxEdgesToHandle));
    ...
    if (from_edges.empty() || to_edges.empty()) {
      VLOG(4) << "Cannot determine visibility expression necessary to fold: "
              << from_node->GetName() << " into: " << to_node->GetName();
      continue;
    }

    if (can_from_to) {
      VLOG(4) << "Adding folding action: " << from_node->GetName() << " "
              << to_node->GetName();
      foldable_actions.push_back(std::make_unique<BinaryFoldingAction>(
          from_node, to_node, from_edges, to_edges));
    }
    if (can_to_from) {
      VLOG(4) << "Adding folding action: " << to_node->GetName() << " "
              << from_node->GetName();
      foldable_actions.push_back(std::make_unique<BinaryFoldingAction>(
          to_node, from_node, to_edges, from_edges));
    }
  }
...
absl::StatusOr<std::unique_ptr<NaryFoldingAction>> MakeNaryFoldingAction(
  ...
  for (int i = 0; i < subset_of_edges_to_n.size(); ++i) {
    ...
    for (Node* other : rest_of_others) {
      if (!analyses.single_select_visibility.IsMutuallyExclusive(
              binary_folding->GetFrom(), other)) {
        is_describable_via_single_select = false;
      }
      if (!analyses.visibility.IsMutuallyExclusive(binary_folding->GetFrom(),
                                                   other)) {
    ...
    if (is_describable_via_single_select) {
      ...
      XLS_ASSIGN_OR_RETURN(
          edges, analyses.visibility.GetEdgesForMutuallyExclusiveVisibilityExpr(
...
SelectFoldingActionsBasedOnCliques(FoldingGraph* folding_graph,
  ...
  for (const absl::flat_hash_set<BinaryFoldingAction*>& clique :
    ...
    for (BinaryFoldingAction* binary_folding : clique) {
      ...
      area_saved += binary_folding->area_saved().value_or(0);
    }
    ...
    XLS_ASSIGN_OR_RETURN(new_action,
                         MakeNaryFoldingAction(folds, area_saved, analyses));
...
bool CanFoldTogether(
  ...
  if (!mutual_exclusivity.contains(std::make_pair(prev_node, next_node))) {
...
struct NaryFoldEstimate {
  std::vector<BinaryFoldingAction*> folds;
  double area_saved;
  int64_t delay_spread;
  uint64_t delay_increase;
};

absl::StatusOr<NaryFoldEstimate> EstimateAndSelectSubsetOfFolds(
  ...
  estimate.folds.reserve(possible_folds.size());
  double area_saved = 0;
  ...
  for (BinaryFoldingAction* folding : possible_folds) {
    ...
    XLS_ASSIGN_OR_RETURN(
        double area_of_from,
        analyses.area_estimator.GetOperationAreaInSquareMicrons(from));
    ...
    XLS_ASSIGN_OR_RETURN(
        double area_select,
        EstimateAreaForSelectingASingleInput(folding, analyses.area_estimator));
      ...
      if (from_operand->op() != Op::kNeg) {
        ...
        XLS_ASSIGN_OR_RETURN(double negate_area,
                             EstimateAreaForNegatingNode(
                                 from->operand(1), analyses.area_estimator));
    ...
    int64_t delay_spread =
        GetDelaySpread(folding, analyses.critical_path_delay);
    ...
    uint64_t delay_increase =
        GetDelayIncrease(analyses.critical_path_delay, estimate.folds, folding);
    ...
    estimate.folds.push_back(folding);
  ...
  estimate.area_saved = area_saved;
  estimate.delay_spread = total_spread;
  estimate.delay_increase =
      max_selector_delay_increase +
      GetDelayIncrease(analyses.critical_path_delay, estimate.folds);
  return estimate;
}
...
ListOfAllFoldingActionsWithDestination(
  ...
  for (BinaryFoldingAction* a : edges_to_n) {
    ...
    bool is_a_mutually_exclusive = absl::c_all_of(
        subset_of_edges_to_n, std::bind(CanFoldTogether, mutual_exclusivity, a,
                                        std::placeholders::_1));
  ...
  XLS_ASSIGN_OR_RETURN(NaryFoldEstimate estimate,
                       EstimateAndSelectSubsetOfFolds(
                           subset_of_edges_to_n, analyses, visibility_estimator,
  ...
  XLS_ASSIGN_OR_RETURN(
      std::unique_ptr<NaryFoldingAction> new_action,
      MakeNaryFoldingAction(estimate.folds, estimate.area_saved, analyses));
...
ListOfFoldingActionsWithDestination(
  ...
  for (int i = 0; i < subsets_of_edges_to_n.size(); ++i) {
    ...
    XLS_ASSIGN_OR_RETURN(
        NaryFoldEstimate estimate,
        EstimateAndSelectSubsetOfFolds(
            subsets_of_edges_to_n[i], analyses, visibility_estimator,
...
LegalizeSequenceOfFolding(
  ...
  for (const std::unique_ptr<NaryFoldingAction>& folding :
    ...
    for (auto& [source, source_edges] : folding->GetFrom()) {
      ...
      if (nodes_already_selected_as_folding_sources.contains(source)) {
        ...
        XLS_ASSIGN_OR_RETURN(
            ...
            analyses.area_estimator.GetOperationAreaInSquareMicrons(source));
    ...
    // Even if safe, the current n-ary fold destination has a better fold.
    // NOTE: the current fold would have been merged into the prior fold when
    // the prior fold was first created if this had been deemed profitable.
...
SelectFoldingActionsBasedOnInDegree(
  ...
  for (Node* n : nodes) {
    ...
    XLS_ASSIGN_OR_RETURN(std::vector<std::unique_ptr<NaryFoldingAction>>
                         ...
                         ListOfFoldingActionsWithDestination(
                             n, folding_graph, mutual_exclusivity, analyses,
  ...
  if (VLOG_IS_ON(5)) {
    ...
    for (const std::unique_ptr<NaryFoldingAction>& folding :
      ...
      VLOG(5) << "";
      VLOG(5) << "    To [delay="
              << *analyses.critical_path_delay.GetInfo(folding->GetTo()) << "] "
              << folding->GetTo()->ToString();
      for (auto& [from_node, _] : folding->GetFrom()) {
        VLOG(5) << "      From [delay="
                << *analyses.critical_path_delay.GetInfo(from_node) << "] "
                << from_node->ToString();
      }
...
SelectRandomlyFoldingActions(
  ...
  for (auto& [destination, indexes] : indexes_of_selected_edges) {
    ...
    for (uint64_t index : indexes) {
      ...
      area_saved += edge->area_saved().value_or(0.0);
    }
...
SelectFoldingActions(
  ...
  switch (heuristics) {
    ...
    case ResourceSharingPass::ProfitabilityGuard::kCliques: {
      XLS_ASSIGN_OR_RETURN(
          folding_actions_to_perform,
          SelectFoldingActionsBasedOnCliques(folding_graph, analyses));
      break;
    }
    ...
    case ResourceSharingPass::ProfitabilityGuard::kRandom: {
      XLS_ASSIGN_OR_RETURN(folding_actions_to_perform,
                           SelectRandomlyFoldingActions(
                               folding_graph, mutual_exclusivity, analyses));
      break;
    }
  ...
  TimingAnalysis ta{folding_actions_to_perform, analyses.critical_path_delay};
  std::vector<std::unique_ptr<NaryFoldingAction>>
...
absl::StatusOr<bool> PerformFoldingActions(
  ...
  std::function<Node*(Node*)> final_rename = [&](Node* initial_node) -> Node* {
    if (auto it = renaming_done_by_previous_folding.find(initial_node);
        it != renaming_done_by_previous_folding.end()) {
      auto final_name = final_rename(it->second);
      return final_name;
    }
    return initial_node;
  };
  ...
  for (const std::unique_ptr<NaryFoldingAction>& folding :
    ...
    for (auto& [from_node, from_edges] : folding->GetFrom()) {
      Node* renamed_node = final_rename(from_node);
      absl::flat_hash_set<OperandVisibilityAnalysis::OperandNode>
          replaced_edges;
      for (auto edge : from_edges) {
        edge.operand = final_rename(edge.operand);
        edge.node = final_rename(edge.node);
        replaced_edges.insert(edge);
      }
    ...
    for (auto& [from_node, from_edges] : froms_to_use) {
      VLOG(4) << "        Source: " << from_node->ToString();
      XLS_ASSIGN_OR_RETURN(Node * from_used,
                           visibility_estimator->BuildVisibilityIRExpr(
                               f, from_node, from_edges));
    ...
    if (from_used_expressions.size() > 1) {
      ...
      new_selector = from_used_expressions[0];
    }
...
absl::StatusOr<bool> ResourceSharingPass::RunOnFunctionBaseInternal(
  ...
  auto critical_path_delay =
      std::make_unique<CriticalPathDelayAnalysis>(delay_model);
  ...
  ResourceSharingPass::Analyses analyses = {
      ...
      .single_select_visibility = *single_select_visibility,
  ...
  absl::flat_hash_set<std::pair<Node*, Node*>> mutual_exclusivity;
  XLS_ASSIGN_OR_RETURN(mutual_exclusivity,
                       ComputeMutualExclusionAnalysis(f, context, analyses));
  ...
  XLS_ASSIGN_OR_RETURN(
      std::vector<std::unique_ptr<BinaryFoldingAction>> foldable_actions,
      ComputeFoldableActions(f, mutual_exclusivity, analyses));
...
TimingAnalysis::TimingAnalysis(
  ...
  for (const std::unique_ptr<NaryFoldingAction>& folding : folding_actions) {
    ...
    for (auto& [from, _] : folding->GetFrom()) {
      int64_t from_delay = *node_delay.GetInfo(from);
