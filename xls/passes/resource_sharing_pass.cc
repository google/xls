...
class FoldingAction {
 public:
  ...
  std::optional<double> GetSavedArea() const;
...
// Return the list of n-ary folding actions that target a given node
absl::StatusOr<std::vector<std::unique_ptr<NaryFoldingAction>>>
ListOfFoldingActionsWithDestination(
  ...
  for (BinaryFoldingAction *folding : subset_of_edges_to_n) {
    ...
    XLS_ASSIGN_OR_RETURN(
        Node * overhead_selector,
        f->MakeNode<BitSlice>(select->loc(), folding->GetSelector(), 0, 1));
    std::vector<Node *> overhead_select_cases = {from->operand(0)};
    XLS_ASSIGN_OR_RETURN(
        Node * overhead_select,
        f->MakeNode<PrioritySelect>(select->loc(), overhead_selector,
                                    overhead_select_cases, n->operand(0)));
    XLS_ASSIGN_OR_RETURN(double area_select,
                         ae.GetOperationAreaInSquareMicrons(overhead_select));
    XLS_RETURN_IF_ERROR(f->RemoveNode(overhead_select));
    XLS_RETURN_IF_ERROR(f->RemoveNode(overhead_selector));
    ...
    if ((from->op() != n->op()) && n->OpIn({Op::kAdd, Op::kSub}) &&
      ...
      if (from_operand->op() != Op::kNeg) {
        ...
        XLS_ASSIGN_OR_RETURN(
            Node * negate_node,
            f->MakeNode<UnOp>(select->loc(), from->operand(1), Op::kNeg));
        XLS_ASSIGN_OR_RETURN(double negate_area,
                             ae.GetOperationAreaInSquareMicrons(negate_node));
        XLS_RETURN_IF_ERROR(f->RemoveNode(negate_node));
...
LegalizeSequenceOfFolding(
  ...
  for (const std::unique_ptr<NaryFoldingAction> &folding :
    ...
    VLOG(2) << "      Area savings = " << *folding->GetSavedArea();
    ...
    std::optional<double> optional_area_saved = folding->GetSavedArea();
    CHECK_EQ(optional_area_saved.has_value(), true);
    ...
    if (area_saved <= 0) {
      VLOG(4) << "      Excluding the current n-ary folding because it does "
                 "not save enough area";
      continue;
    }
    ...
    CHECK(!prior_folding_of_destination.contains(to_node));
    ...
    for (auto [from, from_case_number] : legal_froms) {
      CHECK(!nodes_already_selected_as_folding_sources.contains(from));
...
SelectFoldingActionsBasedOnInDegree(
  ...
  if (VLOG_IS_ON(5)) {
    ...
    for (const std::unique_ptr<NaryFoldingAction> &folding :
      ...
      VLOG(5) << "      Area savings = " << *folding->GetSavedArea();
    }
  }
  ...
  auto area_comparator = [](std::unique_ptr<NaryFoldingAction> &f0,
                            std::unique_ptr<NaryFoldingAction> &f1) -> bool {
    std::optional<double> area_f0 = f0->GetSavedArea();
    std::optional<double> area_f1 = f1->GetSavedArea();
  ...
  absl::c_sort(potential_folding_actions_to_perform, area_comparator);
...
SelectFoldingActions(
  ...
  if (VLOG_IS_ON(2)) {
    ...
    for (const std::unique_ptr<NaryFoldingAction> &folding :
      ...
      std::optional<double> area_saved = folding->GetSavedArea();
...
absl::StatusOr<bool> PerformFoldingActions(
  ...
  for (const std::unique_ptr<NaryFoldingAction> &folding :
    ...
    if (renaming_done_by_previous_folding.contains(to_node)) {
      ...
      CHECK_NE(prior_select, nullptr);
      if (prior_select != select) {
        continue;
      }
    }
    CHECK_NE(to_node, nullptr);
    ...
    for (auto [from_node, from_node_case_number] : folding->GetFrom()) {
      ...
      if (renaming_done_by_previous_folding.contains(from_node)) {
        ...
        Node *prior_select = new_name_and_prior_select.second;
        CHECK_NE(prior_select, nullptr);
        CHECK_EQ(prior_select, select);
        renamed_node = new_name_and_prior_select.first;
      }
      CHECK_NE(renamed_node, nullptr);
...
absl::StatusOr<bool> ResourceSharingPass::RunOnFunctionBaseInternal(
  ...
  // Get the area model
  XLS_ASSIGN_OR_RETURN(AreaEstimator * ae, GetAreaEstimator("v"));
  ...
  XLS_ASSIGN_OR_RETURN(
      std::vector<std::unique_ptr<NaryFoldingAction>>
          folding_actions_to_perform,
      SelectFoldingActions(&folding_graph, profitability_guard_, *ae,
                           mutual_exclusivity_relation));
...
std::optional<double> FoldingAction::GetSavedArea() const {
  return area_saved_;
}
...
uint64_t FoldingGraph::GetOutDegree(Node *n) const {
  NodeIndex node_id = node_to_index_.at(n);
  uint64_t in_degree = graph_->OutDegree(node_id);
