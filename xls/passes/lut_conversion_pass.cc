...
// Builds the index to use when looking up a case in a LUT for `node` as
// generated by GetLutCases. The index is a concat of the unknown bits of `cut`,
// which are assumed to fully determine the value of `node`.
...
// Returns all possible values of `node` given the values of `cut`, which are
// assumed to fully determine the value of `node`. The values are returned in an
// order that matches GetLutIndex, so together these can be used to build a
// lookup table.
...
// Compute the potential delay that would be removed if we replaced the given
...
struct DeltaPPA {
  ...
  DeltaPPA& operator+=(const DeltaPPA& other) {
    net_area += other.net_area;
    net_delay += other.net_delay;
    return *this;
  }
  DeltaPPA& operator-=(const DeltaPPA& other) {
    net_area -= other.net_area;
  ...
  DeltaPPA operator+(const DeltaPPA& other) const {
    DeltaPPA result = *this;
    result += other;
    return result;
  }
  DeltaPPA operator-(const DeltaPPA& other) const {
  ...
  DeltaPPA operator-() const {
    return DeltaPPA{.net_area = -net_area, .net_delay = -net_delay};
  }

  friend bool operator==(const DeltaPPA& a, const DeltaPPA& b) {
    return a.net_area == b.net_area && a.net_delay == b.net_delay;
  }

  friend std::partial_ordering operator<=>(const DeltaPPA& a,
                                           const DeltaPPA& b) {
    std::partial_ordering area_order = std::partial_ordering::equivalent;
    double area_diff = b.net_area - a.net_area;
    if (std::abs(area_diff) > kAreaTolerance) {
      area_order = area_diff > 0 ? std::partial_ordering::less
                                 : std::partial_ordering::greater;
    }

    std::partial_ordering delay_order = a.net_delay <=> b.net_delay;

    if (area_order == std::partial_ordering::equivalent) {
      return delay_order;
    }
    if (delay_order == std::partial_ordering::equivalent) {
      return area_order;
    }
    if (area_order == delay_order) {
      return area_order;
    }
...
using Change = std::variant<NotImproved, Improved, DeltaPPA>;

absl::StatusOr<Change> GetChange(Select* select, Select* new_select,
                                 int64_t unknown_bits, Node* lut_index,
                                 bool selector_is_trivial,
  ...
  XLS_ASSIGN_OR_RETURN(double old_area,
...
absl::StatusOr<bool> MaybeMergeLutIntoSelects(
  ...
  std::optional<DeltaPPA> replaceable_delta;
  ...
  if (!selector_is_trivial && area_estimator != nullptr &&
    ...
    XLS_ASSIGN_OR_RETURN(
        ...
        ComputeSavedDelay(selector, min_cut_set, *dataflow_graph_analysis,
                          *delay_estimator));
    replaceable_delta =
        DeltaPPA{.net_area = replaceable_area, .net_delay = saved_delay};
  }
  ...
  for (Select* select : candidate_selects) {
    // Build a select (with placeholder cases) of the same type as the select we
    // might end up replacing.
    XLS_ASSIGN_OR_RETURN(
    ...
    // Now that we have this select, we can ask our models (if any) how it
    // compares to the original select.
    XLS_ASSIGN_OR_RETURN(Change change,
                         GetChange(select, new_select, unknown_bits, lut_index,
                                   selector_is_trivial, placeholders,
                                   delay_estimator, area_estimator));
    delta_ppa.insert({select, std::move(change)});
  }

  // We need to determine whether we should merge all of the new selects for
  // global benefit, just the ones where a replacement improves local area and
  // delay, or if the LUT transformation isn't worthwhile. By default, we use
  // the LUT transform locally, but if we have area & delay models, we can
  // choose to merge globally.
  ...
  if (replaceable_delta.has_value()) {
    DeltaPPA zero_delta = DeltaPPA{.net_area = 0.0, .net_delay = 0};

    DeltaPPA full_replacement_delta = -(*replaceable_delta);
    DeltaPPA local_replacement_delta = zero_delta;
    ...
    for (const auto& [select, change] : delta_ppa) {
      ...
      full_replacement_delta += delta;
      if (delta < zero_delta) {
        local_replacement_delta += delta;
      }
    }

    if (local_replacement_delta >= zero_delta &&
        full_replacement_delta >= zero_delta) {
      merge_strategy = MergeStrategy::kNone;
    } else if (full_replacement_delta <= local_replacement_delta) {
  ...
  if (merge_strategy == MergeStrategy::kMergeSome) {
    std::erase_if(candidate_selects, [&](Select* select) {
      if (std::holds_alternative<DeltaPPA>(delta_ppa.at(select))) {
        ...
        if (delta.net_area > kAreaTolerance || delta.net_delay >= 0) {
