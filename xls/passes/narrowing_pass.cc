...
class NarrowVisitor final : public DfsVisitorWithDefault {
 public:
  ...
  // unknown bits cannot overwrite them.
  ...
  // arith shift or if they are all 0s.
  //
  ...
  absl::Status HandleDynamicBitSlice(DynamicBitSlice* slice) override {
    ...
    if (auto known_slice_start =
      ...
      XLS_ASSIGN_OR_RETURN(int64_t int_val, known_slice_start->ToUint64());
      XLS_ASSIGN_OR_RETURN(Node * replaced,
                           slice->ReplaceUsesWithNew<BitSlice>(
                               slice->to_slice(), int_val, slice->width()));
      return Change(slice, replaced);
    }
    Bits max_val = slice_start_qe.MaxUnsignedValue(slice->start());

    int64_t val_bit_width =
        1 + std::max(max_val.bit_count(),
                     Bits::MinBitCountUnsigned(slice->width()));
    Bits max_sliced_val_idx =
        bits_ops::Add(bits_ops::ZeroExtend(max_val, val_bit_width),
                      UBits(slice->width(), val_bit_width));
    if (max_sliced_val_idx.FitsInNBitsUnsigned(63)) {
      XLS_ASSIGN_OR_RETURN(int64_t max_sliced_val,
                           max_sliced_val_idx.ToUint64());
