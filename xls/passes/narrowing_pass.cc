...
class NarrowVisitor final : public DfsVisitorWithDefault {
 public:
  ...
  absl::Status HandleDynamicBitSlice(DynamicBitSlice* slice) override {
    ...
    Bits max_val = QueryEngineForNodeAndUser(slice->start(), slice)
                       .MaxUnsignedValue(slice->start());
    int64_t val_bit_width = std::max(max_val.bit_count() + 1,
