...
TEST_F(UselessIORemovalPassTest, RemoveSendIfLiteralFalse) {
  ...
  EXPECT_THAT(
      ...
      ElementsAre(m::Next(proc->GetStateElement(0),
...
TEST_F(UselessIORemovalPassTest, RemoveReceiveNonBlockingIfLiteralFalse) {
  ...
  EXPECT_THAT(
      proc->next_values(proc->GetStateElement(0)),
      ElementsAre(m::Next(proc->GetStateElement(0), m::TupleIndex(tuple, 0))));
  EXPECT_THAT(
      proc->next_values(proc->GetStateElement(1)),
      ElementsAre(m::Next(proc->GetStateElement(1), m::TupleIndex(tuple, 1))));
}
...
TEST_F(UselessIORemovalPassTest, RemoveSendPredIfLiteralTrue) {
  ...
  EXPECT_THAT(proc->next_values(proc->GetStateElement(1)),
              ElementsAre(m::Next(proc->GetStateElement(1), m::Literal(0))));
}
