 ...
 private:
  std::string GetCommentFromMatch(const clang::ASTContext* context,
    ...
    for (clang::comments::Comment* c :
         absl::iterator_range<decltype(comment->child_begin())>(
  ...
  void ParseComment(clang::comments::Comment* c, std::ostringstream& oss) {
    ...
    if (clang::comments::TextComment* tc =
      ...
      for (clang::comments::Comment* c :
           absl::iterator_range<decltype(pc->child_begin())>(pc->child_begin(),
                                                             pc->child_end())) {
        ParseComment(c, oss);
      }
    }
...
absl::StatusOr<std::string> ParseCppComments(
  ...
  std::vector<std::string> argv = {
      ...
      "-fparse-all-comments",
      "-fsyntax-only",
      // TODO(allight): Ideally these & -std should come from bzl rules but its
      // not clear where they come from during the build.
      // Don't add includes.
      "-nostdinc++",
      "-nostdlib++",
      // Use cpp20
