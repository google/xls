...
std::optional<LiftedOpInfo> GetLiftableOperationInfoForOp(
  ...
  // If the shared node is on the RHS, we can only lift identity cases if the
  // operation has a constant left-identity.
  if (!(*shared_is_lhs) &&
    ...
    if (!safe_op) {
      VLOG(3) << "Cannot lift: shared node is RHS of op " << OpToString(test_op)
              << " which lacks a constant left-identity, "
              << " and there are identity cases.";
      return std::nullopt;
    }
  }
