...
absl::StatusOr<std::optional<LiftedOpInfo>> CanLiftSelect(
  ...
  absl::flat_hash_set<Node*> potential_shared_nodes;
  Node* first_case = cases[0];
  if (first_case->op() == Op::kArrayIndex) {
    // Handled after binary operations.
  } else if (first_case->operand_count() == 2) {
    potential_shared_nodes.insert(first_case->operand(0));
    potential_shared_nodes.insert(first_case->operand(1));
  } else {
    potential_shared_nodes.insert(first_case);
  }
...
absl::StatusOr<TransformationResult> LiftSelect(FunctionBase *func,
  ...
  if (!ShouldLiftSelect(func, select_to_optimize, info)) {
