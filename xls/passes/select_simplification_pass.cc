...
// are arranged.
bool AreOppositeComparesHelper(Node* first_compare, Node* second_compare,
                               absl::Span<Op const> gt_ops,
                               absl::Span<Op const> lt_ops,
                               const QueryEngine& query_engine) {
  // The nodes must have two operands.
  ...
  auto operands_equal = [&]() {
    return query_engine.NodesKnownUnsignedEquals(first_compare_lhs,
                                                 second_compare_lhs) &&
           query_engine.NodesKnownUnsignedEquals(first_compare_rhs,
                                                 second_compare_rhs);
  };
  // Check if the nodes operands are equal to each other in flipped order.
  auto flipped_operands_equal = [&]() {
    return query_engine.NodesKnownUnsignedEquals(first_compare_lhs,
                                                 second_compare_rhs) &&
           query_engine.NodesKnownUnsignedEquals(first_compare_rhs,
                                                 second_compare_lhs);
...
// Given two nodes, check if they are comparison nodes and if they are opposite
// to each other. For example, (a > b) and (b < a).
bool AreOppositeCompares(Node* first_compare, Node* second_compare,
                         const QueryEngine& query_engine) {
  return AreOppositeComparesHelper(first_compare, second_compare,
                                   {Op::kUGt, Op::kSGt}, {Op::kULt, Op::kSLt},
                                   query_engine);
}

// Given two nodes, check if they are equality comparison nodes and if they are
// opposite to each other. For example, (a >= b) and (b <= a).
bool AreOppositeAndInversedCompares(Node* first_compare, Node* second_compare,
                                    const QueryEngine& query_engine) {
  return AreOppositeComparesHelper(first_compare, second_compare,
                                   {Op::kUGe, Op::kSGe}, {Op::kULe, Op::kSLe},
                                   query_engine);
}
...
absl::StatusOr<bool> SimplifyNode(Node* node, const QueryEngine& query_engine,
  ...
  if (node->Is<Select>()) {
    Select* sel = node->As<Select>();
    if (IsBinarySelect(sel)) {
      if (IsBinarySelect(sel->cases()[0])) {
        Select* sub_sel_lhs = sel->cases()[0]->As<Select>();
        Node* first_compare = sel->selector();
        Node* second_compare = sub_sel_lhs->selector();
        if (AreOppositeCompares(first_compare, second_compare, query_engine)) {
          // Replace the second compare with an equality op.
          XLS_ASSIGN_OR_RETURN(Node * eq_node,
                               node->function_base()->MakeNode<CompareOp>(
                                   node->loc(), first_compare->operands()[0],
                                   first_compare->operands()[1], Op::kEq));
          XLS_RETURN_IF_ERROR(sub_sel_lhs->ReplaceOperandNumber(0, eq_node));
          // Swap the results so that the equality op results in the equality
          // result.
          sub_sel_lhs->SwapOperands(1, 2);
          return true;
        }
      } else if (IsBinarySelect(sel->cases()[1])) {
        Select* sub_sel_rhs = sel->cases()[1]->As<Select>();
        Node* first_compare = sel->selector();
        Node* second_compare = sub_sel_rhs->selector();
        if (AreOppositeAndInversedCompares(first_compare, second_compare,
                                           query_engine)) {
          XLS_ASSIGN_OR_RETURN(Node * eq_node,
                               node->function_base()->MakeNode<CompareOp>(
                                   node->loc(), first_compare->operands()[0],
                                   first_compare->operands()[1], Op::kEq));
          XLS_RETURN_IF_ERROR(sub_sel_rhs->ReplaceOperandNumber(0, eq_node));
          return true;
        }
      }
    }
      ...
      if (AreOppositeCompares(first_compare, second_compare, query_engine)) {
        XLS_ASSIGN_OR_RETURN(Node * eq_node,
                             node->function_base()->MakeNode<CompareOp>(
                                 node->loc(), first_compare->operands()[0],
                                 first_compare->operands()[1], Op::kEq));
        XLS_RETURN_IF_ERROR(selector->ReplaceOperandNumber(1, eq_node));
      ...
      if (AreOppositeCompares(first_compare, second_compare, query_engine)) {
        XLS_ASSIGN_OR_RETURN(Node * eq_node,
                             node->function_base()->MakeNode<CompareOp>(
                                 node->loc(), first_compare->operands()[0],
                                 first_compare->operands()[1], Op::kEq));
        XLS_RETURN_IF_ERROR(sub_sel->ReplaceOperandNumber(0, eq_node));
