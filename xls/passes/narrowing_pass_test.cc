...
TEST_P(NarrowingPassTest, ProcStateInformationIsUsed) {
  ...
  EXPECT_THAT(incr.node(),
              m::Next(m::StateElement("foo"), m::ZeroExt(m::Add()),
                      m::ULt(m::Type("bits[4]"), m::Type("bits[4]"))));
  EXPECT_THAT(rst.node(),
              m::Next(m::StateElement("foo"), m::Literal(),
...
void IrFuzzNarrowingTernary(FuzzPackageWithArgs fuzz_package_with_args) {
...
void IrFuzzNarrowingRange(FuzzPackageWithArgs fuzz_package_with_args) {
