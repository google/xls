...
absl::Status MakeOutputDataPortsForOutputChannels(
  ...
  absl::flat_hash_set<Node*> multi_output_data_ports;
  std::vector<absl::flat_hash_set<Node*>> multi_output_data_ports_by_stage;
  multi_output_data_ports_by_stage.resize(streaming_outputs.size());
  absl::flat_hash_set<Node*> data_ports;
  for (Stage stage = 0; stage < streaming_outputs.size(); ++stage) {
    absl::flat_hash_set<Node*> stage_data_ports;
    for (StreamingOutput& streaming_output : streaming_outputs[stage]) {
      CHECK(streaming_output.GetDataPort().has_value());
      Node* data_port = *streaming_output.GetDataPort();

      if (!stage_data_ports.insert(data_port).second) {
        multi_output_data_ports_by_stage[stage].insert(data_port);
      }
      if (!data_ports.insert(data_port).second) {
        multi_output_data_ports.insert(data_port);
      }
    }
  }

  absl::flat_hash_map<Node*, absl::btree_map<Stage, std::vector<Node*>>>
      data_values;
  for (Stage stage = 0; stage < streaming_outputs.size(); ++stage) {
    for (StreamingOutput& streaming_output : streaming_outputs[stage]) {
      Node* data = *streaming_output.GetData();
      Node* data_port = *streaming_output.GetDataPort();
      if (std::optional<Node*> predicate = streaming_output.GetPredicate();
          predicate.has_value() &&
          multi_output_data_ports_by_stage[stage].contains(data_port)) {
        // There is more than one node that can produce data for this channel;
        // gate the output on the predicate.
        XLS_ASSIGN_OR_RETURN(
            data, block->MakeNode<Gate>(SourceInfo(), *predicate, data));
      }
      data_values[data_port][stage].push_back(data);
    }
  }
  for (auto& [data_port, data_values_by_stage] : data_values) {
    ...
    for (auto& [stage, stage_data_values] : data_values_by_stage) {
      ...
      XLS_ASSIGN_OR_RETURN(Node * stage_data,
                           FanInAsNeeded(block, stage_data_values));

      if (multi_output_data_ports.contains(data_port)) {
        // More than one stage can produce data for this channel; gate it on the
        // stage being valid to avoid conflict.
        XLS_ASSIGN_OR_RETURN(
            Node * valid,
            block->MakeNode<NaryOp>(SourceInfo(),
                                    ...
                                    Op::kAnd));
        XLS_ASSIGN_OR_RETURN(
            stage_data, block->MakeNode<Gate>(SourceInfo(), valid, stage_data));
      }

      datas.push_back(stage_data);
    }
    XLS_ASSIGN_OR_RETURN(Node * data, FanInAsNeeded(block, datas));
