...
absl::StatusOr<dslx::Expr*> DslxBuilder::HandleIntegerExponentiation(
  ...
  XLS_ASSIGN_OR_RETURN(int64_t rhs_bit_count,
                       BitCountWithConstantFolding(vast_rhs_type));
...
absl::StatusOr<dslx::Expr*> DslxBuilder::Cast(verilog::DataType* vast_type,
                                              dslx::Expr* expr,
                                              bool force_cast_user_defined) {
  if (!BitCountWithConstantFolding(vast_type).ok()) {
...
absl::StatusOr<dslx::TypeAnnotation*> DslxBuilder::VastTypeToDslxTypeForCast(
  ...
  XLS_ASSIGN_OR_RETURN(int64_t bit_count,
                       BitCountWithConstantFolding(vast_type));
...
absl::StatusOr<int64_t> DslxBuilder::BitCountWithConstantFolding(
    verilog::DataType* vast_type) {
  absl::StatusOr<int64_t> direct_bit_count = vast_type->FlatBitCountAsInt64();
  if (direct_bit_count.ok()) {
    return *direct_bit_count;
  }
  XLS_ASSIGN_OR_RETURN(verilog::DataType * folded_type,
                       verilog::FoldVastConstants(vast_type, vast_type_map_));
...
absl::StatusOr<std::string> DslxBuilder::FormatModule() {
  ...
  VLOG(2) << "Determining DSLX type fixes.";
  ...
  VLOG(2) << "Applying DSLX type fixes.";
  XLS_ASSIGN_OR_RETURN(
  ...
  VLOG(2) << "Running simplify replacer.";
  XLS_ASSIGN_OR_RETURN(
