 ...
 public:
  ...
  void AddChannelMetadata(BlockChannelMetadata metadata) {
    if (metadata.direction() == xls::ChannelDirection::kSend) {
      ...
      channel_interface_to_metadata_map_[outputs_.back()->channel_interface()] =
          outputs_.back().get();
    } else {
      inputs_.push_back(
          std::make_unique<BlockChannelMetadata>(std::move(metadata)));

      channel_interface_to_metadata_map_[inputs_.back()->channel_interface()] =
          inputs_.back().get();
    }
  }
...
#endif  // XLS_CODEGEN_PASSES_NG_STAGE_TO_BLOCK_CONVERSION_METADATA_H_
