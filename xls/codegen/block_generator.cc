...
class BlockGenerator {
 ...
 private:
  ...
  bool ShouldEmitAsAssignment(Node* const n, int64_t inline_depth) {
    bool has_multiple_uses = n->users().size() > 1;
    if (!has_multiple_uses) {
      for (Node* user : n->users()) {
        for (int64_t i = 0; i < user->operand_count(); i++) {
          if (user->operand(i) == n) {
            if (OperandAppearsMultipleTimesInExpression(user, i)) {
    ...
    return (n->HasAssignedName() ||
            (has_multiple_uses && !ShouldInlineExpressionIntoMultipleUses(n)) ||
...
AnnotationType GetAnnotationType(
    CodegenOptions::SourceAnnotationStrategy strategy, FileType file_type) {
  switch (strategy) {
    ...
    case xls::verilog::CodegenOptions::SourceAnnotationStrategy::kLineDirective:
      ...
      VLOG(5) << absl::StreamFormat(
          "Line directive is available in SystemVerilog only");
      return AnnotationType::kNone;
    default:
      return AnnotationType::kNone;
  }
