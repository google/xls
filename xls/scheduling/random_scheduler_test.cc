...
class RandomSchedulerTest : public IrTestBase {};

TEST_F(RandomSchedulerTest, SimpleFunction) {
  ...
  RandomScheduler scheduler(graph, delay_estimator,
                            absl::BitGen(std::seed_seq{42}));
  XLS_ASSERT_OK_AND_ASSIGN(
...
TEST_F(RandomSchedulerTest, SimpleProc) {
  auto p = CreatePackage();
...
TEST_F(RandomSchedulerTest, WithIOConstraint) {
  auto p = CreatePackage();
  XLS_ASSERT_OK_AND_ASSIGN(
  ...
  XLS_ASSERT_OK_AND_ASSIGN(
      ScheduleCycleMap cycle_map,
      scheduler.Schedule(/*pipeline_stages=*/4,
                         /*clock_period_ps=*/2, SchedulingFailureBehavior{}));
  EXPECT_EQ(cycle_map.at(rcv.node()) + 1, cycle_map.at(send.node()));
  RecordProperty("graph", proc->DumpIr(ScheduleCycleAnnotator(cycle_map)));
}
