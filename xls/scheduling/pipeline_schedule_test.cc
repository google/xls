...
TEST_F(PipelineScheduleTest,
       ProcFeedbackArcThroughputUnlabeledMatchesUnlabeledPattern) {
  XLS_ASSERT_OK_AND_ASSIGN(
...
TEST_F(PipelineScheduleTest, ProcFeedbackArcThroughputSpecificZeroNotEnforced) {
  XLS_ASSERT_OK_AND_ASSIGN(LabeledFeedbackArcProc setup,
                           BuildLabeledFeedbackArcProc("L_W", "L_R"));

  // worst_case = 2 (enforced), specific = 0 (not enforced).
  // Effective limit should fallback to Worst Case 2 (Next - Read <= 1).
  SchedulingOptions options;
  options.clock_period_ps(2);
  options.worst_case_throughput(2);
  // Not enforced individually
  options.arc_worst_case_throughput({{{"L_W", "L_R"}, 0}});
...
TEST_F(PipelineScheduleTest, VerifyConstraintsSucceedsWithCustomArcThroughput) {
  XLS_ASSERT_OK_AND_ASSIGN(LabeledFeedbackArcProc setup,
                           BuildLabeledFeedbackArcProc("W1", "R1"));
  setup.proc->SetInitiationInterval(0);

  // Manually schedule: read at 0, next at 3 (backedge length = 3)
  ScheduleCycleMap cycle_map;
  for (Node* node : setup.proc->nodes()) {
    if (!node->Is<Literal>()) {
      cycle_map[node] = 0;
    }
  }
  cycle_map[setup.read.node()] = 0;
  cycle_map[setup.next.node()] = 3;

  XLS_ASSERT_OK_AND_ASSIGN(
      PipelineSchedule schedule,
      PipelineSchedule::Create(setup.proc, cycle_map, /*length=*/5));

  SchedulingOptions options;
  options.worst_case_throughput(0);
  options.default_arc_worst_case_throughput(2);
  // Limit 4 allows length <= 3
  options.arc_worst_case_throughput({{{"W1", "R1"}, 4}});

  // Verification should succeed because 3 <= 3.
...
TEST_F(PipelineScheduleTest, VerifyConstraintsFailsWithViolatedArcThroughput) {
  ...
  // Manually schedule: read at 0, next at 4 (backedge length = 4)
  ScheduleCycleMap cycle_map;
  for (Node* node : setup.proc->nodes()) {
    if (!node->Is<Literal>()) {
      cycle_map[node] = 0;
  ...
  cycle_map[setup.read.node()] = 0;
  cycle_map[setup.next.node()] = 4;

  XLS_ASSERT_OK_AND_ASSIGN(
      PipelineSchedule schedule,
      PipelineSchedule::Create(setup.proc, cycle_map, /*length=*/6));
  ...
  // Verification should fail because 4 > 3.
  EXPECT_THAT(
      schedule.VerifyConstraints(options.constraints(), options),
      absl_testing::StatusIs(
          absl::StatusCode::kResourceExhausted,
          testing::HasSubstr("violates the constraint that we can achieve a "
                             "worst-case throughput of one iteration per 4 "
                             "cycles")));
}
