...
_DEFAULT_DSLX_MULTI_TEST_ARGS = ["none"]
...
def _get_dslx_multi_test_cmdline(ctx, src, append_cmd_line_args = True):
    """Returns the command that executes in the xls_dslx_test rule.
    ...
    _dslx_multi_test_args = ctx.attr.compare_list if ctx.attr.compare_list else _DEFAULT_DSLX_MULTI_TEST_ARGS
...
xls_dslx_multi_test_common_attrs = {
    "compare_list": attr.string_list(
        doc = "Runs specified tests with listed compare options. " +
        "Overrides any \"compare\" value",
    ),
}
...
def get_dslx_multi_test_cmd(ctx, src_files_to_test):
    """Returns the runfiles and commands to execute the sources files.
    ...
    cmds = []
    for src in src_files_to_test:
        cmds.extend(_get_dslx_multi_test_cmdline(ctx, src))
    return runfiles, cmds

def _xls_dslx_multi_test_impl(ctx):
    """The implementation of the 'xls_dslx_test' rule.
    ...
    runfiles, cmds = get_dslx_multi_test_cmd(ctx, src_files_to_test)
...
xls_dslx_multi_test = rule(
    doc = """A dslx multi_test executes the tests and quick checks of a DSLX source file
using all specified compare options.

Examples:

1. xls_dslx_multi_test on DSLX source files.
    ...
    xls_dslx_test(
        ...
        compare_list = ["none, "jit"],
        deps = [":bc_dslx"],
    )
    ```
In this example tests in d.x and e.x files will be run twice,
first run will not compare DSLX results,
second run will compare DSLX results with JIT results.

2. xls_dslx_multi_test on a xls_dslx_library.
    ...
    ```
In this example tests in b_dslx library will be run twice,
first run will not compare DSLX results,
second run will compare DSLX results with JIT results.
    """,
    implementation = _xls_dslx_multi_test_impl,
    attrs = dicts.add(
        xls_dslx_library_as_input_attrs,
        xls_dslx_test_common_attrs,
        xls_dslx_multi_test_common_attrs,
