...
// ws_matmul.x -- weight-stationary systolic matmul array for ML (float32).
//
// A parametric K*N weight-stationary systolic array of FP32 processing
// elements, used to assemble small MLP / matmul accelerators.
//
...
proc test_ws_2x2 {
    ...
    next(c: u32) {
        ...
        let exp = fv2(
            u32[2][2]:[
                [0x40e00000, 0x41200000], // 7, 10
                [0x41700000, 0x41b00000], // 15, 22
            ]);
        let chk0 = c == u32:3;
        let chk1 = c == u32:4;
...
proc test_ws_3x3_batch2_fp {
    ...
    next(c: u32) {
        ...
        let wv = fv2(
            u32[3][3]:[
                [0x3fc00000, 0xc0200000, 0x3f666666], // 1.5, -2.5, 0.9
                [0xc0200000, 0x40100000, 0x3d800000], // -2.5, 2.25, 0.0625
                [0x40533333, 0x3fa66666, 0x408ccccd], // 3.3, 1.3, 4.4
            ]);
        let tok = unroll_for! (k, tok): (u32, token) in u32:0..u32:3 {
            unroll_for! (j, tok): (u32, token) in u32:0..u32:3 {
                send_if(tok, w[k][j], c == u32:0, wv[k][j])
        ...
        let img = fv2(
            u32[3][2]:[
                [0xc06ccccd, 0x400ccccd, 0x40700000], // -3.7, 2.2, 3.75
                [0xbeb33333, 0xbfa00000, 0xbf8ccccd], // -0.35, -1.25, -1.1
            ]);
        let tok = unroll_for! (k, tok): (u32, token) in u32:0..u32:3 {
            let tok = send_if(tok, a[k], c == u32:0, img[u32:0][k]);
            send_if(tok, a[k], c == u32:1, img[u32:1][k])
        ...
        let exp = fv2(
            u32[3][2]:[
                [0x3fa99996, 0x41989999, 0x4154eb86], // 1.3249996, 19.074999, 13.307501
                [0xbf83d70b, 0xc057851f, 0xc0a775c3], // -1.0300001, -3.3675001, -5.2331252
            ]);
        let chk0 = c == u32:7;
        let chk1 = c == u32:8;
        let tok = unroll_for! (j, tok): (u32, token) in u32:0..u32:3 {
...
proc test_ws_4x4_fp {
    ...
    next(c: u32) {
        ...
        let wv = fv2(
            u32[4][4]:[
                [0x3e4ccccd, 0xbf333333, 0xbeb33333, 0x400ccccd], // 0.2, -0.7, -0.35, 2.2
                [0x3fc00000, 0xc0c33333, 0xbeb33333, 0x3d800000], // 1.5, -6.1, -0.35, 0.0625
                [0x408ccccd, 0x40700000, 0x40700000, 0xbeb33333], // 4.4, 3.75, 3.75, -0.35
                [0xc06ccccd, 0x3fa66666, 0x40b33333, 0xc0200000], // -3.7, 1.3, 5.6, -2.5
            ]);
        ...
        let img = fv2(
            u32[4][4]:[
                [0x40533333, 0x3e99999a, 0xbf333333, 0x408ccccd], // 3.3, 0.3, -0.7, 4.4
                [0x40700000, 0x40100000, 0x3e000000, 0x3f8ccccd], // 3.75, 2.25, 0.125, 1.1
                [0x3dcccccd, 0x400ccccd, 0x3f666666, 0x40100000], // 0.1, 2.2, 0.9, 2.25
                [0x3f333333, 0x40100000, 0x3dcccccd, 0x3f666666], // 0.7, 2.25, 0.1, 0.9
            ]);
        ...
        let exp = fv2(
            u32[4][4]:[
                [0xc1920000, 0xbf85c28f, 0x41a60a3e, 0xc05e7ae1], // -18.25, -1.045, 20.755, -3.476
                [0x3f1ae149, 0xc1673852, 0x4090eb85, 0x40b3199a], // 0.605, -14.451, 4.529, 5.597
                [0xbf85c28e, 0xc0e6147b, 0x4172b852, 0xc0b2a3d7], // -1.045, -7.19, 15.17, -5.583
                [0x3f200003, 0xc14ab852, 0x408c3d70, 0xbf1ab851], // 0.625, -12.67, 4.383, -0.604
            ]);
...
proc test_ws_2x3 {
    ...
    next(c: u32) {
        ...
        let exp = fv2(
            u32[3][2]:[
                [0x41100000, 0x41400000, 0x41700000], // 9, 12, 15
                [0x41980000, 0x41d00000, 0x42040000], // 19, 26, 33
            ]);
