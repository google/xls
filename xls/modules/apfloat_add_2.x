...
pub fn add<EXP_SZ: u32, SFD_SZ: u32>(
  ...
  let wide_x = u28:0 if x.bexp == uN[EXP_SZ]:0 else wide_x;
  let wide_y = u28:0 if y.bexp == uN[EXP_SZ]:0 else wide_y;
  ...
  let dropped_x = wide_x << ((uN[EXP_SZ]:28 - shift_x) as u28);
  let dropped_y = wide_y << ((uN[EXP_SZ]:28 - shift_y) as u28);
  ...
  let result_sfd = uN[SFD_SZ]:0 if wide_exponent < u9:1 else result_sfd;

  // Handle exponent overflow infinities.
  let result_sfd = result_sfd if wide_exponent < u9:255 else uN[SFD_SZ]:0;
  let result_exponent = wide_exponent as uN[EXP_SZ]
      if wide_exponent < u9:255 else uN[EXP_SZ]:255;

  // Handle arg infinities.
  let is_operand_inf = apfloat::is_inf<EXP_SZ, SFD_SZ>(x) |
      apfloat::is_inf<EXP_SZ, SFD_SZ>(y);
  let result_exponent = uN[EXP_SZ]:255 if is_operand_inf else result_exponent;
  ...
  let result_exponent = uN[EXP_SZ]:255 if is_result_nan else result_exponent;
  let result_sfd = uN[SFD_SZ]:0x400000 if is_result_nan else result_sfd;
