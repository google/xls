...
// here as well.
double ComputeExpected(Fpadd2x64* jit_wrapper, Float2x64 input) {
...
std::unique_ptr<Fpadd2x64> CreateJit() { return Fpadd2x64::Create().value(); }

absl::Status RealMain(uint64_t num_samples, int num_threads) {
  TestbenchBuilder<Float2x64, double, Fpadd2x64> builder(
