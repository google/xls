...
class ZstdDecodedPacket {
 public:
  static std::optional<ZstdDecodedPacket> MakeZstdDecodedPacket(
      const Value& packet) {
    // Expect tuple
    if (!packet.IsTuple()) {
      return std::nullopt;
    ...
    for (int i = 0; i < 3; i++) {
      ...
      if (!packet.element(i).IsBits()) {
        return std::nullopt;
      }
      // All fields must fit in 64bits
      if (!packet.element(i).bits().FitsInUint64()) {
        return std::nullopt;
      }
    }
...
class ZstdDecoderTest : public ::testing::Test {
 public:
  ...
  void ParseAndCompareWithZstd(std::vector<uint8_t> frame) {
    ...
    std::cout << "lib_decomp_size: " << lib_decomp_size << "\n";
