...
class ZstdDecoderTest : public ::testing::Test {
 public:
  ...
  void DecompressWithLibZSTD(std::vector<uint8_t> encoded_frame, std::vector<uint8_t> &decoded_frame) {
      size_t buff_out_size = ZSTD_DStreamOutSize();
      void* const buff_out = new uint8_t[buff_out_size];

      ZSTD_DCtx* const dctx = ZSTD_createDCtx();
      ASSERT_FALSE(dctx == NULL);
      ...
      while (input_buffer.pos < input_buffer.size) {
        ...
        bool decomp_success = ZSTD_isError(decomp_result);
        ASSERT_FALSE(decomp_success);
        ...
        ASSERT_TRUE(decomp_result == 0 && output_buffer.pos < output_buffer.size);
      }
...
INSTANTIATE_TEST_SUITE_P(
    ...
    ::testing::Range<uint32_t>(ZstdDecoderSeededTest::seed_generator_start,
      ZstdDecoderSeededTest::seed_generator_start + ZstdDecoderSeededTest::random_frames_count));
