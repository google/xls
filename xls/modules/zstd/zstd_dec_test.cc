...
class ZstdDecoderTest : public ::testing::Test {
 public:
  ...
  void ParseAndCompareWithZstd(std::vector<uint8_t> frame) {
    ...
    // Decompress the frame with libzstd
    uint8_t* lib_decomp = new uint8_t[decompressed_size];
    size_t lib_decomp_size = ZSTD_decompress(lib_decomp, decompressed_size,
                                             frame.data(), frame.size());
    EXPECT_FALSE(ZSTD_isError(lib_decomp_size));

    std::vector<uint8_t> sim_decomp;
    size_t sim_decomp_size_words =
        (decompressed_size + sizeof(uint64_t) - 1) / sizeof(uint64_t);
