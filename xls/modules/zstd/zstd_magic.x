...
pub struct MagicResult<BSIZE: u32> {
    buffer: Buffer<BSIZE>,
    status: MagicStatus,
}
...
pub fn parse_magic_number<BSIZE: u32>(buffer: Buffer<BSIZE>) -> MagicResult<BSIZE> {
    let (result, data) = buff::buffer_sized_pop_checked<BSIZE, u32:32>(buffer);
    ...
    match result.status {
        BufferStatus::OK => {
            if data == MAGIC_NUMBER {
                ...
                MagicResult {status: MagicStatus::NOT_MAGICAL, buffer: buffer}
            }
        },
        _ => {
...
fn test_parse_magic_number() {
    let buffer = Buffer { contents: MAGIC_NUMBER, length: u32:32};
    ...
    let buffer = Buffer { contents: u32:0x12345678, length: u32:32};
    let result = parse_magic_number(buffer);
    assert_eq(result, MagicResult {
        status: MagicStatus::NOT_MAGICAL,
        buffer: buffer
    });

    let buffer = Buffer { contents: u32:0x1234, length: u32:16};
    let result = parse_magic_number(buffer);
    assert_eq(result, MagicResult {
