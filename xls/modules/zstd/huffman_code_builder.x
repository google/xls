...
struct WeightCodeBuilderState<RECV_COUNT_W: u32> {
...
pub proc WeightCodeBuilder<
    ...
    RECV_COUNT: u32 = {MAX_SYMBOL_COUNT/PARALLEL_ACCESS_WIDTH},
    // recv_counter is used to iterate:
    //   0...MAX_RECV
    //   (MAX_RECV+1)...MAX_RECV*2
    // so the counter type must be wide enough to store (MAX_RECV*2)
    RECV_COUNT_W: u32 = {std::clog2((RECV_COUNT * u32:2) + u32:1)},
> {
    type State = WeightCodeBuilderState<RECV_COUNT_W>;
...
const INST_MAX_WEIGHT = hcommon::MAX_WEIGHT;

pub proc WeightCodeBuilderInst {
    type PreScanData = WeightPreScanOutput;
    type DecoderOutput = CodeBuilderToDecoderOutput;
    type PreDecoderOutput = CodeBuilderToPreDecoderOutput;
    ...
    config(
        ...
        codes_s: chan<DecoderOutput> out,
        lookahead_config_s: chan<PreDecoderOutput> out,
        weights_pow_sum_loopback_s: chan<uN[INST_MAX_WEIGHT + u32:2]> out,
        weights_pow_sum_loopback_r: chan<uN[INST_MAX_WEIGHT + u32:2]> in,
    ) {
        spawn WeightCodeBuilder (
            start_r,
            weight_r,
            codes_s,
            lookahead_config_s,
