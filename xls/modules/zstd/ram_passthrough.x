...
// TODO: This is an interesting exmple showing problems with RAM rewritting
pub proc RamPassthroughFail<
    ADDR_WIDTH: u32,
    DATA_WIDTH: u32,
    NUM_PARTITIONS: u32,
> {
    ...
    next(state: ()) {
        ...
        let tok_sent_rd_req = send_if(tok_rd_req, rd_req_s, rd_req_valid, rd_req);

        let (tok_rd_resp, rd_resp, rd_resp_valid) = recv_non_blocking(join(), rd_resp_r, zero!<ReadResp>());
        let tok_sent_rd_resp = send_if(tok_rd_resp, rd_resp_s, rd_resp_valid, rd_resp);

        let (tok_wr_req, wr_req, wr_req_valid) = recv_non_blocking(join(), wr_req_r, zero!<WriteReq>());
        let tok_sent_wr_req = send_if(tok_wr_req, wr_req_s, wr_req_valid, wr_req);

        let (tok_wr_resp, wr_resp, wr_resp_valid) = recv_non_blocking(join(), wr_resp_r, zero!<WriteResp>());
        let tok_sent_wr_resp = send_if(tok_wr_resp, wr_resp_s, wr_resp_valid, wr_resp);
