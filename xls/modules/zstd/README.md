...
5. Aggregates the processing unit results in the correct order into a stream and routes it to the history buffer,
6. Assembles the data block outputs based on the history buffer contents and updates the history,
7. Prepares the final output of the decoder and writes it to the memory,
8. (Optional) Calculates checksum and compares it against the checksum read from the frame.[^2]
...
architecture](#compressed-block-decoder-architecture) paragraph of this doc.
...
##### RLE Literals
This stage works similarly to the [RLE stage](#rleblockdecoder) for RLE data blocks.
