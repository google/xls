...
pub proc ShiftBuffer<DATA_WIDTH: u32, LENGTH_WIDTH: u32, BUFFER_WIDTH: u32 = {buffer_width(DATA_WIDTH)}> {
    ...
    next(tok0: token, state: ShiftBufferState<BUFFER_WIDTH, LENGTH_WIDTH>) {
        ...
        if ctrl_valid { trace_fmt!("Received new ctrl request! {:#x}", ctrl); } else {  };
        ...
        if recv_data_valid { trace_fmt!("Received new data {:#x}", recv_data); } else {  };
        ...
        if has_valid_request {
            trace_fmt!("Checking existing request: {:#x}", ctrl);
            if has_enough_data {
                trace_fmt!("We have enough data in {:#x} to handle the request {:#x}",
                state.buff, ctrl);
            } else {
                trace_fmt!("We don't have enough data in {:#x} to handle the request {:#x}",
                state.buff, ctrl);
            };
        } else {
        ...
        if do_send { trace_fmt!("Sending data: {:#x}", data); } else {  };
...
proc ShiftBufferTest {
    ...
    config(terminator: chan<bool> out) {
        let (in_data_s, in_data_r) = chan<ShiftBufferInput<TEST_DATA_WIDTH, TEST_LENGTH_WIDTH>>;
        let (in_ctrl_s, in_ctrl_r) = chan<ShiftBufferCtrl<TEST_LENGTH_WIDTH>>;
        let (out_data_s, out_data_r) = chan<ShiftBufferOutput<TEST_DATA_WIDTH, TEST_LENGTH_WIDTH>>;
        let (out_ctrl_s, out_ctrl_r) = chan<ShiftBufferOutput<TEST_DATA_WIDTH, TEST_LENGTH_WIDTH>>;
