...
def _run_binop(op: str, kop: str, stub: synthesis_service_pb2_grpc.SynthesisServiceStub) -> List[delay_model_pb2.DataPoint]:
  results = []
  for bit_count in [1] + list(range(2,257,2)):
    ...
    verilog_text = op_module_generator.generate_parallel_module([mod_generator_result], top_name)
    try:
