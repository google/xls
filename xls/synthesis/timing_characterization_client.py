...
def _synth(stub: synthesis_service_pb2_grpc.SynthesisServiceStub,
    ...
    while high - low > epsilon:
        ...
        if response.slack_ps >= 0:
            logging.info('PASS at %dMHz (slack %d).',
                         current, response.slack_ps)
            low = current
            if current > best_result.max_frequency_hz:
                best_result = response
        else:
            logging.info('FAIL at %dMHz (slack %d).',
                         current, response.slack_ps)
            high = current
...
def _synth_hz(stub: synthesis_service_pb2_grpc.SynthesisServiceStub,
              verilog_text: str,
              top_module_name: str) -> synthesis_pb2.CompileResponse:
    """Bisects the space of frequencies and sends requests to the server."""
    ...
    while high_hz - low_hz > epsilon_hz:
        ...
        if response.slack_ps >= 0:
            logging.info('PASS at %.2fMHz (slack %dps).',
                         current_hz / 1e6, response.slack_ps)
            low_hz = current_hz
            if current_hz > best_result.max_frequency_hz:
                best_result = response
        else:
            logging.info('FAIL at %.2fMHz (slack %dps).',
                         current_hz / 1e6, response.slack_ps)
            high_hz = current_hz
    best_result.max_frequency_hz = int(current_hz)
    return best_result


def _synth_ps(stub: synthesis_service_pb2_grpc.SynthesisServiceStub,
              verilog_text: str,
              top_module_name: str) -> synthesis_pb2.CompileResponse:
    """Bisects the space of frequencies and sends requests to the server."""
    high_hz = FLAGS.max_freq_mhz * 1e6
    ...
    while high_hz - low_hz > epsilon_hz:
        ...
        if response.slack_ps >= 0:
            logging.info('PASS at %.1fps (slack %dps).',
                         1e12 / current_hz, response.slack_ps)
            low_hz = current_hz
            if current_hz > best_result.max_frequency_hz:
                best_result = response
        else:
            logging.info('FAIL at %.1fps (slack %dps).',
                         1e12 / current_hz, response.slack_ps)
            high_hz = current_hz
    best_result.max_frequency_hz = int(current_hz)
    return best_result
...
def _synthesize_ir(stub: synthesis_service_pb2_grpc.SynthesisServiceStub,
    ...
    result = _synth_ps(stub, verilog_text, module_name)
