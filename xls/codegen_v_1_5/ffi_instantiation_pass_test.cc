...
class FfiInstantiationPassTest : public IrTestBase {
 protected:
  absl::StatusOr<bool> Run(ScheduledBlock* block) {
    PassResults results;
    return FfiInstantiationPass().RunInternal(
        block->package(), BlockConversionPassOptions(), &results);
  }
};
...
TEST_F(FfiInstantiationPassTest, InvocationsReplacedByInstance) {
  ...
  ScheduledBlockBuilder bb(TestName(), p.get());
  const BValue input_port_a = bb.InputPort("block_a_input", u32);
  const BValue input_port_b = bb.InputPort("block_b_input", u17);
  bb.StartStage(bb.Literal(UBits(1, 1)), bb.Literal(UBits(1, 1)));
  BValue invocation = bb.Invoke({input_port_a, input_port_b}, ffi_fun);
  bb.EndStage(bb.Literal(UBits(1, 1)), bb.Literal(UBits(1, 1)));
  bb.OutputPort("out", invocation);
  XLS_ASSERT_OK_AND_ASSIGN(ScheduledBlock * block, bb.Build());
...
TEST_F(FfiInstantiationPassTest, FunctionParameterIsTuple) {
  ...
  bb.StartStage(bb.Literal(UBits(1, 1)), bb.Literal(UBits(1, 1)));
  BValue invocation = bb.Invoke({param}, ffi_fun);
  bb.EndStage(bb.Literal(UBits(1, 1)), bb.Literal(UBits(1, 1)));
  bb.OutputPort("out", invocation);
  XLS_ASSERT_OK_AND_ASSIGN(ScheduledBlock * block, bb.Build());
...
TEST_F(FfiInstantiationPassTest, FunctionTakingNoParametersJustReturns) {
  ...
  ScheduledBlockBuilder bb(TestName(), p.get());
  bb.StartStage(bb.Literal(UBits(1, 1)), bb.Literal(UBits(1, 1)));
  BValue invocation = bb.Invoke({}, ffi_fun);
  bb.EndStage(bb.Literal(UBits(1, 1)), bb.Literal(UBits(1, 1)));
  bb.OutputPort("out", invocation);
  XLS_ASSERT_OK_AND_ASSIGN(ScheduledBlock * block, bb.Build());
...
TEST_F(FfiInstantiationPassTest, FunctionReturningTupleAccessAsTuple) {
  ...
  XLS_ASSERT_OK_AND_ASSIGN(InstantiationPort return_port,
                           extern_inst->GetOutputPort("return"));
  EXPECT_EQ(return_port.name, "return");
  EXPECT_EQ(return_port.type, retval.node()->GetType());
}
