...
TEST_F(ProcLoweringBlockEvalTest, StitchNetworkWithDirectConnections) {
  auto p = CreatePackage();
  Type* u32 = p->GetBitsType(32);
  XLS_ASSERT_OK_AND_ASSIGN(
      StreamingChannel * ch0,
...
TEST_F(ProcLoweringBlockEvalTest, StitchBlockWithLoopback) {
  auto p = CreatePackage();
  Type* u32 = p->GetBitsType(32);
  XLS_ASSERT_OK_AND_ASSIGN(
      StreamingChannel * ch0,
...
TEST_F(ProcLoweringBlockEvalTest, StitchBlockWithIdleOutput) {
  auto p = CreatePackage();
  Type* u32 = p->GetBitsType(32);
  XLS_ASSERT_OK_AND_ASSIGN(
      StreamingChannel * ch0,
...
absl::StatusOr<BlockEvaluationResults> EvalBlock(
  ...
  XLS_ASSIGN_OR_RETURN(GlobalChannelMap channel_map,
                       GlobalChannelMap::Create(block->package()));
  for (const std::unique_ptr<Block>& block : block->package()->blocks()) {
    GlobalChannelMap block_level_map =
        channel_map.GetBlockLevelMap(block.get());
    for (const auto& [channel, metadata] :
         block_level_map.streaming_input_channels()) {
      if (channel->supported_ops() == ChannelOps::kSendReceive) {
        continue;
      }
      ChannelSource source(*metadata.data_port, *metadata.valid_port,
                           *metadata.ready_port, 0.5, block.get(),
                           ChannelSource::BehaviorDuringReset::kIgnoreReady);
      VLOG(3) << absl::StreamFormat(
          "Adding source for channel %s with ports %s %s %s", channel->name(),
          *metadata.data_port, *metadata.valid_port, *metadata.ready_port);
      std::vector<uint64_t> cast_data_sequence;
      cast_data_sequence.reserve(inputs.at(channel->name()).size());
      for (int64_t in : inputs.at(channel->name())) {
    ...
    for (const auto& [channel, metadata] :
         block_level_map.single_value_input_channels()) {
      if (channel->supported_ops() == ChannelOps::kSendReceive) {
        continue;
      }
      for (absl::flat_hash_map<std::string, Value>& cycle_values :
        ...
        cycle_values[*metadata.data_port] = Value(UBits(
            input_iter->second.front(), metadata.type->GetFlatBitCount()));
      }
    }

    for (const auto& [channel, metadata] :
         block_level_map.streaming_output_channels()) {
      if (channel->supported_ops() == ChannelOps::kSendReceive) {
        continue;
      }
      sinks.push_back(ChannelSink(
          *metadata.data_port, *metadata.valid_port, *metadata.ready_port, 0.5,
          block.get(), ChannelSink::BehaviorDuringReset::kIgnoreValid));
      sink_names_by_data_name[sinks.back().data_name()] = channel->name();
    }
  }
  ...
  for (const std::unique_ptr<Block>& block : block->package()->blocks()) {
    GlobalChannelMap block_level_map =
        channel_map.GetBlockLevelMap(block.get());
    for (const auto& [channel, metadata] :
         block_level_map.single_value_output_channels()) {
      if (channel->supported_ops() == ChannelOps::kSendReceive) {
        continue;
      }
...
TEST_F(ProcLoweringBlockEvalTest, NestedProcsWithSingleValue) {
  // Nested procs where the inner proc does a trivial arithmetic operation.
  auto p = CreatePackage();
  Type* u32 = p->GetBitsType(32);
  XLS_ASSERT_OK_AND_ASSIGN(
...
TEST_F(ProcLoweringBlockEvalTest, NestedProcsWithConditionalSingleValueSend) {
...
TEST_F(ProcLoweringBlockEvalTest,
       ProcsWithNonblockingReceivesWithDroppingProc) {
  auto p = CreatePackage();
  Type* u32 = p->GetBitsType(32);
...
proc input_passthrough(__state: (), init={()}) {
  ...
  XLS_ASSERT_OK_AND_ASSIGN(Block * top_block, p->GetBlock("foo"));
  EXPECT_THAT(EvalBlock(top_block, {{"data_in", {5, 10}}}),
