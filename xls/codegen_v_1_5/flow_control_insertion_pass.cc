...
absl::StatusOr<bool> FlowControlInsertionPass::InsertFlowControl(
  ...
  // - All other stages start with invalid inputs.
  // - When stage N is done, its inputs are no longer valid, and stage N+1's
  //   inputs are now valid.
  // - If stage N and N+1 both finish in the current cycle, stage N+1's inputs
  //   stay valid.
  ...
  for (int64_t stage_index = 1; stage_index < block->stages().size();
    ...
    std::string inputs_valid_name =
        PipelineSignalName("inputs_valid", stage_index);
    XLS_ASSIGN_OR_RETURN(
  ...
  // - Stage N's outputs are ready if stage N+1's inputs are empty (filling a
  //   bubble) or stage N+1 is finishing in the current cycle (no stall).
