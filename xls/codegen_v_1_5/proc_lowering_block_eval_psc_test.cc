...
absl::StatusOr<bool> RunPasses(
  ...
  absl::StatusOr<bool> changed =
      ProcInstantiationLoweringPass().Run(p, options, &results, context);
  if (!changed.ok()) {
    return changed;
  }
...
proc input_passthrough<pt_data_in: bits[32] in, from_inner_proc: bits[32] out>(__state: (), init={()}) {
  ...
  next_value.23: () = next_value(state_element=__state, value=tuple.21, id=23)
}
...
top proc foo<data_in: bits[32] in, data_out: bits[32] out>(__state: (), init={()}) {
  ...
  next_value.13: () = next_value(state_element=__state, value=tuple.11, id=13)
}
)";
