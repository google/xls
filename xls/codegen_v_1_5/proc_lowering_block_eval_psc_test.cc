...
using ProcLoweringBlockEvalTest = IrTestBase;
...
verilog::CodegenOptions DefaultCodegenOptions() {
  return verilog::CodegenOptions()
      .flop_inputs(false)
      .flop_outputs(false)
      .clock_name("clk")
...
absl::StatusOr<BlockConversionPassOptions> CreateOptions(
    Package* p, SchedulingOptions scheduling_options,
...
absl::StatusOr<bool> RunBlockStitchingPass(
  ...
  absl::StatusOr<bool> changed =
      ProcInstantiationLoweringPass().Run(p, options, &results);
  if (!changed.ok()) {
    return changed;
  }
...
TEST_F(ProcLoweringBlockEvalTest, DISABLED_StitchNetworkWithDirectConnections) {
  auto p = CreatePackage();
  Type* u32 = p->GetBitsType(32);
  XLS_ASSERT_OK_AND_ASSIGN(
      StreamingChannel * ch0,
...
absl::StatusOr<BlockEvaluationResults> EvalBlock(
  ...
  for (const auto& [key, metadata] : block->GetChannelPortMetadata()) {
    ...
    ChannelSource source(*metadata.data_port, *metadata.valid_port,
                         *metadata.ready_port, 0.5, block,
                         ChannelSource::BehaviorDuringReset::kIgnoreReady);
    VLOG(3) << absl::StreamFormat(
        "Adding source for channel %s with ports %s %s %s", channel_name,
        *metadata.data_port, *metadata.valid_port, *metadata.ready_port);
    std::vector<uint64_t> cast_data_sequence;
    cast_data_sequence.reserve(inputs.at(channel_name).size());
    for (int64_t in : inputs.at(channel_name)) {
  ...
  for (const auto& [key, metadata] : block->GetChannelPortMetadata()) {
    if (metadata.channel_kind != ChannelKind::kSingleValue) {
      // Only process single value channels in this block
      continue;
    }
    for (absl::flat_hash_map<std::string, Value>& cycle_values : fixed_values) {
      auto input_iter = inputs.find(*metadata.data_port);
      if (input_iter == inputs.end()) {
        return absl::InvalidArgumentError(absl::StrFormat(
            "No input provided for channel %s", *metadata.data_port));
      }
      XLS_RET_CHECK_EQ(input_iter->second.size(), 1)
          << "Single value channels may only have a single input";
      XLS_RET_CHECK(metadata.type->IsBits());
      cycle_values[*metadata.data_port] = Value(
          UBits(input_iter->second.front(), metadata.type->GetFlatBitCount()));
    }
  }
  ...
  for (const auto& [key, metadata] : block->GetChannelPortMetadata()) {
    ...
    sink_names_by_data_name[sinks.back().data_name()] = channel_name;
  }
  ...
  for (const auto& [key, metadata] : block->GetChannelPortMetadata()) {
    ...
    std::vector<uint64_t>& channel_int_outputs =
        actual_outputs[*metadata.data_port];
    channel_int_outputs.reserve(results.outputs.size());
    for (const absl::flat_hash_map<std::string, Value>& value_map :
         results.outputs) {
      XLS_RET_CHECK(value_map.contains(*metadata.data_port));
      const Value& value = value_map.at(*metadata.data_port);
...
absl::StatusOr<std::unique_ptr<SerialProcRuntime>> CreateInterpreter(
  ...
  for (const auto& [ch_name, ch_inputs] : inputs) {
    XLS_ASSIGN_OR_RETURN(
        ChannelInterface * ch,
        top_proc->GetChannelInterface(ch_name, ChannelDirection::kReceive));
    ...
    XLS_ASSIGN_OR_RETURN(
        ChannelQueue * queue,
        interpreter->queue_manager().GetQueueByName(ch_name, top_proc->name()));
...
absl::StatusOr<std::unique_ptr<SerialProcRuntime>> EvalAndExpect(
  ...
  std::vector<ChannelInterface*> output_channels;
  absl::flat_hash_map<ChannelInstance*, int64_t> expected_output_count;
  XLS_ASSIGN_OR_RETURN(Proc * top_proc, p->GetTopAsProc());
  const ProcElaboration& elab = interpreter->elaboration();
  ProcInstance* top_proc_instance =
      elab.GetProcInstance(top_proc->name()).value();
  for (const auto& [ch_name, expected_values] : expected_outputs) {
    XLS_ASSIGN_OR_RETURN(
        ChannelInterface * ch,
        top_proc->GetChannelInterface(ch_name, ChannelDirection::kSend));
    XLS_RET_CHECK(ch->type()->IsBits());
    output_channels.push_back(ch);
    XLS_ASSIGN_OR_RETURN(ChannelInstance * channel_instance,
                         top_proc_instance->GetChannelInstance(ch_name));
    expected_output_count[channel_instance] = expected_values.size();
  }
  // Sort output channels the output expectations are done in a deterministic
  // order.
  absl::c_sort(output_channels, ChannelInterface::NameLessThan);
  ...
  for (ChannelInterface* ch : output_channels) {
    std::vector<int64_t> outputs;
    XLS_ASSIGN_OR_RETURN(ChannelQueue * queue,
                         interpreter->queue_manager().GetQueueByName(
                             ch->name(), top_proc->name()));
...
TEST_F(ProcLoweringBlockEvalTest, DISABLED_NestedProcsWithSingleValue) {
  // Nested procs where the inner proc does a trivial arithmetic operation.
  auto p = CreatePackage();
  Type* u32 = p->GetBitsType(32);
  XLS_ASSERT_OK_AND_ASSIGN(
...
TEST_F(ProcLoweringBlockEvalTest, DISABLED_SingleValueAndStreamingChannels) {
  ...
  EXPECT_THAT(EvalBlock(top_block, {{"x", {123, 22, 42}}, {"sv", {25}}}),
...
TEST_F(ProcLoweringBlockEvalTest, DISABLED_TwoSendsOneReceive) {
  ...
  XLS_ASSERT_OK_AND_ASSIGN(
      bool changed, RunBlockStitchingPass(
                        p.get(), /*top_name=*/"A",
                        /*scheduling_options=*/
...
TEST_F(ProcLoweringBlockEvalTest, DISABLED_RandomProcNetworks) {
  ...
  for (int sample = 0; sample < kNumberSamples; ++sample) {
    ...
    XLS_ASSERT_OK_AND_ASSIGN(
        bool changed, RunBlockStitchingPass(p.get(), /*top_name=*/"top_proc"));
    EXPECT_TRUE(changed);

    VLOG(1) << "Sample " << sample << " (after inlining):\n" << p->DumpIr();
    XLS_ASSERT_OK_AND_ASSIGN(Block * top_block, p->GetBlock("top_proc"));
    EXPECT_THAT(EvalBlock(top_block, inputs, /*num_cycles=*/100),
...
TEST_F(ProcLoweringBlockEvalTest, DISABLED_MultipleSends) {
  ...
  EXPECT_THAT(
      ...
      EvalBlock(top_block, {{"in", {1, 2, 3, 42, 123, 200}}}),
...
TEST_F(ProcLoweringBlockEvalTest,
  ...
  XLS_ASSERT_OK_AND_ASSIGN(
      bool changed, RunBlockStitchingPass(p.get(), /*top_name=*/"accum"));
  EXPECT_TRUE(changed);
  EXPECT_THAT(p->blocks(),
...
TEST_F(ProcLoweringBlockEvalTest, DISABLED_ProcWithAssert) {
  ...
  XLS_ASSERT_OK_AND_ASSIGN(
      bool changed, RunBlockStitchingPass(p.get(), /*top_name=*/"loopback"));
  EXPECT_TRUE(changed);
  ...
  EXPECT_THAT(EvalBlock(top_block, {{"in", {100, 200, 300}}}),
              IsOkAndHolds(BlockOutputsEq({{"out", {100, 200, 300}}})));

  // Eval with a zero input, which should trip the assertion.
  EXPECT_THAT(
      EvalBlock(top_block, {{"in", {100, 200, 300, 0}}}),
      IsOkAndHolds(Field(
...
TEST_F(ProcLoweringBlockEvalTest, DISABLED_ProcWithTrace) {
  ...
  EXPECT_THAT(
      EvalBlock(top_block, {{"in", {100, 200, 300, 0, 400, 0, 500}}},
                /*num_cycles=*/45),
      IsOkAndHolds(AllOf(
    ...
    for (bool bypass : {true, false}) {
      for (bool register_push_outputs : {true, false}) {
        for (bool register_pop_outputs : {true, false}) {
          ...
          XLS_ASSERT_OK_AND_ASSIGN(
              bool changed, RunBlockStitchingPass(p.get(), /*top_name=*/"foo"));
          EXPECT_TRUE(changed);
          EXPECT_THAT(p->blocks(),
...
TEST_F(ProcLoweringBlockEvalTest, NestedProcsUsingEmptyAfterAll) {
  // Uses tokens created using empty after_all.
  constexpr std::string_view ir_text = R"(package test

proc input_passthrough<pt_data_in: bits[32] in, from_inner_proc: bits[32] out>(__state: (), init={()}) {
  chan_interface pt_data_in(direction=receive, kind=streaming, strictness=proven_mutually_exclusive, flow_control=ready_valid, flop_kind=none)
  chan_interface from_inner_proc(direction=send, kind=streaming, strictness=proven_mutually_exclusive, flow_control=ready_valid, flop_kind=none)
  after_all.15: token = after_all(id=15)
  literal.14: bits[1] = literal(value=1, id=14)
  receive.16: (token, bits[32]) = receive(after_all.15, predicate=literal.14, channel=pt_data_in, id=16)
  ...
  next_value.23: () = next_value(param=__state, value=tuple.21, id=23)
}

top proc foo<data_in: bits[32] in, data_out: bits[32] out>(__state: (), init={()}) {
  chan_interface data_in(direction=receive, kind=streaming, strictness=proven_mutually_exclusive, flow_control=ready_valid, flop_kind=none)
  ...
  chan_interface from_inner_proc(direction=send, kind=streaming, strictness=proven_mutually_exclusive, flow_control=none, flop_kind=none)
  chan_interface from_inner_proc(direction=receive, kind=streaming, strictness=proven_mutually_exclusive, flow_control=none, flop_kind=none)
  proc_instantiation input_passthrough_inst(data_in, from_inner_proc, proc=input_passthrough)
  ...
  next_value.13: () = next_value(param=__state, value=tuple.11, id=13)
}
)";
  ...
  XLS_ASSERT_OK_AND_ASSIGN(Block * top_block, p->GetBlock("foo"));
  EXPECT_THAT(EvalBlock(top_block, {{"data_in", {6, 10}}}),
              IsOkAndHolds(BlockOutputsEq({{"data_out", {6, 10}}})));
}
