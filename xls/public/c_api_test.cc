...
TEST(XlsCApiTest, BitsToBytesWithPadding) {
  char* error_out = nullptr;
  xls_bits* bits = nullptr;
  ASSERT_TRUE(xls_bits_make_ubits(34, 0b11'0000'0000'0000'0000'0000'0000'0000,
                                  &error_out, &bits));
  uint8_t* bytes = nullptr;
  size_t byte_count = 0;
  absl::Cleanup free_memory([error_out, bits, bytes] {
...
TEST(XlsCApiTest, BitsToUint64NotFit) {
  char* error_out = nullptr;
  xls_bits* bits = nullptr;
  ASSERT_TRUE(xls_bits_make_ubits(128, 0x0A0B0C0D, &error_out, &bits));
  absl::Cleanup free_memory([error_out, bits] {
    xls_c_str_free(error_out);
    xls_bits_free(bits);
  });
  ...
  uint64_t value = 0;
  ASSERT_FALSE(xls_bits_to_uint64(bits, &error_out, &value));
}
...
TEST(XlsCApiTest, BitsToInt64NotFit) {
  char* error_out = nullptr;
  xls_bits* bits = nullptr;
  ASSERT_TRUE(xls_bits_make_ubits(128, 0x0A0B0C0D, &error_out, &bits));
  absl::Cleanup free_memory([error_out, bits] {
  ...
  int64_t value = 0;
  ASSERT_FALSE(xls_bits_to_int64(bits, &error_out, &value));
}
