...
bool AreMutuallyExclusive(Z3_context ctx, Z3_ast cond1, Z3_ast cond2) {
  ...
  absl::Cleanup solver_cleanup = [&] { Z3_solver_dec_ref(ctx, solver); };
  Z3_solver_assert(ctx, solver, cond1);
  Z3_solver_assert(ctx, solver, cond2);
  return Z3_solver_check(ctx, solver) == Z3_L_FALSE;
}
...
bool CheckSmtEncodingEquivalence(Z3_context ctx, Function* fn,
  ...
  absl::StatusOr<std::unique_ptr<solvers::z3::IrTranslator>> translator_status =
  ...
  std::unique_ptr<solvers::z3::IrTranslator> translator =
      std::move(translator_status).value();
...
absl::Status ExploreAndVerifyFunction(Z3_context ctx, Function* func,
  ...
  XLS_ASSIGN_OR_RETURN(SymExEngine engine, SymExEngine::Create(ctx));
