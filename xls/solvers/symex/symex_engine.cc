...
absl::StatusOr<std::vector<SymbolicPath>> SymExEngine::ExplorePaths(
  ...
  for (const auto& [param_name, value] :
    ...
    if (!value.IsBits()) {
      return absl::InvalidArgumentError(
          "Only Bits values are currently supported for concolic inputs");
    }
    if (!param->GetType()->IsBits()) {
      return absl::InvalidArgumentError(absl::StrCat(
          "Concolic parameter '", param_name,
          "' must have Bits type, but has type: ",
          param->GetType()->ToString()));
    }
    int64_t param_bit_count = param->GetType()->AsBitsOrDie()->bit_count();
    if (value.bits().bit_count() != param_bit_count) {
      return absl::InvalidArgumentError(absl::StrCat(
          "Concolic parameter '", param_name, "' bitwidth mismatch: expected ",
          param_bit_count, " bits, got ", value.bits().bit_count(), " bits"));
    }
    Z3_ast val_ast = solvers::z3::BitsToZ3(ctx_, value.bits());
    Z3_solver_assert(ctx_, solver, Z3_mk_eq(ctx_, param_ast, val_ast));
  }
