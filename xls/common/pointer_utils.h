...
// Unsafe cast the type-erased pointer back to the original. This requires the
// type and deleter to be the same.
template <typename T, typename Deleter = std::default_delete<T>>
std::unique_ptr<T, Deleter> RestoreType(TypeErasedUniquePtr ptr) {
  return std::unique_ptr<T, Deleter>(reinterpret_cast<T*>(ptr.release()),
                                     Deleter());
}
