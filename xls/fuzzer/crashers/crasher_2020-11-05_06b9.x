...
fn main(x0: s32, x1: u26) -> u62 {
  ...
  let x21: s11 = (((x20) as s11)) << ((u11:0x9) if ((x13) >= (s11:0x9)) else (x13 as u11));
  let x22: u30 = (x0)[0x2:];
  let x23: s32 = -(x7);
  let x24: u1 = (((x23) as u1)) >> ((u1:0x0) if ((x6) >= (u1:0x0)) else (x6));
  let x25: u1 = one_hot_sel(x15, [x8]);
  let x26: s39 = for (i, x): (u4, s39) in range(u4:0x0, u4:0x0) {
    x
  }(x3);
