...
class WorkerConfig:
  ...
  worker_number: int
  ast_generator_options: ast_pb2.AstGeneratorOptionsProto
...
def _run_worker(config: WorkerConfig):
  ...
  serialized_ast_generator_options = base64.b64encode(
      config.ast_generator_options.SerializeToString()
  ).decode()
  serialized_sample_options = base64.b64encode(
      config.sample_options.SerializeToString()
  ).decode()
  args = [
      runfiles.get_path('xls/fuzzer/run_fuzz_multiprocess_worker'),
      '--alsologtostderr',
      '--number=%d' % config.worker_number,
      '--ast_generator_options=%s' % serialized_ast_generator_options,
      '--sample_options=%s' % serialized_sample_options,
...
def parallel_generate_and_run_samples(
  ...
  for i in range(worker_count):
    config = WorkerConfig(
        ...
        sample_count=(
            None if sample_count is None else (sample_count + i) // worker_count
        ),
        duration=duration,
