...
absl::StatusOr<std::vector<InterpreterResult<Value>>> EvaluateParamSets(
    Function* f, std::vector<std::vector<Value>> param_sets) {
...
std::string StringifyResults(std::vector<InterpreterResult<Value>> results) {
  std::stringstream ss;
  ss << "[";
...
absl::StatusOr<bool> OptimizationPassEvaluator(
  ...
  XLS_ASSIGN_OR_RETURN(Function * f, p->GetFunction(IrTestBase::TestName()));
  ...
  XLS_ASSIGN_OR_RETURN(
      ...
      EvaluateParamSets(f, param_sets));
