...
std::vector<std::vector<Value>> GenRandomArgsForPackage(Package* p,
  ...
  for (Param* param : f->params()) {
    arg_sets.push_back(GenRandomArgs(arg_set_count, param->GetType()));
  }
...
void OptimizationPassChangesOutputs(
  ...
  std::vector<std::vector<Value>> arg_sets =
      GenRandomArgsForPackage(p.get(), arg_set_count);
...
absl::Status PassChangesOutputsWithProto(std::string proto_string,
  ...
  // Call the optimization pass test.
  PackageAndFuzzProgram package_and_fuzz_program(std::move(p), fuzz_program);
