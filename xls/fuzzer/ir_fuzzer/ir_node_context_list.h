...
// Specifies the type of context list for indexing.
...
class IrNodeContextList {
 public:
  IrNodeContextList() {
    // Number of lists.
    context_list_.resize(2);
  }

  BValue GetElementAt(int64_t list_idx,
                      ContextListType list_type = COMBINED_LIST);
  BValue GetLastElement(ContextListType list_type = COMBINED_LIST);
  int64_t GetListSize(ContextListType list_type = COMBINED_LIST);
  bool IsEmpty(ContextListType list_type = COMBINED_LIST);
