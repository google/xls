...
SchedulingOptionsFlagsProto DefaultSchedulingOptions() {
  ...
  scheduling_options.set_clock_period_ps(4000);
...
void CodegenSucceedsForEveryFunctionWithNoPiplineLimit(
    absl::StatusOr<std::string> verilog) {
  EXPECT_THAT(verilog, IsOkAndHolds(Not(IsEmpty())));
}
FUZZ_TEST(CodegenFuzzTest, CodegenSucceedsForEveryFunctionWithNoPiplineLimit)
    .WithDomains(VerilogFuzzDomain(DefaultSchedulingOptions(),
