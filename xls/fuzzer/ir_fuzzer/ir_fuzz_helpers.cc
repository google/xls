...
BValue ChangeBitWidth(FunctionBuilder* fb, BValue bvalue, int64_t bit_width,
                      DecreaseBitWidthMethod decrease_width_method,
                      IncreaseBitWidthMethod increase_width_method) {
  if (bvalue.BitCountOrDie() > bit_width) {
    ...
    return DecreaseBitWidth(fb, bvalue, bit_width, decrease_width_method);
    ...
    return IncreaseBitWidth(fb, bvalue, bit_width, increase_width_method);
  } else {
...
// Overloaded version of ChangeBitWidth that uses the default bit width
// modification methods.
BValue ChangeBitWidth(FunctionBuilder* fb, BValue bvalue, int64_t bit_width) {
  return ChangeBitWidth(fb, bvalue, bit_width, UNSET_DECREASE_WIDTH_METHOD,
                        UNSET_INCREASE_WIDTH_METHOD);
}
