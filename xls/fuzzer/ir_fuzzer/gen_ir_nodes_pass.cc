...
void GenIrNodesPass::HandleAdd(FuzzAddProto* add) {
  FuzzOperandRefProto* lhs_ref = add->mutable_lhs_ref();
  FuzzOperandRefProto* rhs_ref = add->mutable_rhs_ref();
  BValue lhs = HandleOperandRef(lhs_ref);
  BValue rhs = HandleOperandRef(rhs_ref);
  ...
  lhs = ChangeBitWidth(fb_, lhs, add->bit_width(),
                       lhs_ref->decrease_width_method(),
                       lhs_ref->increase_width_method());
  rhs = ChangeBitWidth(fb_, rhs, add->bit_width(),
                       rhs_ref->decrease_width_method(),
                       rhs_ref->increase_width_method());
...
BValue GenIrNodesPass::HandleOperandRef(FuzzOperandRefProto* operand_ref) {
  if (stack_.empty()) {
