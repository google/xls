...
// Commented out for now because the interpreter does not work with Select.
...
//         }
//       )");
//   ASSERT_TRUE(google::protobuf::TextFormat::ParseFromString(proto_string,
//   &fuzz_program)); IrFuzzBuilder ir_fuzz_builder(&fuzz_program, p.get(),
//   &fb); BValue ir = ir_fuzz_builder.BuildIr();
//   XLS_ASSERT_OK_AND_ASSIGN(Function * f, fb.BuildWithReturnValue(ir));
//   EXPECT_THAT(f->return_value(),
//               m::Select(m::Param("p0"), {m::SignExt(m::Param("p1")),
//                                          m::BitSlice(m::Param("p2"), 0,
//                                          20)}));
// }
...
TEST(IrFuzzBuilderTest, OneHotSelectWithLargeSelectorWidth) {
  ...
  EXPECT_THAT(
      f->return_value(),
      m::OneHotSelect(m::Literal(0, 5),
                      {m::Param("p1"), m::Literal(0, 20), m::Literal(0, 20),
                       m::Literal(0, 20), m::Literal(0, 20)}));
}
...
TEST(IrFuzzBuilderTest, PrioritySelectWithLargeSelectorWidth) {
  ...
  EXPECT_THAT(
      f->return_value(),
      m::PrioritySelect(m::Literal(0, 5),
                        {m::Param("p1"), m::Literal(0, 20), m::Literal(0, 20),
                         m::Literal(0, 20), m::Literal(0, 20)},
                        m::Param("p1")));
}
