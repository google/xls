...
// correct.
void EquateProtoToIrTest(std::string proto_string,
                         testing::Matcher<const Node*> expected_ir_node) {
  // Create the package.
  ...
  XLS_ASSERT_OK_AND_ASSIGN(Function * f, fb.BuildWithReturnValue(proto_ir));
...
TEST(IrFuzzBuilderTest, AddTwoLiterals) {
  ...
  EquateProtoToIrTest(proto_string, expected_ir_node);
}
