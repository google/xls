...
absl::StatusOr<int64_t> FfiDelayEstimator::GetOperationDelayInPs(
  ...
  const Invoke* invoke = node->As<Invoke>();
  Function* callee = invoke->to_apply();
  if (callee->ForeignFunctionData().has_value() &&
      callee->ForeignFunctionData()->has_delay_ps()) {
    return callee->ForeignFunctionData()->delay_ps();
  }
