...
namespace {
struct ChannelActionCallback final : public ChannelQueueCallback {
 public:
  explicit ChannelActionCallback(ChannelActions& actions, int64_t& act,
                                 bool& full, std::optional<int64_t> size)
      : size_(size), actions_(actions), activation_(act), full_(full) {}
  void HandleValue(const Value& value) {
    actions_.actions.push_back(ChannelActions::SingleAction{
        .value = value, .activation_number = activation_});
    if (size_) {
...
absl::StatusOr<ProcResults> GetProcResults(
  ...
  if (p->is_new_style_proc()) {
    for (const auto& channel_interface : p->interface()) {
      ...
      queue->AddCallback(std::make_unique<ChannelActionCallback>(
...
Annotation ProcResultsAnnotator::NodeAnnotation(Node* node) const {
  ...
  auto it = results_.node_values.find(node);
  if (it != results_.node_values.end()) {
    annotation.suffix = absl::StrFormat(
        " | [%v]", absl::StrJoin(it->second, ",\t",
...
Annotation ProcResultsAnnotator::ChannelAnnotation(Channel* chan) const {
  Annotation annotation;
  auto it = results_.channel_actions.find(chan);
  if (it != results_.channel_actions.end()) {
    annotation.suffix = absl::StrFormat(" | %v", it->second);
  }
  return annotation;
}
