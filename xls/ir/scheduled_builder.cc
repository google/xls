...
ScheduledFunctionBuilder::ScheduledFunctionBuilder(std::string_view name,
                                                   Package* package,
                                                   bool should_verify)
    : FunctionBuilder(name, package, ScheduledFunctionTag{}, should_verify) {
  SetNodeAddedCallback([this](Node* n) { NodeAdded(n); });
}
...
void ScheduledFunctionBuilder::NodeAdded(Node* node) {
...
ScheduledProcBuilder::ScheduledProcBuilder(std::string_view name,
                                           Package* package, bool should_verify)
    : ProcBuilder(name, package, ScheduledProcTag{}, should_verify) {
  SetNodeAddedCallback([this](Node* n) { NodeAdded(n); });
}
ScheduledProcBuilder::ScheduledProcBuilder(NewStyleProc tag,
...
void ScheduledProcBuilder::NodeAdded(Node* node) {
  node_to_stage_[node] = current_stage_;
}
