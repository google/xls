 ...
 public:
  ...
  NodeMap(It first, It last)
      : pkg_(nullptr),
        id_(-1),
        values_(std::make_unique<std::list<DataHolder*>>()) {
    for (auto& [k, v] : iter::imap(
             [this](auto& kv) { return std::make_pair(kv.first, kv.second); },
             iter::zip(iter::imap([](auto& kv) { return kv.first; }, first),
                       iter::imap([](auto& kv) { return kv.second; }, last)))) {
      this->insert(k, v);
    }
  }
