...
static const std::string_view kSubclassTemplate = R"(class {OP_CLASS_NAME} final : public Node {
 public:
{OPERAND_INDICES}
...
static std::string RenderOpIndices(const OpClass& op_class) {
  ...
  for (const OperandInfo& op_set : op_class.GetFixedOperands()) {
    lines.push_back(absl::StrFormat("  static constexpr int64_t k%sOperand = %d;", op_set.operand().CamelCaseName(), op_set.index()));
  }
...
static std::string RenderMethods(const OpClass& op_class) {
  ...
  for (const Method& method : op_class.GetMethods()) {
    lines.push_back(absl::StrFormat("  %s %s(%s)%s", method.return_cpp_type(), method.name(), method.params(), (method.is_const() ? " const" : "")));
    if (method.expression().has_value()) {
      lines.back() += " {";
      if (method.expression_is_body()) {
        lines.push_back(absl::StrFormat("    %s", method.expression().value()));
      } else {
        lines.push_back(absl::StrFormat("    return %s;", method.expression().value()));
      }
      lines.push_back("  }");
    } else {
      lines.back() += ";";
...
static std::string RenderOptionalOperands(const OpClass& op_class) {
  ...
  for (const OperandInfo op_set : op_class.GetOptionalOperands()) {
    if (!op_set.operand().AsOptionalOrDie().manual_optional_implementation()) {
      lines.push_back(absl::StrFormat("  absl::StatusOr<int64_t> %s_operand_number() const {", op_set.operand().name()));
      lines.push_back(absl::StrFormat("    if (!has_%s_) { return absl::InternalError(\"%s is not present\"); }", op_set.operand().name(), op_set.operand().name()));
      ...
      for (const OperandInfo other_optional : op_class.GetOptionalOperands()) {
        if (other_optional.index() < op_set.index()) {
          lines.push_back(absl::StrFormat("    if (!has_%s_) { ret--; }", other_optional.operand().name()));
        }
      }
...
std::string RenderNodeSubclass(const OpClass& op_class) {
  std::string constructor_decl = absl::StrFormat("  %s(%s);",
      op_class.name(), op_class.GetConstructorArgsStr());

  std::string definitely_equal_to_decl;
  if (!op_class.GetDataMembers().empty()) {
    definitely_equal_to_decl = "  bool IsDefinitelyEqualTo(const Node *other) const final;";
  }

  return absl::StrReplaceAll(kSubclassTemplate, {
