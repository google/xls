...
absl::StatusOr<ReceiveChannelRef> GetReceiveChannelRef(
  ...
  if (proc->is_new_style_proc()) {
    return proc->GetReceiveChannelInterface(node->channel_name());
  }
  return node->package()->GetChannel(node->channel_name());
}
...
absl::StatusOr<ReceiveChannelRef> Peek::GetReceiveChannelRef() const {
  return ::xls::GetReceiveChannelRef(this);
}
...
Peek::Peek(const SourceInfo& loc, Node* token,
           ...
           Type* payload_type, std::string_view name,
           FunctionBase* function)
    : ChannelNode(
          loc, Op::kPeek, GetReceiveType(
              function->package(), /*is_blocking=*/false, payload_type),
...
absl::StatusOr<Node*> Peek::CloneInNewFunction(
  ...
  return new_function->MakeNodeWithName<Peek>(
      ...
      new_operands.size() > 1 ? std::make_optional(new_operands[1])
                              : std::nullopt,
      channel_name(), GetPayloadType(), GetNameView());
}
