...
absl::StatusOr<ReceiveChannelRef> ChannelNode::GetReceiveChannelRef() const {
  FunctionBase* fb = function_base();
  if (fb->IsBlock()) {
    XLS_RET_CHECK(fb->IsScheduled());
    ScheduledBlock* sb = absl::down_cast<ScheduledBlock*>(fb);
    XLS_RET_CHECK_NE(sb->source(), nullptr);
  ...
  Proc* proc = fb->AsProcOrDie();
  if (proc->is_new_style_proc()) {
    return proc->GetReceiveChannelInterface(channel_name_);
  }
  return package()->GetChannel(channel_name_);
}
