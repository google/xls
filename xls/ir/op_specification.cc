...
std::string Operand::CamelCaseName() const {
  ...
  for (std::string_view part : parts) {
    capitalized.push_back(
        absl::StrCat(absl::AsciiStrToUpper(part.substr(0, 1)), part.substr(1)));
  }
...
std::string OpClass::GetEqualToExpr() const {
  ...
  for (const DataMember& data_member : GetDataMembers()) {
    std::string rhs =
        absl::StrFormat("other->As<%s>()->%s", name(), data_member.name);
    pieces.push_back(
        absl::StrReplaceAll(data_member.equals_tmpl,
                            {{"{lhs}", data_member.name}, {"{rhs}", rhs}}));
  }
...
std::vector<DataMember> OpClass::GetDataMembers() const {
  ...
  for (const OperandInfo& optional_operand : GetOptionalOperands()) {
    if (!optional_operand.operand()
             .AsOptionalOrDie()
             .manual_optional_implementation()) {
...
std::string OpClass::GetBaseConstructorInvocation() const {
  return absl::StrFormat("Node(%s, %s, loc, name, function)", op_,
                         xls_type_expression());
}
...
std::string OpClass::GetConstructorArgsStr() const {
  ...
  args.push_back(
      ConstructorArgument{"function", "FunctionBase*", "function()"});
...
class OperandBuilder {
 public:
  ...
  OperandBuilder& AddOptional(std::string name,
                              bool manual_optional_implementation = false) {
    operands_.push_back(std::make_unique<OptionalOperand>(
        std::move(name),
        /*manual_optional_implementation=*/manual_optional_implementation));
    return *this;
  }
...
class AttributeBuilder {
 public:
  ...
  std::vector<std::unique_ptr<Attribute>> Build() {
...
static absl::btree_map<std::string, OpClass>* CreateOpClassKindsSingleton() {
  ...
  map->emplace(
      ...
      OpClass(
          ...
          OpClassOptions{
              ...
              .extra_methods = {Method("size", "int64_t", "operand_count()")},
  ...
  map->emplace(
      ...
      OpClass{
          ...
          /*options=*/
          OpClassOptions{
              .extra_methods = {Method("array", "Node*", "operand(0)"),
                                Method("indices", "absl::Span<Node* const>",
                                       "operands().subspan(1)")},
              .custom_clone_method = true}});
  map->emplace(
      "ARRAY_SLICE",
      OpClass{/*name=*/"ArraySlice",
              /*op=*/"Op::kArraySlice",
              /*operands=*/OperandBuilder().Add("array").Add("start").Build(),
              /*xls_type_expression=*/
              "function->package()->GetArrayType(width, "
  ...
  map->emplace("ARRAY_CONCAT",
               OpClass{
                   /*name=*/"ArrayConcat",
                   /*op=*/"Op::kArrayConcat",
                   /*operands=*/OperandBuilder().AddSpan("args").Build(),
                   /*xls_type_expression=*/
                   "GetArrayConcatType(function->package(), args)",
               });
  map->emplace(
      "BIN_OP",
      OpClass{
          ...
          OpClassOptions{
              .extra_constructor_args = {ConstructorArgument{
                  .name = "op", .cpp_type = "Op", .clone_expression = "op()"}},
          }});
  map->emplace(
      "ARITH_OP",
      OpClass{/*name=*/"ArithOp",
              /*op=*/"op",
              /*operands=*/OperandBuilder().Add("lhs").Add("rhs").Build(),
  ...
  map->emplace(
      ...
      OpClass{
          ...
          /*operands=*/OperandBuilder().Add("lhs").Add("rhs").Build(),
          /*xls_type_expression=*/
          "function->package()->GetTupleType({function->package()->GetBitsType("
          "width), function->package()->GetBitsType(width)})",
          OpClassOptions{
              .attributes = AttributeBuilder().AddInt64("width").Build(),
              .extra_constructor_args = {ConstructorArgument{
                  /*name=*/"op", /*cpp_type=*/"Op",
      ...
      OpClass{
          ...
          OpClassOptions{
              .attributes = AttributeBuilder()
                                .AddString("message")
                                .AddOptionalString("label")
                                .AddOptionalString("original_label")
                                .Build(),
              .extra_methods =
                  {
              ...
              .extra_methods =
                  {
                      Method("token", "Node*", "operand(0)"),
                      Method("condition", "Node*", "operand(1)"),
                      Method("args", "absl::Span<Node* const>",
                             "operands().subspan(2)"),
                  },
              .custom_clone_method = true,
          },
  ...
  map->emplace(
      "BITWISE_REDUCTION_OP",
      OpClass{/*name=*/"BitwiseReductionOp",
              /*op=*/"op",
  ...
  map->emplace(
      ...
      OpClass{
          ...
          OperandBuilder()
              .Add("token")
              .AddOptional("predicate")
              .Build(),
          /*xls_type_expression=*/
          "GetReceiveType(function, channel_name, is_blocking)",
          OpClassOptions{
              .attributes = AttributeBuilder()
                                .AddString("channel_name")
                                .AddBool("is_blocking")
                                .Build(),
              .extra_methods =
                  {
                      Method("token", "Node*", "operand(0)"),
                      Method("predicate", "std::optional<Node*>",
                             "predicate_operand_number().ok() ? "
                             "std::optional<Node*>(operand(*predicate_operand_"
                             "number())) : std::nullopt"),
  ...
  map->emplace(
      ...
      OpClass{
          ...
          OpClassOptions{
              .attributes =
                  AttributeBuilder().AddString("channel_name").Build(),
              .extra_methods =
                  {
                      Method("token", "Node*", "operand(0)"),
              ...
              .extra_methods =
                  {
                      Method(
                          ...
                          /*expression=*/std::nullopt,
                          /*options=*/
                          MethodOptions{.params = "int64_t operandno"}),
                  },
          },
      });
  ...
  map->emplace(
      "COUNTED_FOR",
      OpClass{/*name=*/"CountedFor",
              /*op=*/"Op::kCountedFor",
              /*operands=*/
              OperandBuilder()
                  .Add("initial_value")
                  .AddSpan("invariant_args")
                  .Build(),
                  ...
                  .extra_methods =
                      {
                          ...
                          Method("invariant_args", "absl::Span<Node* const>",
                                 "operands().subspan(1)"),
                      },
                  .custom_clone_method = true,
              }});
  ...
  map->emplace("EXTEND_OP",
               OpClass{
                   /*name=*/"ExtendOp",
                   /*op=*/"op",
                   /*operands=*/OperandBuilder().Add("arg").Build(),
                   /*xls_type_expression=*/
                   "function->package()->GetBitsType(new_bit_count)",
                   OpClassOptions{
                       .attributes =
                           AttributeBuilder().AddInt64("new_bit_count").Build(),
                       .extra_constructor_args = {ConstructorArgument{
                           /*name=*/"op", /*cpp_type=*/"Op",
                           /*clone_expression=*/"op()"}},
                   },
  ...
  map->emplace(
      "ONE_HOT_SELECT",
      OpClass{/*name=*/"OneHotSelect",
              /*op=*/"Op::kOneHotSel",
              /*operands=*/
              OperandBuilder().Add("selector").AddSpan("cases").Build(),
              /*xls_type_expression=*/"cases[0]->GetType()",
              OpClassOptions{
                  .extra_methods =
                      {
                          Method("selector", "Node*", "operand(0)"),
                          Method("cases", "absl::Span<Node* const>",
                                 "operands().subspan(1)"),
                          Method("get_case", "Node*",
              ...
              .extra_methods =
                  {
                      ...
                      Method("get_case", "Node*", "cases().at(case_no)",
                             MethodOptions{
                                 .params = "int64_t case_no",
                             }),
                      Method("default_value", "Node*",
                             "operand(1 + cases_size_)"),
                  },
              .custom_clone_method = true,
          },
      });
      ...
      OpClass{
          ...
          OpClassOptions{
              .extra_constructor_args =
                  {
                      ConstructorArgument{/*name=*/"type", /*cpp_type=*/"Type*",
      ...
      OpClass{
          ...
          OpClassOptions{
              .extra_data_members =
                  {
                      DataMember("cases_size_", "int64_t", "cases.size()"),
                      DataMember("has_default_value_", "bool",
              ...
              .extra_methods =
                  {
                      ...
                      Method("default_value", "std::optional<Node*>",
                             "has_default_value_ ? "
                             "std::optional<Node*>(operands().back()) : "
                             "std::nullopt"),
                      Method("AllCases", /*return_cpp_type=*/"bool",
                             /*expression=*/std::nullopt,
                             MethodOptions{.params =
                                               "std::function<bool(Node*)> p"}),
                      Method("any_case", /*return_cpp_type=*/"Node*",
                             "!cases().empty() ? cases().front() : "
                             "default_value().has_value() ? "
  ...
  map->emplace(
      ...
      OpClass{
          ...
          /*operands=*/OperandBuilder().Add("arg").Build(),
          /*xls_type_expression=*/
          "arg->GetType()->AsTupleOrDie()->element_type(index)",
          OpClassOptions{.attributes =
                             AttributeBuilder().AddInt64("index").Build()},
      });
  ...
  map->emplace(
      ...
      OpClass{
          ...
          OpClassOptions{
              .extra_constructor_args =
                  {// `register` is a C++ keyword so an attribute of name
                   // register can't
                   // be defined. Use `reg` for the data member and constructor
                   // arg, and
                   // GetRegister for the accessor method.
                   ConstructorArgument{/*name=*/"reg", /*cpp_type=*/"Register*",
                                       /*clone_expression=*/"GetRegister()"}},
              .extra_data_members = {DataMember("reg_", "Register*", "reg")},
              .extra_methods = {Method("GetRegister", "Register*", "reg_")},
          },
      });
  ...
  map->emplace(
      ...
      OpClass{
          ...
          OperandBuilder()
              .Add("data")
              .AddOptional("load_enable")
              .AddOptional("reset")
              .Build(),
          /*xls_type_expression=*/"function->package()->GetTupleType({})",
          OpClassOptions{
              // `register` is a C++ keyword so an attribute of name register
              // can't
              // be defined. Use `reg` for the data member and constructor arg,
              // and
              // GetRegister for the accessor method.
              .extra_constructor_args = {ConstructorArgument{
                  /*name=*/"reg", /*cpp_type=*/"Register*",
                  /*clone_expression=*/"GetRegister()"}},
              .extra_data_members = {DataMember("reg_", "Register*", "reg")},
              .extra_methods =
                  {
                      Method("data", "Node*", "operand(0)"),
                      Method("load_enable", "std::optional<Node*>",
                             "load_enable_operand_number().ok() ? "
                             "std::optional<Node*>(operand(*load_enable_"
                             "operand_number())) : std::nullopt"),
                      Method("reset", "std::optional<Node*>",
                             "reset_operand_number().ok() ? "
                             "std::optional<Node*>(operand(*reset_operand_"
                             "number())) : std::nullopt"),
                      Method("GetRegister", "Register*", "reg_"),
                      Method(
                          "ReplaceExistingLoadEnable",
                          /*return_cpp_type=*/"absl::Status",
                          "has_load_enable_ ? "
                          "ReplaceOperandNumber(*load_enable_operand_number(), "
                          "new_operand) : absl::InternalError(\"Unable to "
                          "replace load enable on RegisterWrite -- register "
                          "does not have an existing load enable operand.\")",
                          MethodOptions{
                              .params = "Node* new_operand",
                              .is_const = false,
                          }),
                      Method(
                          "AddOrReplaceReset", "absl::Status",
                          R"(reg_->UpdateReset(new_reset_info);
    if (!has_reset_) {
    ...
    return ReplaceOperandNumber(*reset_operand_number(), new_reset_node);
                          ...
                          MethodOptions{
                              .expression_is_body = true,
                              .params =
                                  "Node* new_reset_node, Reset new_reset_info",
                              .is_const = false}),
                  },
          },
      });

  map->emplace("INSTANTIATION_OUTPUT",
               OpClass{/*name=*/"InstantiationOutput",
                       /*op=*/"Op::kInstantiationOutput",
                       /*operands=*/{},
                       /*xls_type_expression=*/
                       "instantiation->GetOutputPort(port_name).value().type",
                       OpClassOptions{
                           .attributes = AttributeBuilder()
...
const std::vector<Op>& GetOpsSingleton() {
  const absl::btree_map<std::string, OpClass>& op_classes =
      GetOpClassKindsSingleton();
  static const std::vector<Op>* singleton = new std::vector<Op>{
      ...
      Op{
          ...
          /*op_class=*/op_classes.at("NARY_OP"),
          /*properties=*/
          {Property::kBitwise, Property::kAssociative, Property::kCommutative},
      },
      Op{
          /*enum_name=*/"kOrReduce",
          /*name=*/"or_reduce",
