...
TEST_F(GetProcResultsTest, OldStyleProcBasic) {
  ...
  auto state = pb.StateElement("cnt", UBits(10, 4));
  ...
  EXPECT_THAT(foo_actions.values,
              testing::ElementsAre(Value(UBits(1, 4)), Value(UBits(2, 4))));
  EXPECT_THAT(foo_actions.activation_number, testing::ElementsAre(0, 1));

  const ChannelActions& ret_actions = results.channel_actions.at(ret_ch);
  EXPECT_THAT(ret_actions.values,
              testing::ElementsAre(Value(UBits(11, 4)), Value(UBits(13, 4))));
  EXPECT_THAT(ret_actions.activation_number, testing::ElementsAre(0, 1));
...
TEST_F(GetProcResultsTest, OldStyleProcBlocksOnInput) {
  ...
  const ChannelActions& foo_actions = results.channel_actions.at(foo_ch);
  EXPECT_THAT(foo_actions.values, testing::ElementsAre(Value(UBits(1, 4))));
  EXPECT_THAT(foo_actions.activation_number, testing::ElementsAre(0));

  const ChannelActions& ret_actions = results.channel_actions.at(ret_ch);
  EXPECT_THAT(ret_actions.values, testing::ElementsAre(Value(UBits(11, 4))));
  EXPECT_THAT(ret_actions.activation_number, testing::ElementsAre(0));
...
TEST_F(GetProcResultsTest, NewStyleProcBasic) {
  ...
  auto tok = pb.StateElement("tok", Value::Token());
  auto state = pb.StateElement("state", UBits(20, 8));
  ...
  EXPECT_THAT(in_actions.values,
              testing::ElementsAre(Value(UBits(1, 8)), Value(UBits(2, 8))));
  EXPECT_THAT(in_actions.activation_number, testing::ElementsAre(0, 1));

  const ChannelActions& out_actions =
      results.channel_actions.at(chan_out.channel_interface());
  EXPECT_THAT(out_actions.values,
              testing::ElementsAre(Value(UBits(21, 8)), Value(UBits(24, 8))));
  EXPECT_THAT(out_actions.activation_number, testing::ElementsAre(0, 1));
