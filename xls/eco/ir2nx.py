...
class IrParser:
    ...
    def _parse_after_all(self, line, node_name, data_type_str):
        ...
        """
        regex = re.compile(r"after_all\((?P<operands>[^)]*?),?\s*id\s*=\s*(?P<id>\d+)")
    ...
    def _parse_binary_node(self, line, node_name, data_type_str, commutative=False):
    ...
    def _parse_sel_node(self, line, node_name, data_type_str):
        ...
        operand_dtype_strs.extend(
            self.graph.nodes[operand]["cost_attributes"]["dtype_str"]
            for operand in operands
        )
        if default is not None:
            operand_dtype_strs.append(
                self.graph.nodes[default]["cost_attributes"]["dtype_str"]
            )
        operand_data_types = (
            [self.graph.nodes[selector]["data_type"]]
            + [self.graph.nodes[operand]["data_type"] for operand in operands]
            + ([self.graph.nodes[default]["data_type"]] if default is not None else [])
        )
        self.graph.add_node(
            node_name,
            op=op,
    ...
    def _parse_receive(self, line, node_name, data_type_str):
        ...
        blocking = self._str2bool(details["blocking"]) if details["blocking"] else False
        ...
        if predicate:
            ...
            node_args["cost_attributes"]["operand_dtype_str"] = (
                ...
                self.graph.nodes[details["predicate"]]["cost_attributes"]["dtype_str"],
            )
        else:
    ...
    def _parse_next_value(self, line, node_name, data_type_str):
        ...
        if predicate:
            operand_data_types.append(self.graph.nodes[predicate]["data_type"])
            operand_dtype_strs.append(
                self.graph.nodes[predicate]["cost_attributes"]["dtype_str"]
            )
        node_args["operand_data_types"] = operand_data_types
        node_args["cost_attributes"]["operand_dtype_strs"] = tuple(operand_dtype_strs)
        self.graph.add_node(node_name, **node_args)
    ...
    def _add_edge(self, source, sink, index):
        ...
        cost_attributes = {
            ...
            "source_op": self.graph.nodes[source]["cost_attributes"]["op"],
            "sink_data_type": self.graph.nodes[sink]["cost_attributes"]["dtype_str"],
            "sink_op": self.graph.nodes[sink]["cost_attributes"]["op"],
        }
        if (
