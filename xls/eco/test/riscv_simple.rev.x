...
// Revised version of the trivial RV32I subset implementation below.
// See original for full description.
...
fn run_r_instruction
    ...
    let next_regs = if rd == R0 {
        regs
    } else {
        update(regs, rd, new_value)
    };
    (pc + u32:4, next_regs, dmem)
}
...
fn run_u_instruction
    (pc: u32, ins: u32, regs: u32[REG_COUNT], dmem: u8[DMEM_SIZE])
    -> (u32, u32[REG_COUNT], u8[DMEM_SIZE]) {
    let (imm20, rd, _opcode) = decode_u_instruction(ins);
    let new_value = (imm20 as u32) << u32:12;
    let next_regs = if rd == R0 {
        regs
    } else {
        update(regs, rd, new_value)
...
fn run_uj_instruction
    (pc: u32, ins: u32, regs: u32[REG_COUNT], dmem: u8[DMEM_SIZE])
    -> (u32, u32[REG_COUNT], u8[DMEM_SIZE]) {
    let (imm20, rd, _opcode) = decode_j_instruction(ins);
    let new_value = pc + u32:4;
    let next_regs = if rd == R0 {
        regs
    } else {
        update(regs, rd, new_value)
    };
    (pc + signex(imm20 ++ u1:0, u32:0), next_regs, dmem)
}
...
fn run_instruction
    ...
    // Removed the redundant R0 reset here, as per-instruction handling is now in place.
    (pc, regs, dmem)
}
