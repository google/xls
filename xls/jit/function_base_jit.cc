...
class LlvmFunctionWrapper {
 public:
  static LlvmFunctionWrapper Create(absl::string_view name,
                                    absl::Span<Node* const> input_args,
    ...
    llvm::FunctionType* function_type = llvm::FunctionType::get(
        llvm::Type::getVoidTy(jit_context.context()), param_types,
        /*isVarArg=*/false);
...
std::vector<std::vector<Node*>> PartitionFunctionBase(FunctionBase* f) {
  ...
  for (Node* node : TopoSort(f)) {
    ...
    partitions[partition].push_back(node);
  }
...
absl::StatusOr<llvm::Function*> BuildPartitionFunction(
    absl::string_view name, absl::Span<Node* const> nodes,
  ...
  LlvmFunctionWrapper wrapper = LlvmFunctionWrapper::Create(
      name, global_input_nodes, global_output_nodes, jit_context);
  ...
  for (Node* node : nodes) {
    ...
    b.CreateCall(node_function.function, args);
  }
  b.CreateRetVoid();
...
absl::Status AllocateTempBuffers(
    absl::Span<const std::vector<Node*>> partitions,
    const LlvmFunctionWrapper& wrapper, TempBufferAllocator& allocator) {
  for (const std::vector<Node*>& partition : partitions) {
    absl::flat_hash_set<Node*> partition_set(partition.begin(),
                                             partition.end());
...
// size scaling issues. The jitted function implementing `f` might look like:
//
...
// memory use errors.
absl::StatusOr<llvm::Function*> BuildFunctionInternal(
  ...
  LlvmFunctionWrapper wrapper = LlvmFunctionWrapper::Create(
      xls_function->name(), inputs, outputs, jit_context);
  ...
  for (int64_t i = 0; i < partitions.size(); ++i) {
    const std::vector<Node*>& partition = partitions[i];
    std::string name =
        absl::StrFormat("__%s_partition_%d", xls_function->name(), i);
    XLS_ASSIGN_OR_RETURN(
        llvm::Function * partition_function,
        BuildPartitionFunction(name, partition, inputs, outputs, allocator,
                               jit_context));
  ...
  for (llvm::Function* partition_function : partition_functions) {
    wrapper.entry_builder().CreateCall(wrapper.function_type(),
                                       partition_function, args);
  }
  ...
  if (unpoison_outputs) {
    for (Node* output : outputs) {
      ...
      llvm::Value* output_buffer = LoadPointerFromPointerArray(
          index, wrapper.GetOutputsArg(), &wrapper.entry_builder());
  ...
  wrapper.entry_builder().CreateRetVoid();
  return wrapper.function();
}
...
absl::StatusOr<llvm::Function*> BuildPackedWrapper(
  ...
  LlvmFunctionWrapper wrapper = LlvmFunctionWrapper::Create(
      ...
      GetJittedFunctionOutputs(xls_function), jit_context);
...
absl::StatusOr<JittedFunctionBase> BuildFunctionAndDependencies(
  ...
  for (FunctionBase* f : functions) {
    ...
    jit_context.SetLlvmFunction(f, llvm_function);
    if (f == xls_function) {
      top_function = llvm_function;
    }
  }
...
absl::StatusOr<JittedFunctionBase> BuildProcFunction(
  ...
  JitBuilderContext jit_context(orc_jit, queue_mgr, recv_fn, send_fn);
