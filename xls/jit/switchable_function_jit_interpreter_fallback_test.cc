...
class SwitchableFunctionJitInterpreterFallbackTest : public IrTestBase {
 public:
...
TEST_F(SwitchableFunctionJitInterpreterFallbackTest, CanExecuteInterpreter) {
...
TEST_F(SwitchableFunctionJitInterpreterFallbackTest, CanExecuteJit) {
  auto p = CreatePackage();
  XLS_ASSERT_OK_AND_ASSIGN(auto f, TestFunction(p.get()));
...
TEST_F(SwitchableFunctionJitInterpreterFallbackTest, CanExecuteDefault) {
  auto p = CreatePackage();
  XLS_ASSERT_OK_AND_ASSIGN(auto f, TestFunction(p.get()));

  XLS_ASSERT_OK_AND_ASSIGN(auto runner, SwitchableFunctionJit::Create(f));
  EXPECT_FALSE(runner->function_jit().has_value());
  XLS_ASSERT_OK_AND_ASSIGN(
      auto result,
      runner->Run(std::vector<Value>{Value(UBits(8, 8)), Value(UBits(4, 8))}));
  EXPECT_EQ(result.value,
