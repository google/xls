...
class EvalMainLLVMTest(parameterized.TestCase):
  ...
  @parameterized.named_parameters(
      ('top_is_main', 'main'), ('top_is_write', 'write')
  )
  def test_generate_main_can_call_top_posix_functions(self, top_function: str):
    ir_file = self.create_tempfile(content=f"""package foo

top fn {top_function}(x: bits[32], y: bits[32]) -> bits[32] {{
  ret add.1: bits[32] = add(x, y)
}}
""")
    main_wrapper = self.create_tempfile()
