...
top proc ObserverTest<in: bits[32] in, out: bits[32] out>(st: bits[32], init={0}) {
  ...
  chan_interface out(direction=send, kind=streaming, strictness=proven_mutually_exclusive, flow_control=ready_valid, flop_kind=none)
...
top proc SingleValueTest<in: bits[32] in, out: bits[32] out>(st: (), init={()}) {
  chan_interface in(direction=receive, kind=single_value, flow_control=none, flop_kind=none)
  chan_interface out(direction=send, kind=streaming, strictness=proven_mutually_exclusive, flow_control=ready_valid, flop_kind=none)
  literal.2: token = literal(value=token, id=2)
  receive.3: (token, bits[32], bits[1]) = receive(literal.2, channel=in, blocking=false, id=3)
  tuple_index.6: bits[32] = tuple_index(receive.3, index=1, id=6)
