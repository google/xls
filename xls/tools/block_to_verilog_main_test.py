...
class BlockToVerilogMainTest(absltest.TestCase):
  ...
  def test_inline_residual_data_affects_inlining(self):
    # Small block exercising multiple ops combined via OR. We try different
    # inline sets and check the combined expression line in Verilog.
    ir_file = self.create_tempfile(content=INLINE_OR_IR)

    def run_with_inline_specs(node_specs, name: str):
      """Writes residual for inline_or with `node_specs` and returns Verilog text."""
      ref_inline = self.write_residual_data(
          'inline_or', node_specs, f'{name}.textproto')
    ...
    # Inline a different subset of expressions.
    v = run_with_inline_specs([
    ...
    # No nodes inlined: expect all named assignments and combined uses names.
    v = run_with_inline_specs([
        NodeMetadata('and_ab', 3, False),
        NodeMetadata('xor_ab', 4, False),
        NodeMetadata('not_a', 5, False),
