...
proc node<ROW: u32, COL: u32> {
    ...
    next(weight: F32) {
        ...
        let is_weight_undefined = float32::is_nan(weight);
        let (weight_tok, weight) = recv_if(token(), load_weight, is_weight_undefined, weight);
...
proc matmul<ROWS: u32, COLS: u32> {
    ...
    store_weights: chan<F32>[COLS][ROWS] out;
    ...
    config(weights: chan<F32[COLS][ROWS]> in, activations: chan<F32>[ROWS] in,
           results: chan<F32>[COLS] out) {
        let (store_weights, load_weights) = chan<F32>[COLS][ROWS]("weights");
        ...
        unroll_for! (row, _): (u32, ()) in u32:0..ROWS {
            unroll_for! (col, _): (u32, ()) in u32:0..COLS {
                spawn node<row, col>(
                    load_weights[row][col], from_wests[row][col], from_norths[row][col],
                    to_easts[row][col + u32:1], to_souths[row + u32:1][col]);
            }(());
        }(());
        (weights, store_weights, activations, results, from_wests, to_easts, from_norths, to_souths)
    }
    ...
    next(weight_loaded: bool) {
        ...
        // Note: this is a one-of operation that happens on the first activation.
        let (tok, weight_loaded) = if !weight_loaded {
            let (recv_weights_tok, weights_matrix) = recv(token(), weights);
            let tok = unroll_for! (row, tok): (u32, token) in u32:0..ROWS {
                unroll_for! (col, tok): (u32, token) in u32:0..COLS {
                    let send_weight_tok =
                        send(recv_weights_tok, store_weights[row][col], weights_matrix[row][col]);
                    join(send_weight_tok, tok)
                }(tok)
            }(token());
            (tok, true)
        } else {
