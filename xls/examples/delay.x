...
struct DelayState
    <DATA_WIDTH: u32,
     ADDR_WIDTH: u32 // Current index into the RAM
     > {
    idx: bits[ADDR_WIDTH],
...
proc Delay0or1<DATA_WIDTH: u32, DELAY_IS_ONE: bool, INIT_DATA: u32> {
    ...
    prev_recv: bits[DATA_WIDTH],
}

impl Delay0or1<DATA_WIDTH, DELAY_IS_ONE, INIT_DATA> {
    fn new(data_in: chan<bits[DATA_WIDTH]> in, data_out: chan<bits[DATA_WIDTH]> out) -> Self {
        Delay0or1 { data_in, data_out, prev_recv: INIT_DATA }
    }

    fn next(self) {
        let prev_recv = read(self.prev_recv);
        let (recv_tok, next_recv) = recv(join(), self.data_in);
        let to_send = if DELAY_IS_ONE { prev_recv } else { next_recv };
        trace!(DELAY_IS_ONE);
        let send_tok = send(join(), self.data_out, to_send);
        write(self.prev_recv, (next_recv));
    }
}
...
proc DelayInternal<DATA_WIDTH: u32, DELAY: u32, INIT_DATA: u32 = {u32:0},
    ...
    ram_wr_comp: chan<()> in,
    state: DelayState<DATA_WIDTH, ADDR_WIDTH>,
}

impl DelayInternal<DATA_WIDTH, DELAY, INIT_DATA, ADDR_WIDTH, DOUBLE_DATA_WIDTH, HALF_FLOOR_DELAY> {
    type DelayInternalRamReq = RamReq<ADDR_WIDTH, DOUBLE_DATA_WIDTH>;

    fn new
        (data_in: chan<bits[DATA_WIDTH]> in, data_out: chan<bits[DATA_WIDTH]> out,
         ram_req: chan<RamReq<ADDR_WIDTH, DOUBLE_DATA_WIDTH>> out,
         ram_resp: chan<RamResp<DOUBLE_DATA_WIDTH>> in, ram_wr_comp: chan<()> in) -> Self {
        DelayInternal {
            data_in,
            data_out,
            ram_req,
    ...
    fn next(self) {
        ...
        let (tok, next_write) = recv(join(), self.data_in);

        let data = next_write ++ state.prev_write;

        let tok = send(
            tok, self.ram_req,
            DelayInternalRamReq { addr: state.idx, data, write_mask: (), read_mask: (), we, re });
        let zero_resp = RamResp<DOUBLE_DATA_WIDTH> { data: bits[DOUBLE_DATA_WIDTH]:0 };
        let (read_tok, resp) = recv_if(tok, self.ram_resp, re, zero_resp);
        let empty_tuple: () = ();
        let (write_tok, _) = recv_if(tok, self.ram_wr_comp, we, empty_tuple);
        let tok = join(read_tok, write_tok);

        let current_data = if state.init_done {
            if state.is_read_stage { resp.data[0:(DATA_WIDTH as s32)] } else { state.prev_read }
        } else {
            INIT_DATA
        };

        let tok = send(read_tok, self.data_out, current_data);
        write(
            self.state,
...
pub proc Delay<DATA_WIDTH: u32, DELAY: u32, INIT_DATA: u32 = {u32:0},
               ADDR_WIDTH: u32 = {addr_width(half_floor(DELAY))},
               DOUBLE_DATA_WIDTH: u32 = {double(DATA_WIDTH)},
               HALF_FLOOR_DELAY: u32 = {half_floor(DELAY)}, DELAY_IS_ODD: bool = {is_odd(DELAY)}> {
    data_in: chan<bits[DATA_WIDTH]> in,
    data_out: chan<bits[DATA_WIDTH]> out,
    ram_req: chan<RamReq<ADDR_WIDTH, DOUBLE_DATA_WIDTH>> out,
    ram_resp: chan<RamResp<DOUBLE_DATA_WIDTH>> in,
...
impl Delay<DATA_WIDTH, DELAY, INIT_DATA, ADDR_WIDTH, DOUBLE_DATA_WIDTH, HALF_FLOOR_DELAY, DELAY_IS_ODD> {
    fn new
        (data_in: chan<bits[DATA_WIDTH]> in, data_out: chan<bits[DATA_WIDTH]> out,
         ram_req: chan<RamReq<ADDR_WIDTH, DOUBLE_DATA_WIDTH>> out,
         ram_resp: chan<RamResp<DOUBLE_DATA_WIDTH>> in, ram_wr_comp: chan<()> in) -> Self {
        let (internal_data_s, internal_data_r) = chan<bits[DATA_WIDTH]>("even_delay");
        DelayInternal<DATA_WIDTH, DELAY, INIT_DATA, ADDR_WIDTH, DOUBLE_DATA_WIDTH, HALF_FLOOR_DELAY>::new(
            data_in, internal_data_s, ram_req, ram_resp, ram_wr_comp).spawn(
            );
        Delay0or1<DATA_WIDTH, DELAY_IS_ODD, INIT_DATA>::new(internal_data_r, data_out).spawn();
        Delay { data_in, data_out, ram_req, ram_resp, ram_wr_comp }
    }
}

// Define a concretized Delay for codegen.
type DelayRamReq32x2048 = RamReq<10, 64>;
...
impl Delay32x2048_init3 {
    fn new
        (data_in: chan<u32> in, data_out: chan<u32> out, ram_req: chan<DelayRamReq32x2048> out,
         ram_resp: chan<DelayRamResp32x2048> in, ram_wr_comp: chan<()> in) -> Self {
        Delay<u32:32, u32:2048, u32:3>::new(data_in, data_out, ram_req, ram_resp, ram_wr_comp).spawn(
            );
        Delay32x2048_init3 { data_in, data_out, ram_req, ram_resp, ram_wr_comp }
    }
}
...
impl delay_smoke_test_even {
    fn new(terminator: chan<bool> out) -> Self {
        let (ram_req_s, ram_req_r) = chan<RamReq<10, 64>>("ram_req");
        let (ram_resp_s, ram_resp_r) = chan<RamResp<64>>("ram_resp");
        let (ram_wr_comp_s, ram_wr_comp_r) = chan<()>("ram_wr_comp");
        ram::SinglePortRamModel<u32:64, u32:1024>::new(ram_req_r, ram_resp_s, ram_wr_comp_s).spawn();

        let (data_in_s, data_in_r) = chan<u32>("data_in");
        let (data_out_s, data_out_r) = chan<u32>("data_out");
        Delay<u32:32, TEST0_DELAY, u32:3>::new(
            data_in_r, data_out_s, ram_req_s, ram_resp_r, ram_wr_comp_r).spawn(
            );
        delay_smoke_test_even { data_in_r: data_in_s, data_out_s: data_out_r, terminator }
    }

    fn next(self) {
        let stok = for (i, tok): (u32, token) in u32:0..TEST0_DELAY * u32:5 {
            trace!(i);
            send(tok, self.data_in_r, i)
        }(join());
        // first, receive the inits
        let rtok = for (i, tok): (u32, token) in u32:0..TEST0_DELAY {
            trace!(i);
            let (tok, result) = recv(tok, self.data_out_s);
...
impl delay_smoke_test_odd {
    fn new(terminator: chan<bool> out) -> Self {
        ...
        let (data_in_s, data_in_r) = chan<u32>("data_in");
        let (data_out_s, data_out_r) = chan<u32>("data_out");
        Delay<u32:32, TEST1_DELAY, u32:3>::new(
            data_in_r, data_out_s, ram_req_s, ram_resp_r, ram_wr_comp_r).spawn(
            );
        delay_smoke_test_odd { data_in_r: data_in_s, data_out_s: data_out_r, terminator }
    }
