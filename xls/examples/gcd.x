...
fn gcd_binary_match<N: u32>(a: uN[N], b: uN[N], d: uN[N]) -> (uN[N], uN[N], uN[N]) {
    // TODO(proppy): switch to match express once https://github.com/google/xls/issues/934 is fixed
    if ((a[0:1] == u1:0) && (b[0:1] == u1:1)) {
        (b, a >> 1, d)
    } else if ((a[0:1] == u1:1) && (b[0:1] == u1:0)) {
        (a, b >> 1, d)
    } else if ((a[0:1] == u1:0) && (b[0:1] == u1:0)) {
        (a >> 1, b >> 1, d+uN[N]:1)
    } else { // ((a[0:1] == u1:1) && (b[0:1] == u1:1))
        ((a - b) >> 1, b, d)
    }
}
...
fn gcd_euclidean_test() {
    let _ = assert_eq(u8:6, gcd_euclidean(u8:48, u8:18));
    let _ = assert_eq(u8:6, gcd_euclidean(u8:18, u8:48));
    _
}

#[test]
fn gcd_binary_test() {
    let _ = assert_eq(u8:6, gcd_binary(u8:48, u8:18));
    let _ = assert_eq(u8:6, gcd_binary(u8:18, u8:48));
    _
}
