...
fn byte_array_shl_test() {
    assert_eq(
        u8[4]:[u8:0, u8:1, u8:2, u8:3], byte_array_shl<u32:0>(u8[4]:[u8:0, u8:1, u8:2, u8:3], u8:7));
    assert_eq(
        u8[4]:[u8:1, u8:2, u8:3, u8:7], byte_array_shl<u32:1>(u8[4]:[u8:0, u8:1, u8:2, u8:3], u8:7));
    assert_eq(
        u8[4]:[u8:2, u8:3, u8:7, u8:7], byte_array_shl<u32:2>(u8[4]:[u8:0, u8:1, u8:2, u8:3], u8:7));
    assert_eq(
        u8[4]:[u8:3, u8:7, u8:7, u8:7], byte_array_shl<u32:3>(u8[4]:[u8:0, u8:1, u8:2, u8:3], u8:7));
    assert_eq(
        u8[4]:[u8:7, u8:7, u8:7, u8:7], byte_array_shl<u32:4>(u8[4]:[u8:0, u8:1, u8:2, u8:3], u8:7));
}
...
pub proc varint_streaming_u32_decode<INPUT_BYTES: u32, OUTPUT_WORDS: u32, BIG_SHIFT: u32,
                                     ...
                                     COMBINED_BYTES_WIDTH:
                                     u32 = {
                                         std::clog2(COMBINED_BYTES + u32:1)}> {
    bytes_in: chan<(u8[INPUT_BYTES], uN[INPUT_BYTES_WIDTH])> in,
    words_out: chan<(u32[OUTPUT_WORDS], uN[OUTPUT_WORDS_WIDTH])> out,
    state: State<INPUT_BYTES, INPUT_BYTES_WIDTH>,
}

impl varint_streaming_u32_decode<INPUT_BYTES, OUTPUT_WORDS, BIG_SHIFT, INPUT_BYTES_WIDTH, OUTPUT_WORDS_WIDTH, BIG_SHIFT_WIDTH, COMBINED_BYTES, COMBINED_BYTES_WIDTH> {
    fn new
        (bytes_in: chan<(u8[INPUT_BYTES], uN[INPUT_BYTES_WIDTH])> in,
         words_out: chan<(u32[OUTPUT_WORDS], uN[OUTPUT_WORDS_WIDTH])> out) -> Self {
        varint_streaming_u32_decode {
            bytes_in,
            words_out,
            state:
                {
    ...
    fn next(self) {
        ...
        let (input_tok, (input_data, input_len)) =
            recv_if(join(), self.bytes_in, do_input, (u8[INPUT_BYTES]:[u8:0, ...], InputIdx:0));
        ...
        // Do a big shift if we're dropping a big shift's worth of bytes or if the last terminator
        // is at or after the end of the big shift's window.
        ...
        let (next_old_bytes, next_old_bytes_len) = if do_input {
            ...
            let next_old_bytes_len = if state.old_bytes_len < u3:4 {
                state.old_bytes_len + u3:1
            } else {
                state.old_bytes_len
            };
        ...
        trace_fmt!("next_old_bytes calc: do_input={} do_big_shift={} num_terminators={}",
        do_input, do_big_shift, num_terminators);
        ...
        let (output_words, num_output_words, _) =
            for (i, (output_words, num_output_words, bytes_taken)): (
                u32, (OutputWordArray, OutputIdx, CombinedIdx)
            ) in u32:0..OUTPUT_WORDS {
                ...
                trace_fmt!("encoded={}, decoded={}, this_bytes_taken={}, total_bytes_taken={}, bytes_len={}",
                encoded, decoded, this_bytes_taken, total_bytes_taken, bytes_len);
                if total_bytes_taken <= bytes_len {
                    (
                        update(output_words, i, decoded), num_output_words + OutputIdx:1,
        ...
        let output_tok = send_if(
            input_tok, self.words_out, !do_input && !do_drop && num_output_words > OutputIdx:0,
...
impl varint_streaming_u32_decode_test {
    fn new(terminator: chan<bool> out) -> Self {
        let (bytes_s, bytes_r) = chan<(u8[TEST_INPUT_BYTES], uN[TEST_INPUT_BYTES_WIDTH])>("bytes");
        let (words_s, words_r) =
            chan<(u32[TEST_OUTPUT_WORDS], uN[TEST_OUTPUT_WORDS_WIDTH])>("words");
        varint_streaming_u32_decode<TEST_INPUT_BYTES, TEST_OUTPUT_WORDS, TEST_BIG_SHIFT>::new(
            bytes_r, words_s).spawn(
            );
        varint_streaming_u32_decode_test { bytes_out: bytes_s, words_in: words_r, terminator }
    }

    fn next(self) {
        // Pump in a bunch of small numbers.
        let tok = for (_, tok): (u32, token) in u32:0..u32:100 {
            send(tok, self.bytes_out, (u8[7]:[u8:0, u8:1, u8:0, u8:1, u8:0, u8:1, u8:0], u3:7))
        }(join());
        let tok = for (_, tok): (u32, token) in u32:0..u32:100 {
            let (tok, (recv_bytes, bytes_recvd)) = recv(tok, self.words_in);
        ...
        let tok = send(tok, self.bytes_out, (u8[7]:[u8:172, u8:0, ...], u3:1));
        let tok = send(tok, self.bytes_out, (u8[7]:[u8:2, u8:0, ...], u3:1));
        let (tok, (recv_bytes, bytes_recvd)) = recv(tok, self.words_in);
        assert_eq(bytes_recvd, uN[TEST_OUTPUT_WORDS_WIDTH]:1);
        ...
        let tok = send(
            ...
            (
                u8[7]:[
                    u8:1,
                    // WORD 5 = 2^27
                    u8:128, u8:128, u8:128, u8:64, u8:0, u8:0,
                ],
                u3:5
            ));
