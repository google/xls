...
std::ostream& DumpPipline(std::ostream& os,
  ...
  for (const auto& [channel, ops] : channel_operations) {
    ...
    for (const auto& op : ops) {
      ...
      if (op.op == Op::kReceive) {
        os << channel_name << " -> " << op_name(op) << ";\n";
      } else {
        os << op_name(op) << " -> " << channel_name << ";\n";
      }
    }
  }
  return os;
...
std::ostream& DumpToDot(
  ...
  for (const auto& [name, schedule] : schedule.schedules()) {
    DumpPipline(os, schedule, collapse_functions, strip_prefix);
  }
...
absl::Status RealMain(std::string_view ir_path, std::string_view schedule_path,
  ...
  DumpToDot(std::cout, schedule_proto, channel_operations, collapse_functions,
            strip_prefix);
...
int main(int argc, char** argv) {
  ...
  QCHECK_EQ(positional_arguments.size(), 2) << "Exected 2 postinal arguments.";
