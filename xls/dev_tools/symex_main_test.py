...
class SymexMainTest(test_base.TestCase):
  ...
  def test_missing_input_file_error(self):
    ...
    self.assertNotEqual(proc.returncode, 0)
    self.assertIn('Expected exactly one positional argument', proc.stderr)

  def test_extra_positional_args_error(self):
    proc = subprocess.run(
        [
            SYMEX_MAIN_PATH,
            self.ir_file.full_path,
  ...
  def test_reads_ir_from_stdin(self):
    output = subprocess.check_output(
        [
            SYMEX_MAIN_PATH,
            '-',
  ...
  def test_typed_concrete_input_type_mismatch_error(self):
    proc = subprocess.run(
        [
            SYMEX_MAIN_PATH,
            self.ir_file.full_path,
            '--top=execute_alu',
            '--concrete_inputs=op=bits[8]:0',
        ],
        capture_output=True,
        text=True,
        check=False,
    ...
    self.assertNotEqual(proc.returncode, 0)
    self.assertIn('Type mismatch for parameter', proc.stderr)
