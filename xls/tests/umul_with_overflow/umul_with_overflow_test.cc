...
RC_GTEST_PROP(UmulOverflowRapidcheck, UmulOverflow21Test, ()) {
  XLS_ASSERT_OK_AND_ASSIGN(std::unique_ptr<xls::test::UmulWithOverflow212118> f,
                           xls::test::UmulWithOverflow212118::Create());

  int64_t x = *rc::gen::inRange(0, 0x20'0000).as("x");
  int64_t y = *rc::gen::inRange(0, 0x4'0000).as("y");
  uint8_t result[8] = {0};
  ...
  RC_ASSERT(overflow == expected_overflow);
  RC_ASSERT(product == expected_product);
}

RC_GTEST_PROP(UmulOverflowRapidcheck, UmulOverflow35Test, ()) {
  XLS_ASSERT_OK_AND_ASSIGN(std::unique_ptr<xls::test::UmulWithOverflow353218> f,
                           xls::test::UmulWithOverflow353218::Create());

  int64_t x = *rc::gen::inRange(0l, 0x1'0000'0000l).as("x");
  int64_t y = *rc::gen::inRange(0l, 0x4'0000l).as("y");
  uint8_t result[16] = {0};
