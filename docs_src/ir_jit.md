...
## Understanding XLS IR Execution Flow

XLS Intermediate Representation (IR) serves as the central abstraction for modeling computations in the XLS compiler. It represents programs as a directed graph of operations (nodes), where each node corresponds to a computation such as arithmetic, bitwise logic, or data movement.
...
- Executes IR directly without compilation
- Useful for debugging and validating correctness
- Provides a simple and transparent execution model


### JIT Execution (LLVM-based)

The JIT backend converts XLS IR into LLVM IR and then compiles it into native machine code.

- Provides significantly faster execution compared to interpretation
- Suitable for evaluating performance-critical computations
- Used internally by tools such as `eval_ir_main`

This approach bridges high-level IR evaluation with efficient native execution.
...
This is the stage where software-like computation is transformed into hardware.


### Complete Execution Flow
DSLX → IR → (Interpret / JIT / Optimize) → Codegen → Verilog


### Why this matters

Understanding the different IR execution paths is important because:

- It allows developers to debug and validate designs before hardware generation
- It enables performance comparisons between execution strategies
- It forms the foundation for implementing compiler optimizations and hardware-aware transformations

This knowledge is especially useful for contributors working on extending XLS toward more complex workloads such as machine learning and accelerator-oriented designs.
