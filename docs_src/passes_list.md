...
1.  **Truth Table Generation**: For a given Boolean expression, the pass
    ...
    currently
    performed for up to 3 frontier nodes (X, Y, Z), resulting in an 8-bit
    truth table.
...
`recv0` in activation `N` must complete before `recv1` in activation `N` can
proceed, and vice-versa, thereby enforcing a total order of access across
...
`kLeafOnly`, `callee` would be inlined into `caller`, but if `callee` itself
called another function, that inner function would not be inlined unless it
was also a leaf function.
...
2.  **Minimum Cut Analysis for Selector**: The core of this pass involves a
    ...
    *   A crucial parameter, `max_unknown_bits`, is used with
        ...
        bits (bits whose
        values are not fixed to 0 or 1) allowed in the `min_cut`. If the
        number of unknown bits exceeds this limit, the cut is considered too
        large to be efficiently represented as a LUT, and the optimization is
        not performed. This prevents the generation of excessively large LUTs
        that might consume too many hardware resources.
...
fn simple_select(x: bits[3]) -> bits[3] {
  ...
  ret result: bits[3] = sel(x, cases=[
    ...
    literal(2),
    literal(1),
    // Fallback or computed values for higher x, effectively original x if
    // default is x
    literal(x),
    literal(x),
    literal(x),
...
1. **Removal of Useless Conditional I/O:** If a `send` or `receive`
   ...
   data input is replaced with a literal zero to avoid altering the
   channel's behavior in a way that might break subsequent passes or
   assumptions about channel liveness.
